 ...
 101  CONTINUE
     ...
     >  'REAL H1 EXPECTED')
      ...
      ELSE IF(CARLIR(1:4) .EQ. 'GLOB') THEN
        IGLOB=1
* Add EDEP keyword for EVOGET : choice of energy deposition mode to be ued in rates normalization
* KEEP EDEP 0 as NOGL, KEEP EDEP 1 as GLOB, ADD EDEP 2 as new Serpent2/OpenMC like normalization mode bsed on fission q values 
      ELSE IF(CARLIR(1:4) .EQ. 'EDEP') THEN
        CALL REDGET(ITYPLU,INTLIR,REALIR,CARLIR,DBLLIR)
        IF(ITYPLU .NE. 1) GO TO 101
        IGLOB=INTLIR
        IF((IGLOB.NE.0).AND.(IGLOB.NE.1).AND.(IGLOB.NE.2)) THEN
          CALL XABORT('EVOGET: INVALID EDEP INDEX.')
        ENDIF     
