...
*
  ...
  100 CONTINUE
      ...
      IF(NIN.LE.0) THEN
        WRITE (HSMG,'(36HLIBTR2: UNABLE TO OPEN LIBRARY FILE ,A12,1H.)')
     1  NAMFIL
        CALL XABORT(HSMG)
      ENDIF
*----
...
*     -------------------
      IER=KDRCLS(NIN,1)
      IF(IER.LT.0) THEN
        WRITE (HSMG,'(37HLIBTR2: UNABLE TO CLOSE LIBRARY FILE ,A12,1H.
     1  )') NAMFIL
        CALL XABORT(HSMG)
      ENDIF
*----
...
*
  ...
  900 FORMAT(/53H AVAILABLE IDENTIFIERS OF REACTION MATRICES FOR TYPE ,
     1 A6,14H AND MATERIAL ,A6,1H:/(1X,18A7))
  920 FORMAT(/33H PROCESSING MATXS2 LIBRARY NAMED ,A12,1H.)
