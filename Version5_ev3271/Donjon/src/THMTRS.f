*DECK THMTRS
     ...
     > CFLUX,POROS,FNFU,NFD,NDTOT,IFLUID,SNAME,SCOMP,FCOOL,FFUEL,ACOOL,
...
*CGT TODO: GET SATURATION TEMPERATURE FROM MSTPDB. GET ALSO FREEZING??
     ...
     1   40H K) GREATER THAN SATURATION TEMPERATURE.)') TINLET
      ...
      ELSE IF(IFLUID.EQ.2) THEN
        CALL THMSPT(SNAME,SCOMP,TINLET,RHOIN0,HINLE0,R3,R4,R5,IMPX)
      ENDIF
      ...
      ELSE IF(IFLUID.EQ.2) THEN
        CALL THMSPT(SNAME,SCOMP,TINLET,RHOINL,HINLET,R3,MUIN,CPVIN,IMPX)
      ENDIF
      MFLXIN=SPDIN*RHOINL
...
*CGT
     ...
     >    SNAME,SCOMP,IHCONV,KHCONV,ISUBM,RAD(NDTOT-1,K),ZF,PHI(K),
     >    XFL(K),
...
*----
      ...
      DO K=1,NZ
        ...
        IF(TCOOL(K).LT.TB) THEN
          ...
          ELSE IF(IFLUID.EQ.2) THEN
           CALL THMSPT(SNAME,SCOMP,TCOOL(K),R11,H11,K11,MUT(K),C11,IMPX)
          ENDIF
        ELSE
          IF(IFLUID.EQ.0) THEN
