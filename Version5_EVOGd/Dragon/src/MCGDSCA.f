*DECK MCGDSCA
      SUBROUTINE MCGDSCA(M,NSEG,NSUB,LPS,IS,JS,H,KANGL,NOM,NZON,TR,W,
...
*           SCR for a non-cyclic tracking
               ...
               ENDDO
               CALL XABORT('MCGDSCA: UNABLE TO SET IC.')
 10            HJD=H(J)
               TRJ=TR(NZON(NOMJ))
               TAU=HJD*TRJ
...
*        this cell is a volume
            IF(LNEW) THEN
               ISUB=ISUB+1
               IF(ISUB.GT.NSUB) CALL XABORT('MCGDSCA: NSUB OVERFLOW.')
               LNEW=.FALSE.
               IANG=KANGL(ISUB)
               IF(IANG.GT.NANGL) CALL XABORT('MCGDSCA: NANGL OVERFLOW.')
            ENDIF
            TRI=TR(NZI)
            HID=H(I)
            TAUD=HID*TRI
