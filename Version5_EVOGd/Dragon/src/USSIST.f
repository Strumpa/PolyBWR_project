...
*  LOCAL VARIABLES
*----
      TYPE(C_PTR) IPSYS,JPLIB,KPLIB,KPSYS,JPLI0,JPLI01,IPMACR,IPSOU
...
*----
      ...
      REAL, ALLOCATABLE, DIMENSION(:) :: SIGTXS,SIGS0X,SIGG,FLNEW,WCOR,
     1 FUN,SUN
...
*----
      ...
      IPSYS=LCMLID(IPLI0,'ASSEMB-RIBON',NASM)
      CALL LCMSIX(IPLI0,' ',2)
*----
...
*----
         IF(NOR(JRES,IGRP).EQ.1) THEN
            XFLUX(:NBNRS,:MAXNOR,JRES)=1.0
         ELSE
            WRITE(CBDPNM,'(3HCOR,I4.4,1H/,I4.4)') JRES,NIRES
            CALL LCMSIX(IPLI0,CBDPNM,1)
            JPLI01=LCMGID(IPLI0,'NWT0-PT')
            CALL LCMLEL(JPLI01,IGRP,ILON,ITYLCM)
     ...
     1         TEXT12
               CALL XABORT(HSMG)
            ENDIF
            CALL LCMGDL(JPLI01,IGRP,XFLUX(1,1,JRES))
            CALL LCMSIX(IPLI0,' ',2)
         ENDIF
*----
     ...
     1        (IPPT2(IRES,3).EQ.IPPT2(JRES,3))) THEN
            ...
            ELSE
              ...
              IF((ICORR.EQ.1).OR.(NOR(JRES,IGRP).EQ.1)) THEN
                WWW=WEIGH(INOR,IRES)*WEIGH(I2,JRES)*XFLUX(IND,I2,JRES)
              ELSE
                WWW=WCOR((I2-1)*NORI+INOR)*XFLUX(IND,I2,JRES)
              ENDIF
...
*----
     ...
     1      TEXT12
            CALL XABORT(HSMG)
         ENDIF
         CALL LCMGDL(JPLI01,IGRP,XFLUX(1,1,IRES))
         CALL LCMSIX(IPLI0,' ',2)
*----
*  ITERATIVE PROCEDURE.
*----
  ...
  150   CONTINUE
        IF(IND.GT.0) THEN
           ...
           IF(JNOR.NE.INOR) THEN
              SIGG(IBM)=SIGG(IBM)+WEIGH(JNOR,IRES)*CONR(IND,IRES)*
     1        SIGWS(JNOR,IRES)*XFLUX(IND,JNOR,IRES)
           ENDIF
  160      CONTINUE
        ENDIF
  190   CONTINUE
...
*----
  240   IF(IMPX.GT.0) THEN
     ...
     2  NIRES,IRES,NBNRS,NOR,CONR,IPPT1,IPPT2,STGAR,SSGAR,VOLMER,
     3  XFLUX(1,1,IRES),UNGAR)
...
*----
      DEALLOCATE(WCOR,FLNEW,SIGWS,TOTPT,WEIGH,SIGG,SIGS0X,SIGTXS,XFLUX)
      DEALLOCATE(NPSYS)
      RETURN
      END
