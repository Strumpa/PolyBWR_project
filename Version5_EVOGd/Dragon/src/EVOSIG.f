...
*
*Author(s): A. Hebert
*          R. Guasch (06/10/2025) : - implementation of OpenMC "fission-q" normalisation mode (IGLOB=-2)
*                                   - fix to Serpent2 edepomode 0 option (IGLOB=-1)         
*                              
*
*Parameters: input
...
*         =-2: using the OpenMC "fission-q" (default) method;
*         =-1: using the (default) Serpent mode 0 empirical formula in the fuel;
...
*----
     ...
     2 SIG(NVAR+1,NREAC+1,NBMIX),FLUMIX(NGROUP,NBMIX),Q235,CONTRI
*----
*  LOCAL VARIABLES
*----
      ...
      DOUBLE PRECISION GAR,GAR1,GAR2,GAR3,GARD,XDRCST,EVJ,FITD,PHI,
     1 FNORM,VPH
      INTEGER IPRLOC
      LOGICAL LKERMA, U235F
      REAL, ALLOCATABLE, DIMENSION(:) :: ZKERMA,ZNFTOT,NFTOTI
...
*----
      ...
      Q235=0.0
      U235F = .FALSE.
      PRINT *, 'EVOSIG: IGLOB=', IGLOB
      PRINT *, 'NCOMB = ',NCOMB, 'NBMIX=', NBMIX
      IF(IGLOB.EQ.-1) THEN
        DO 30 IST=1,NVAR+NSUPS
          IF(IZAE(IST).EQ.922350) THEN
            PRINT *, 'IZAE(IST)=',IZAE(IST)
            PRINT *, 'RER(1,IST)=',RER(1,IST)
            PRINT *, 'RER(2,IST)=',RER(2,IST)
          ENDIF
...
*----
   ...
   70 CONTINUE
! Loop over mixtures to compute energy released.
      DO 220 IBM=1,NBMIX
      IF(VX(IBM).EQ.0) GO TO 220
      IF(MIXPWR(IBM).EQ.0) GO TO 220
...
! Check here for contrubtions of RRD(IST) and RER(1,IST)
...
*----
  ...
  100    CONTINUE
        ELSE IF(IGLOB.EQ.-1) THEN
          ...
          ENDIF
          IF(K.EQ.0) GO TO 118
!          IF((K.EQ.0).AND.(.NOT.U235F)) CALL XABORT('EVOSIG: NO U235 INFO(2).')
          KPLIB5=IPISO(K)
     ...
     >      12HHE MICROLIB.)') (ISONAM(I0,K),I0=1,3)
          ...
          GAR=0.0D0
          GAR3=0.0D0
          ! INTEGRATE H-FACTORS FOR HM ISO IS and RECOVER NFTOT
          IF(IZAE(IST).GE.900000) THEN
           ALLOCATE(NFTOTI(NGROUP))
           CALL LCMGET(KPLIB,'NFTOT',NFTOTI)
           DO 105 IU=1,NGROUP
            GAR=GAR+1.0E-6*DBLE(ZKERMA(IU)*FLUMIX(IU,IBM)) ! convert to MeV
            GAR3=GAR3+DBLE(NFTOTI(IU)*FLUMIX(IU,IBM))
  105      CONTINUE
          ENDIF
          ! RECOVER U235 TOTAL FISSION XS AND H-FACTORS TO COMPUTE "Q_FISS_U235"
  ...
  110     CONTINUE
          IF(IZAE(IST).GE.900000) THEN
            PRINT *, 'FOR ISO with IZAE =', IZAE(IS)
            PRINT *, 'GAR (summed H-fact)', GAR, 'MeV'
            PRINT *, 'GAR1 (summed U235 H-fact) =', GAR1, 'MeV'
            PRINT *, 'GAR2 (summed U235 sigfiss) =', GAR2, 'barns'
            PRINT *, 'RATIO Hi/H235 =', GAR/GAR1 
            PRINT *, 'GAR3 (summed NFTOT) =', GAR3, 'barns'
            PRINT *, 'RATIO NFTOT/NFTOT235 =', GAR3/GAR2
            GAR=202.27D0*GAR*GAR2/GAR1
            PRINT *, 'TOTAL CONTRIBUTION =', GAR 
          ENDIF
          DEALLOCATE(ZNFTOT)
          IF(IZAE(IST).GE.900000) DEALLOCATE(NFTOTI)
        ELSE IF(IGLOB.EQ.-2) THEN
         ! use openmc q-fission normalisation, applied to H-FACTOR formalism
         PRINT *, 'EVOSIG: Using IGLOB=-2: QFIS normalisation'
         IF(IZAE(IST).GE.900000) THEN
           ! RECOVER H-FACTORS FOR ISO IS
            GAR=0.0D0
             DO 115 IU=1,NGROUP
             GAR=GAR+1.0E-6*DBLE(ZKERMA(IU)*FLUMIX(IU,IBM)) ! convert to MeV
  115        CONTINUE
  ...
  118     DEALLOCATE(ZKERMA)
      ELSE
! Implement Serpent2 edepmode 0 and openmc "fission-q" normalizations for the case without kermas.
        IF((IGLOB.EQ.-2).AND.(IZAE(IST).GE.900000)) THEN
        PRINT *, 'EVOSIG: QFIS selected (OpenMC-like normalization)'
        PRINT *, 'NO KERMA/H-FACTORS FOR IZAE=',IZAE(IS)
        PRINT *, 'ATTEMPT NORMALISATION WITH Q-values' 
        ELSE IF((IGLOB.EQ.-1).AND.(IZAE(IST).GE.900000)) THEN
        PRINT *, 'EVOSIG: EDEP0 selected (Serpent2-like normalization)'
        PRINT *, 'NO KERMA/H-FACTORS FOR IZAE=',IZAE(IS)
        PRINT *, 'ATTEMPT NORMALISATION WITH Q-values' 
...
! SERPENT2 EDEPMODE 0 with fission Q-values
      IF((IGLOB.EQ.-1).AND.(.NOT.LKERMA)) THEN
      IF(Q235.EQ.0.0) THEN 
        PRINT *, 'EVOSIG: EDP0 selected but no Q235 found'
      ENDIF
      IF((IREAC.EQ.2).AND.(Q235.NE.0.0).AND.(IZAE(IST).GE.900000)) THEN
        SIG(IS,NREAC,IBM)=SIG(IS,NREAC,IBM)+1.0E-3*FACT*RER(IREAC,IST)*
     1  REAL(GAR)*DELTAT(IXSPER)*202.27D0/Q235
      ENDIF
      ELSE IF((IGLOB.EQ.-2).AND.(.NOT.LKERMA)) THEN 
      IF((IREAC.EQ.2).AND.(IZAE(IST).GE.900000)) THEN
        SIG(IS,NREAC,IBM)=SIG(IS,NREAC,IBM)+1.0E-3*FACT*RER(IREAC,IST)*
     1  REAL(GAR)*DELTAT(IXSPER)
        ENDIF
      ELSE IF(IGLOB.GT.-1) THEN ! --> Check this here too <--
        ! Lumped energy is not included with EDEPMODE=0.
        SIG(IS,NREAC,IBM)=SIG(IS,NREAC,IBM)+1.0E-3*FACT*RER(IREAC,IST)*
     1  REAL(GAR)*DELTAT(IXSPER)
      ELSE IF(.NOT.LKERMA) THEN
        CALL XABORT('EVOSIG: INVALID RATES NORMALISATION SELECTED.')
      ENDIF
  140 CONTINUE
  150 CONTINUE
  210 CONTINUE
  220 CONTINUE
      ...
      IF((.NOT.U235F).AND.(IGLOB.EQ.-1).AND.(LKERMA)) THEN
         CALL XABORT('EVOSIG: NO U235 INFO(2) IN BURNABLE MIXES.')
      ENDIF
*----
...
*           FITD IS THE DECAY POWER IN WATT PER GRAM.
     ...
     1         E11.4,12H FIT(INPUT)=,E11.4,1H.)') FITD,FIT
               CALL XABORT(HSMG)
            ENDIF
!            PRINT *, 'FITD=', FITD, ' FIT=', FIT
!            PRINT *, 'FUELDN(1)=', FUELDN(1), ' VTOT=', VTOT
!            PRINT *, 'EVJ=', EVJ, ' GAR=', GAR
