...
class Serpent2_case:
    def __init__(self, case_name, lib_name, edep_id, areQfissSet, isEcaptSet, pcc_id, specific_power, tracked_nuclides, save_dir, path=None):
        ...
        self.read_S2_outputs(path)
        self.rescale_BU("MWd/tU")
        

    
    def read_S2_outputs(self, path):
        if path is None:
            ...
            self.res = st.read(f"{path}_res.m")
            self.depl = st.read(f"{path}_dep.m")
            self.keff = self.res.resdata["absKeff"].T[0] # Retrieve the keffs
            self.sigmas_keff = self.res.resdata["absKeff"].T[1] # Retrieve the uncertainties (standard deviation) on the keffs
            self.mat = self.depl.materials['total'] # Retrieve the total, homogenized material
            self.BU = self.mat.burnup # Burnup in MWd/kgU
...
class DRAGON_case:
    def __init__(self, pyCOMPO, dlib_name, bu_points, ssh_opt, correlation, sat, depl_sol, tracked_nuclides, BU_lists, save_dir):
        ...
        self.EVO_BU_steps = BU_lists["BUList"]
        self.AUTOP_BU_steps = BU_lists["USSList"] # only relevant for PCC0 and PCC2 schemes as ssh is performed at all preditor-corrector steps in PCC1, PCC3 and PCC3b 
        self.COMPO_BU_steps = BU_lists["COMPOList"]
