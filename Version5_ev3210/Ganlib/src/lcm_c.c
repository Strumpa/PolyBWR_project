   ...
   if (idir == 1) {
      ...
      if (itylcm == 1) {
         ...
         if( file != NULL && imode == 1) {
            ...
            nb_ligne = (int)jlong/4;
            for (i = 0; i < nb_ligne; i++) {
               if(fgets(line, 80, file) == NULL) goto L30;
               for (j = 0; j < 80; j++) {
                  if(line[j] == 'D') line[j] = 'E';
               }
               for (j = 0; j < 4; j++) {
                 if(sscanf(line, "%lE", &ddd[i*4+j]) == EOF) goto L20;
               }
               if(sscanf(line, "\n") == EOF) goto L20;
            }
            if(fgets(line, 80, file) == NULL) goto L30;
            ...
            reste = jlong%4;
            for (j = 0; j < reste; j++) {
              if(sscanf(line, "%lE", &ddd[nb_ligne*4+j]) == EOF) goto L20;
            }
            if(reste != 0) {
              if( sscanf(line, "\n") == EOF) goto L20;
            }
...
L20:
  ...
  xabort_c(AbortString);
L30:
  sprintf(AbortString,"%s: fgets failure", nomsub);
  xabort_c(AbortString);
}
