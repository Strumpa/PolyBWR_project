...
\hskip 1.0cm $|$ \\
\hskip 1.0cm \moc{COMB} $[[$ \dusa{mati} \dusa{relvol} $]]~|$ \moc{COPY} \dusa{numold} $\}$
\end{DataStructure}
...
\item[\moc{COPY}]  keyword to specify that mixture \dusa{matnum} in \dusa{MICLIB} is a copy of mixture \dusa{numold} in a
{\sc microlib}-formatted object (type \moc{MICROLIB}) named \dusa{NAMEFIL}, previously produced by DRAGON. This option is
useful to permute mixture indices before the flux calculation.

\item[\dusa{numold}] mixture index existing in \moc{MICROLIB} object named \dusa{NAMEFIL}.
