...
def createLib(mix_numbering_option, draglib_name, anisotropy_level, self_shielding_method, resonance_correlation, transport_correction, composition_option, rates_norm_opt, 
    ...
    if mix_numbering_option == "number_mix_families_per_enrichment":
        ...
        fill_lib_proc(proc_name, composition_option, mix_connectivity, resonance_correlation, rates_norm_opt)
...
def fill_lib_call(composition_option, mix_connectivity_dict, correlation_option, rates_norm_opt):
    """
    ...
    if rates_norm_opt == "EDEP0":
        # Modify the DEPL structure to account for Serpent2's set edepmode 0 with corrected fission Q values.
...
def fill_lib_proc(file_name, composition_option, mix_connectivity_dict, correlation_option, rates_norm_opt):
    """
    ...
    rates_norm_opt : str
        Option to impose modifiaction to the DEPL structure at library creation to account for Serpent2's set edepmode 0 with corrected fission Q values.
    """
    header = (  "* --------------------------------\n"
    ...
    lib_proc = (
        ...
        "*    CALL TO LIB: MODULE\n"
        "* --------------------------------\n"
        f"{fill_lib_call(composition_option, mix_connectivity_dict, correlation_option, rates_norm_opt)}\n"
        "* -----------------------------------------\n"
