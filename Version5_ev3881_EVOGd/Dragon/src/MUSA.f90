...
!-----------------------------------------------------------------------
!
SUBROUTINE MUSA(IPSYS,IPTRK,IFTRAK,IMPX,NREG,NBMIX,MAT,SIGT0,SIGW0, &
  & NBATCH,TITREC)
  USE GANLIB
  !----
  ...
  INTEGER IFTRAK,IMPX,NREG,NBMIX,MAT(NREG),NBATCH
  ...
  IF(IJAT.NE.NMC_SURF(NMACRO+1)) CALL XABORT('MUSA: INVALID IJAT.')
