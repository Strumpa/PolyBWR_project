...
*----
     ...
     1 FIT,AWR(NVAR),FUELDN(3),DELTAT(2),SIG(NVAR+1,NREAC+1,NBMIX),
     2 PFACT,VPHV(NBMIX),FLUMIX(NGROUP,NBMIX)
*----
*  LOCAL VARIABLES
*----
...
*----
      DO IST=1,NVAR+NSUPS
         PRINT *, 'EVOSIG: Checking isotope IZAE(IST)=', IZAE(IST)
         IF(IZAE(IST).EQ.922350) THEN
            IS235 = IST
            PRINT *, 'EVOSIG: U235 found at IST =', IS235
            Q235 = RER(2,IS235)
            PRINT *, 'EVOSIG: RER(1,IS235) =', RER(1,IS235)
            PRINT *, 'EVOSIG: RER(2,IS235) =', RER(2,IS235)
...
*  COMPUTE MICRO RATES
*----
      PRINT *, 'EVOSIG: STARTING MICRO RATES COMPUTATION.'
      IPRLOC=0
...
*----
      PRINT *, 'EVOSIG: RECOVERING XS FOR IS=',IS
!      CALL LCMLIB(KPLIB)
!      PRINT *, 'RER =', RER
  ...
  130 CONTINUE
     ...
     1 DELTAT(IXSPER)
      IF ((IMPX.GE.10).AND.(AWR(IS).GE.210.0)) THEN
         PRINT *, 'EVOSIG: IS,IREAC-1,IBM=',IS,IREAC-1,IBM
         WRITE(HSMG,'(17HEVOSIG: REACTION ,A6,18H PROCESSED FOR IS
     1     7HOTOPE '',3A4,2H''.)') NAMDXS(IREAC-1),(ISONAM(I0,K),I0=1,3)
         WRITE(IOUT,'(1X,A)') HSMG
...
* Only include fission energy in normalisation
     ...
     1 REAL(GAR)*DELTAT(IXSPER)*H_235/Q235
      ...
      IF ((IMPX.GE.10).AND.(AWR(IS).GE.210.0)) THEN
         ...
         PRINT *, 'EVOSIG: SIG(IS,NREAC,IBM)=',SIG(IS,NREAC,IBM)
      ENDIF
  140 CONTINUE
  150 CONTINUE
  210 CONTINUE
  ...
  230 CONTINUE
      ...
      ELSE IF(INR.GE.2) THEN
         ...
         IF(MIXPWR(IBM).EQ.1) THEN
            DO 240 IS=1,NVAR
* Exclude non-HM isotopes from the normalization
*            IF((IGLOB.EQ.0).AND.(AWR(IS).LE.210.0)) GO TO 240
            K=JM(IBM,IS)
            IF(K.GT.0) THEN
