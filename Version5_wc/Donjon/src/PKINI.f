...
*----
      ...
      TYPE(C_PTR) IPMAP,JPMAP,KPMAP,JPPAR,KPPAR,IPMAC
...
*  RECOVER THE FUELMAP
*----
      IF((NENTRY.NE.1).AND.(NENTRY.NE.2)) THEN
         CALL XABORT('PKINI: ONE OR TWO PARAMETER EXPECTED.')
      ENDIF
      IPMAC=C_NULL_PTR
      DO I=1,NENTRY
         IF((IENTRY(I).NE.1).AND.(IENTRY(I).NE.2))CALL XABORT('PKINI: '
     1  //'LCM OBJECT EXPECTED.')
        TEXT12=HENTRY(I)
        IF(I.EQ.1) THEN
          IPMAP=KENTRY(1)
          IF(JENTRY(1).NE.1) CALL XABORT('PKINI: FIRST ENTRY IN MODIFC'
     1    //'CATION MODE EXPECTED.')
     ...
     1      '. L_MAP EXPECTED.')
        ...
        ELSE IF(I.EQ.2) THEN
          IPMAC=KENTRY(2)
          IF(JENTRY(2).NE.2) CALL XABORT('PKINI: SECOND ENTRY IN READ-'
     1    //'ONLY MODE EXPECTED.')
...
*----
      ...
      CALL LCMSIX(IPMAP,'P-KINETICS',1)
*----
*  MACROLIB OBJECT
*----
      IF(C_ASSOCIATED(IPMAC)) THEN
        ...
        NGROUP=ISTATE(7)
        IF(NGROUP.EQ.0) THEN
          CALL XABORT('PKINI: NO DELAYED NEUTRONS IN INPUT MACROLIB.')
        ELSE IF(ISTATE(18).EQ.0) THEN
          CALL XABORT('PKINI: NO P-KINETICS RECORD IN INPUT MACROLIB.')
        ENDIF
        CALL LCMSIX(IPMAC,'P-KINETICS',1)
        CALL LCMEQU(IPMAC,IPMAP)
        CALL LCMSIX(IPMAC,' ',2)
      ENDIF
*----
*  READ INPUT DATA
...
*       Prompt neutron generation time (s)
        IF(C_ASSOCIATED(IPMAC)) THEN
          CALL XABORT('PKINI: LAMBDA IS DEFINED IN INPUT MACROLIB.')
        ENDIF
...
*       Set initial time and stage length (s)
        ...
        CALL LCMPUT(IPMAP,'T-VALUE-INIT',1,2,T)
        IF(IMPX.GT.0) WRITE(6,100) T,DT
      ELSE IF(TEXT12.EQ.'NGROUP') THEN
*       Read number of delayed precursor groups.
        IF(C_ASSOCIATED(IPMAC)) THEN
          CALL XABORT('PKINI: NGROUP IS DEFINED IN INPUT MACROLIB.')
        ENDIF
...
*       Delayed neutron fraction
        IF(NGROUP.EQ.0) THEN
          CALL XABORT('PKINI: NGROUP NOT DEFINED.')
        ELSE IF(C_ASSOCIATED(IPMAC)) THEN
          CALL XABORT('PKINI: BETAI IS DEFINED IN INPUT MACROLIB.')
        ENDIF
...
*       Delayed neutron time constant
        IF(NGROUP.EQ.0) THEN
          CALL XABORT('PKINI: NGROUP NOT DEFINED.')
        ELSE IF(C_ASSOCIATED(IPMAC)) THEN
          CALL XABORT('PKINI: LAMDAI IS DEFINED IN INPUT MACROLIB.')
        ENDIF
