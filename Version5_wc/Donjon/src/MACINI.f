...
*     MACROLIB(2)-INFO
      ...
      IF(NENTRY.EQ.4) THEN
        ...
        IF((NF2.NE.NF1).AND.(NF1.GT.1)) THEN
          WRITE(IOUT,*)'MACROLIB=',HENTRY(1),' NF=',NF1,' (1 EXPECTED)'
          WRITE(IOUT,*)'MACROLIB=',HENTRY(2),' NF=',NF2
          CALL XABORT('@MACINI: INCONSISTENT NUMBER OF FISSILE ISOTOPE'
     1    //'S.')
        ELSE IF((NDEL2.NE.NDEL1).AND.(NDEL1*NDEL2.GT.0)) THEN
          WRITE(IOUT,*)'MACROLIB=',HENTRY(1),' NDEL=',NDEL1,
     1    ' (0 EXPECTED)'
          WRITE(IOUT,*)'MACROLIB=',HENTRY(2),' NDEL=',NDEL2
          CALL XABORT('@MACINI: INCONSISTENT NUMBER OF DELAYED PRECURS'
     1    //'OR GROUPS.')
        ENDIF
...
*  RECOVER MIX INDICES
*----
   20 ALLOCATE(MIX(NTOT))
      MIX(:NTOT)=0
      IF((.NOT.LMAP).AND.(NENTRY.EQ.3)) THEN
        CALL LCMLEN(IPMTX,'MAT-INIT',LENGTH,ITYLCM)
        IF(LENGTH.EQ.0) CALL XABORT('@MACINI: MISSING MAT-INIT.')
        CALL LCMGET(IPMTX,'MAT-INIT',MIX)
        IF(IMPX.GT.0) THEN
          WRITE(IOUT,'(37H MACINI: USE MACROLIB CROSS SECTIONS.)')
        ENDIF
      ELSE
...
*----
      IF(IMPX.GT.1)WRITE(IOUT,*)'NUMBER OF ENERGY GROUPS  ',NGRP
...
*     DO NOT INCLUDE FUEL PROPERTIES
      ...
      CALL MACCRE(IPMAC1,IPMAC,NL,NW,NF1,NGRP,NMIX1,NMIX,NTOT,MIX,LMAP,
     1 IMPX)
...
*     INCLUDE FUEL PROPERTIES
      IF(NENTRY.EQ.4) THEN
        ...
        CALL MACCRE(IPMAC2,IPMAC,NL,NW,NF2,NGRP,NMIX2,NMIX,NTOT,MIX,
     1  LMAP,IMPX)
      ENDIF
      DEALLOCATE(MIX)
*----
...
*----
      ...
      LWD1=(LENGTH.EQ.NDEL1*NF1).AND.(NDEL1.GT.0)
      LWD2=.FALSE.
      IF(NENTRY.EQ.4) THEN
        CALL LCMLEN(IPMAC2,'LAMBDA-D',LENGTH,ITYLCM)
        LWD2=(LENGTH.EQ.NDEL2*NF2).AND.(NDEL2.GT.0)
      ENDIF
      NDEL=0
      IF(LWD1) THEN
        NDEL=NDEL1
        ALLOCATE(WDLA(NDEL*NF1))
        CALL LCMGET(IPMAC1,'LAMBDA-D',WDLA)
        CALL LCMPUT(IPMAC,'LAMBDA-D',NDEL*NF1,2,WDLA)
        DEALLOCATE(WDLA)
      ELSE IF(LWD2) THEN
        NDEL=NDEL2
        ALLOCATE(WDLA(NDEL*NF2))
        CALL LCMGET(IPMAC2,'LAMBDA-D',WDLA)
        CALL LCMPUT(IPMAC,'LAMBDA-D',NDEL*NF2,2,WDLA)
        DEALLOCATE(WDLA)
      ENDIF
*----
*  STATE-VECTOR
