...
*       L_FLUX object
        CALL LCMLEN(IPFLX,'FLUX',LENGT,ITYLCM)
        IF(LENGT.GT.0) THEN
          MPFLX=LCMGID(IPFLX,'FLUX')
        ELSE
          CALL LCMLEN(IPFLX,'MODE',LENGT,ITYLCM)
          IF(LENGT.GT.0) THEN
            JPFLX=LCMGID(IPFLX,'MODE')
            KPFLX=LCMGIL(JPFLX,1)
            MPFLX=LCMGID(KPFLX,'FLUX')
          ELSE
            CALL LCMLIB(IPFLX)
            CALL XABORT('FLPNRM: UNABLE TO RECOVER A DIRECT FLUX.')
          ENDIF
        ENDIF
        DO 10 JGR=1,NGRP
        CALL LCMGDL(MPFLX,JGR,EVECT(1,JGR))
   10   CONTINUE
...
*----
      ...
      IBM=MAT(IEL)
      IF(IBM.EQ.0)GOTO 20
      IF(IBM.GT.NMIX) CALL XABORT('FLPNRM: NMIX OVERFLOW.')
      FLUX(IEL,JGR)=EVECT(IDL(IEL),JGR)
      ZNRM=ZNRM+HFAC(IBM,JGR)*FLUX(IEL,JGR)*VOL(IEL)*EVJ
   20 CONTINUE
   25 CONTINUE
      IF(ZNRM.EQ.0.0) CALL XABORT('FLPNRM: H-FACTOR ALL ZERO.')
      ZNRM=PTOT/ZNRM
