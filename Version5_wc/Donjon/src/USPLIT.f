...
*----
      ...
      INTEGER, ALLOCATABLE, DIMENSION(:) :: ISPLX,ISPLY,ISPLZ,MAT,KSP,
     1 INDX,IRMIX,IFMIX,MIXA
...
*----
   20 IMPX0=MAX(0,IMPX-1)
      ALLOCATE(ISPLX(MAXR),ISPLY(MAXR),ISPLZ(MAXR),MAT(MAXR),KSP(MAXR))
     ...
     2 ISPLL)
      CALL USPD3D(MAXR,MAXR,MAXR,MAXR,IPGEO,IMPX0,LX,LY,LZ,KSP)
      CALL LCMPUT(IPMTX,'MATEX',NEL,1,KSP)
*----
...
*     MODIFY GEOMETRY
      ...
      DEALLOCATE(IFMIX,IRMIX,INDX,KSP,MAT)
