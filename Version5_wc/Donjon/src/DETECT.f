...
*----
      ...
      REAL    FLOT,DT,VNORM
      TYPE(C_PTR) IPFLUX,JPFLUX,KPFLUX,MPFLUX,IPTRK,IPGEO,IPDET
     ...
     +  //'GROUPS INCOMPATIBLE BETWEEN FLUX AND DETECT')
       ...
       ELSE
         ...
         IF(LENGT.GT.0) THEN
           ...
           KPFLUX=LCMGIL(JPFLUX,1)
           MPFLUX=LCMGID(KPFLUX,'FLUX')
         ELSE
           CALL LCMLIB(IPFLUX)
           CALL XABORT('DETECT: UNABLE TO RECOVER A DIRECT FLUX.')
         ENDIF
       ENDIF
       CALL LCMLEL(MPFLUX,1,ILONG,ITYLCM)
       ALLOCATE(FUNKN(ILONG))
       DO 30 IGR=1,NGRP
        CALL LCMGDL(MPFLUX,IGR,FUNKN)
        DO 25 IUN=1,NUN
