...
*----
*  SET THE MAXIMUM SIGS (NL) AND NTOT (NW) ORDERS
*----
      NL=1
      NW=1 ! activate NTOT1 data
      IF(IACCS.EQ.0) THEN
   ...
   10   CONTINUE
      ELSE
        ...
        NW=ISTATE(10)
      ENDIF
      IF(IMPX.GT.0) WRITE(6,'(16H NCRMAC: SET NL=,I3,8H AND NW=,I3,
     1 1H.)') NL,NW
*----
...
*----
     ...
     1 GAR3(NMIX,NGRP,NGRP,NL),GAR4(NMIX*NGRP))
...
*----
      IF(IACCS.EQ.0) THEN
         MPCPO=LCMGIL(LPCPO,1)
         CALL LCMGET(MPCPO,'STATE-VECTOR',ISTATE)
         IF(ISTATE(4).GT.NL) CALL XABORT('NCRMAC: NL OVERFLOW.')
         IF(ISTATE(25).GT.NW) CALL XABORT('NCRMAC: NW OVERFLOW.')
         NBISO=ISTATE(2)
         NF=0
         ITRAN=ISTATE(5)
         NED=ISTATE(13)
         ...
         IF(NBISO.GT.0) THEN
           ...
           DO ISO=1,NBISO
             ...
             IF((ILONG.EQ.NDEL).AND.(NDEL.GT.0)) THEN
               ...
               DEALLOCATE(WDLA)
               IF(HNAMIS(ISO).EQ.'U235') GO TO 20
               IF(HNAMIS(ISO).EQ.'*MAC*RES') GO TO 20
             ENDIF
           ENDDO
   20      DEALLOCATE(HNAMIS)
         ENDIF
     ...
     1    //'XPECTED.')
         ...
         ELSE IF(ISTATE(3).NE.NL) THEN
            CALL XABORT('NCRMAC: INVALID NUMBER OF LEGENDRE ORDERS(1).')
         ELSE IF(ISTATE(10).NE.NW) THEN
            CALL XABORT('NCRMAC: INVALID NUMBER OF SIGT ORDERS.')
         ENDIF
         NF=ISTATE(4)
         NED=ISTATE(5)
         NDEL=ISTATE(7)
...
*----
   ...
   25 CONTINUE
      ...
      HMAK1(9+2*NW:8+2*NW+IED)=HVECT(:NED)
      DO 30 IL=1,NL
      WRITE(CM,'(I2.2)') IL-1
...
*----
  165 CALL LCMSIX(IPTMP,'MACROLIB',1)
      ...
      ELSE IF(ISTATE(3).GT.NL) THEN
         CALL XABORT('NCRMAC: INVALID NUMBER OF LEGENDRE ORDERS(2).')
      ELSE IF((ISTATE(4).NE.0).AND.(ISTATE(4).NE.NF)) THEN
         CALL XABORT('NCRMAC: INVALID NUMBER OF FISSILE ISOTOPES.')
      ELSE IF((ISTATE(7).NE.NDEL).AND.(ISTATE(7).GT.0)) THEN
         CALL XABORT('NCRMAC: INVALID NUMBER OF PRECURSOR GROUPS.')
      ELSE IF(ISTATE(10).GT.NW) THEN
         CALL XABORT('NCRMAC: INVALID NUMBER OF NTOT ORDERS.')
      ENDIF
      JPTMP=LCMGID(IPTMP,'GROUP')
      DO 195 IGR=1,NGRP
      KPTMP=LCMGIL(JPTMP,IGR)
