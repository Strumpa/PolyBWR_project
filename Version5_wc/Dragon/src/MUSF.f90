...
!
  ...
  INTEGER, ALLOCATABLE, DIMENSION(:) :: NMC_FLUX,NMC_SURF,IFR,MIX,INUM,IGEN,IMAC
  ...
  ALLOCATE(NMC_FLUX(NMACRO+1),NMC_SURF(NMACRO+1),IMAC(NREG))
  ...
  DO II=1,NGEFF
    ...
    & NMIX,NIFR,FUNKNO(1,II),SUNKNO(1,II),IMPX,NMC_FLUX,NMC_SURF,IFR,ALB, &
  ...
  DEALLOCATE(IMAC,NMC_SURF,NMC_FLUX,INUM,IGEN)
  RETURN
  END
