SUBROUTINE DOORS_SN2(IPTRK,NANIS,NREG,NMAT,NUN,MATCOD,SIGG,SUNKNO,FUNKNO)
  !
  !-----------------------------------------------------------------------
  !
  !Purpose:
  ! Compute the source for the solution of SN equations without a flat
  ! flux approximation.
  !
  !Copyright:
  ...
  REAL SIGG(0:NMAT,NANIS+1),SUNKNO(NUN),FUNKNO(NUN)
  !----
  !  LOCAL VARIABLES
  !----
  PARAMETER(NSTATE=40)
  ...
  IF(JPAR(1).NE.NREG) CALL XABORT('DOORS_SN2: INCONSISTENT NREG.')
  IF(JPAR(2).NE.NUN) CALL XABORT('DOORS_SN2: INCONSISTENT NUN.')
  ITYPE=JPAR(6)
  NSCT=JPAR(7)
  IELEM=JPAR(8)
  ISCAT=JPAR(16)
  ...
  DO P=1,NSCT
    ...
    ELSE IF((ITYPE.EQ.7).OR.(ITYPE.EQ.9)) THEN
      ...
      ENDDO ! IR
    ELSE
      CALL XABORT('DOORS_SN2: TYPE OF DISCRETIZATION NOT IMPLEMENTED.')
    ENDIF
  ENDDO ! P
  RETURN
END SUBROUTINE DOORS_SN2
