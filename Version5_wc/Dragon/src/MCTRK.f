*DECK MCTRK
      SUBROUTINE MCTRK(IPTRK,IPRINT,NFREG,NFSUR,NDIM,NBCDA,NMIX,ANGBC,
     1                 ITYPBC,MAXMSH,NUCELL,MXGSUR,MXGREG,MAXPIN,BCRT,
     2                 ICODE,ALBEDO,IUNFLD,DGMESH,XYZL,INDEX,IDREG,
     3                 DCMESH,ITPIN,DRAPIN,ISEED,NGRP,NL,NFM,NDEL,NED,
     4                 MATCOD,XST,XSS,XSN2N,XSN3N,XSSNN,XSNUSI,XSCHI,
     5                 XSEDI,MIX,ISONBR,NU,POS,ITALLY,NBSCO,NMERGE,
     6                 NGCOND,IMERGE,INDGRP,SCORE1,SCORE2)
*
*-----------------------------------------------------------------------
*
*Purpose:
...
*----
      TYPE(C_PTR) IPTRK
      INTEGER IPRINT,NFREG,NFSUR,NDIM,NBCDA,NMIX,ANGBC,ITYPBC,MAXMSH,
     1 NUCELL(3),MXGSUR,MXGREG,MAXPIN,MIX,ISONBR,BCRT(NFSUR),
     2 ICODE(NBCDA),IUNFLD(2,NUCELL(1),NUCELL(2),NUCELL(3)),
     ...
     5 NGCOND,IMERGE(NFREG),INDGRP(NGRP)
      REAL ALBEDO(NBCDA),XST(NMIX,NGRP),XSS(NMIX,NGRP,NL),
     1 XSN2N(NMIX,NGRP),XSN3N(NMIX,NGRP),XSSNN(NMIX,NGRP,NGRP,NL),
...
*----
     ...
     2 ISUR,ISUR2,IBM,IDIM,IFIRST,IS
...
*             SPECULAR REFLECTIVE BOUNDARY CONDITION
              ...
              IS=-ICODE(INDIC(IDSO))
              IF(IS.GT.NBCDA) CALL XABORT('MCTRK: NBCDA OVERFLOW(1).')
              ALB=ALBEDO(IS)
              IF(ALB.EQ.0.0) THEN
*               no leakage
                ISONBR = 0
...
*             WHITE BOUNDARY CONDITION
              IS=-ICODE(INDIC(IDSO))
              IF(IS.GT.NBCDA) CALL XABORT('MCTRK: NBCDA OVERFLOW(2).')
              ALB=ALBEDO(IS)
              IF(ALB.EQ.0.0) THEN
*               no leakage
                ISONBR = 0
