...
*----
      INTERFACE
        SUBROUTINE MOCFFI_TEMPLATE(SUBSCH,NR,NS,NUN,NBCDA,MT,LINE,
     1             SEGLEN,NRSEG,NE,MATALB,SIGANG,KEYFLX,YG,FLUX,EXPT,
     2             EXP2,FLM,FLP,CYM,CYP,IDIR,OMG2)
          INTEGER NR,NS,NUN,NBCDA,MT,LINE,NRSEG(LINE),NE,MATALB(-NS:NR),
     1    KEYFLX(NR),IDIR
          REAL SIGANG(-NBCDA:MT),YG(NE)
     ...
     2     CYP(NE,LINE),OMG2(NE,3)
        ...
        !
        SUBROUTINE MOCFFA_TEMPLATE(SUBSCH,NR,NS,NUN,NBCDA,MT,LINE,
     1             SEGLEN,NRSEG,NE,NF,MATALB,SIGANG,KEYFLX,YG,FLUX,
     2             EXPT,EXP2,FLM,FLP,CYM,CYP,NPHI,NSUB,KANGL,TRHAR)
     ...
     2    CYP(NE,LINE)
        ...
        !
        SUBROUTINE MOCSCH_TEMPLATE(N,NREG,NSOUT,NBCDA,M,NOM,NZON,H,
     1             SIGANG,EXPT,EXP2,NMU,ZMU)
...
*----
      ...
      REAL, ALLOCATABLE, DIMENSION(:) :: REPS,EPS,CPO
...
*----
     ...
     1 CALL XABORT('MCCGF: INVALID TRACKING FILE: NCOR.NE.1')
      IF(NBCDA.NE.NBALB) CALL XABORT('MCCGF: INVALID NUMBER OF ALBEDOS'
...
*     --------------------------------
     ...
     2       NSOU,NBCDA,NGRP,NGEFF,NGIND,NZON,MATALB,V,FUNKNO,SUNKNO,
