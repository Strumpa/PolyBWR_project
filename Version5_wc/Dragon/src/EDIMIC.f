*DECK EDIMIC
     ...
     4 B2,DEN,ITYPE,IEVOL,LSISO,EMEVF,EMEVG,DECAY,YIELD,FIPI,PYIELD,
...
*----
     ...
     5          FIPI(NBISO,NMERGE),ITRANC,NMLEAK
...
*
     ...
     2 ITYPRO(NL),MILVO(NMERGE),ITYPS(NBISO),IMERGL(NBMIX))
      ALLOCATE(MASK(NBISO))
      ALLOCATE(GAR(NGROUP,MAXH),WGAR(NGROUP**2,NL),XSECT(0:NBMIX),
     1 DIFHET(NMLEAK,NGROUP),WSTRD(NGCOND),SDEN(NBISO*NMERGE),
     2 VOLISO(NBISO*NMERGE),TNISO(NBISO*NMERGE),TMPXS(NGCOND),
     3 WDLA(NDEL),WORK(NGROUP))
      ALLOCATE(WSCAT(NGCOND,NGCOND,NL),GAS(NGCOND,MAXH))
      ALLOCATE(HMAKE(MAXH+NL))
      ALLOCATE(JPIFI(NBISO*NMERGE),YPIFI(NBISO*NMERGE))
*----
...
*----
        ...
        IF(ISMIX(IISO).EQ.INM) THEN
          ...
          IF(YIELD(1,ISO,INM).GT.0.0) THEN
            ...
            JSO=ISTYP(JJSO)
            IFI=FIPI(JSO,INM)
            IF(IFI.GT.0) YPIFI(J)=PYIELD(IFI,ISO,INM)
  480       CONTINUE
...
*----
  ...
  510      CONTINUE
           ...
           IF((ISTOD(IISO).NE.1).AND.(ISTYP(IISO).GE.1)) THEN
              INM=ISMIX(IISO)
              IF(INM.EQ.0) GO TO 530
              DO 520 J=1,NCOMB
              IF(INM.EQ.MILVO(J)) GO TO 530
  520         CONTINUE
              NCOMB=NCOMB+1
