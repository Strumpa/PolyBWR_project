*DECK EDIMIC
     ...
     4 EIGENK,EIGINF,B2,DEN,ITYPE,IEVOL,LSISO,EMEVF,DECAY,YIELD,FIPI,
...
*----
     ...
     2          NGCOND,NBMIX,NREGIO,NMERGE,NDFI,NDFP,ILEAKS,ILUPS,
     3          INORM,NW,MATCOD(NREGIO),KEYFLX(NREGIO),IGCOND(NGCOND),
...
*        DIVIDE MATRIX XS BY INTEGRATED FLUX
  ...
  330    CONTINUE
         ...
         IF((QEN.GT.0.0).AND.(IADJ.EQ.0)) THEN
            ...
            ELSE
              IF(ERR.GT.1.0E-3) WRITE(6,625) IGRCND,IL,100.0*ERR,HNEW
...
*
  ...
  620 FORMAT(/53H EDIMIC: *** WARNING *** THE WITHIN-GROUP SCATTERING ,
     1 17HTRANSFER IN GROUP,I5,10H AND ORDER,I3,15H IS IN ERROR BY,F6.2,
     2 10H%. ISOTOPE,2H=',A12,2H'.)
  625 FORMAT(/53H EDIMIC: *** WARNING *** NORMALIZATION OF THE WITHIN-,
     1 34HGROUP SCATTERING TRANSFER IN GROUP,I5,10H AND ORDER,I3,3H BY,
     2 F6.2,10H%. ISOTOPE,2H=',A12,2H'.)
