...
SUBROUTINE SALACG(FGEO ,ITRACK, RCUTOF, IPRINT, GG)
  ...
  INTEGER NALBG,OK,I,J,NREG,ELEM,NFREG,LEAK,NSOUT,INDEX,MMAX,NBCDA,IMERGE
  ...
  IF(LMERGM) THEN
    ...
    DO I=1,MMAX
      ...
      20 CONTINUE
    ENDDO
    IMERGE=MAXVAL(GG%NUM_MERGE(:))
    IF(IPRINT>0) WRITE(6,'(/40h SALACG: MERGMIX reduces GG%NB_FLUX from,i5, &
    & 3h to,i5,1h.)') GG%NB_FLUX,IMERGE
    GG%NB_FLUX=IMERGE
  ENDIF
  !----
  ...
  NBCDA=MAX(6,GG%NALBG)
  ALLOCATE(ICODE(NBCDA),GALBED(NBCDA))
  ICODE(:NBCDA)=(/ (-I,I=1,NBCDA) /)
  GALBED(:NBCDA)=REAL(GG%ALBEDO)
  IF(GG%NB_SURF2 /= 0) THEN
    NALBG=GG%NALBG
    IF(NALBG.EQ.0) NALBG=6
    DO I=1,NSOUT
      KEYMRG(-I)=-I
      VOLSUR(-I)=GG%SURF2(I)
      ...
      ELSE
        IF(INDEX.GT.NBCDA) CALL XABORT('SALACG: INDEX overflow(1).')
