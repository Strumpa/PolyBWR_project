...
*----
      ...
      REAL, ALLOCATABLE, DIMENSION(:) :: SCATC,FGAR
...
*----
      ...
      DO 100 IGR=1,NGROUP
        KPEDIT=LCMGIL(JPEDIT,IGR)
          IF(IGR.EQ.1) CALL LCMLIB(KPEDIT)
        DO 110 IED=1,NEDMAC
          IXSR=IVECT(IED)
          IF(IXSR.GT.0) THEN
            CALL LCMLEN(KPEDIT,HVECT(IED),ILCMLN,ITYLCM)
            IF(ILCMLN.EQ.0) GO TO 110
            CALL LCMGET(KPEDIT,HVECT(IED),SCATC)
            DXSMAC(IGR,IXSR,:NMERGE)=DBLE(SCATC(:NMERGE))
          ENDIF
 110    CONTINUE
        IXSR=NDPROC+NL+1
        ...
        DXSMAC(IGR,IXSR,:NMERGE)=DBLE(SCATC(:NMERGE))
        IF(NIFISS.GT.0) THEN
          ALLOCATE(FGAR(NMERGE*NIFISS))
          IXSR=3
          JXSR=16
          CALL LCMLEN(KPEDIT,'NUSIGF',ILCMLN,ITYLCM)
          IF(ILCMLN.GT.NMERGE*NIFISS) CALL XABORT('CPOMAR: NUSIGF OVER'
     >    //'FLOW.')
          CALL LCMGET(KPEDIT,'NUSIGF',FGAR)
          ILOCED=1
     ...
     >          +DBLE(FGAR(ILOCED))*DXSMAC(IGR,JXSR,IMRG)
              ILOCED=ILOCED+1
 151        CONTINUE
 150      CONTINUE
          IXSR=4
          CALL LCMGET(KPEDIT,'NFTOT',FGAR)
...
*----
      IF(NIFISS.GT.0) THEN
        ...
        DO 160 IGR=1,NGROUP
          KPEDIT=LCMGIL(JPEDIT,IGR)
          IXSR=5
          CALL LCMLEN(KPEDIT,'CHI',ILCMLN,ITYLCM)
          IF(ILCMLN.GT.NMERGE*NIFISS) CALL XABORT('CPOMAR: CHI OVERFLO'
     >    //'W.')
          CALL LCMGET(KPEDIT,'CHI',FGAR)
          ILOCED=1
          DO 161 IFIS=1,NIFISS
            DO 162 IMRG=1,NMERGE
              DXSMAC(IGR,IXSR,IMRG)=DXSMAC(IGR,IXSR,IMRG)
     >          +DBLE(FGAR(ILOCED))*DNUFI(IMRG,IFIS)
              ILOCED=ILOCED+1
 162        CONTINUE
 161      CONTINUE
 160    CONTINUE
