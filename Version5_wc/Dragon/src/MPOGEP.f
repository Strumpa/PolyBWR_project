*DECK MPOGEP
      SUBROUTINE MPOGEP(IPMPO,IPDEPL,IPLB1,IPLB2,IPMICR,IPEDIT,HEDIT,
     1 IMPX,ITIM,NPAR,NLOC,MUPLET,LGNEW,NMIL,NG,NCALAR)
*
*-----------------------------------------------------------------------
*
*Purpose:
...
* IPMICR  pointer to the microlib to include.
* IPEDIT  pointer to the edition object to include.
...
*----
      TYPE(C_PTR) IPMPO,IPDEPL,IPLB1,IPLB2,IPEDIT,IPMICR
...
*        RECOVER GLOBAL PARAMETER VALUES FROM THE DEPLETION OBJECT.
     ...
     1   NREAC,NVAR,VALPAR)
      ELSE IF((PARTYP(IPAR).EQ.'TEMP').OR.(PARTYP(IPAR).EQ.'CONC')) THEN
*
...
*        RECOVER LOCAL VARIABLES FROM THE DEPLETION OBJECT.
         IF(.NOT.C_ASSOCIATED(IPDEPL)) THEN
           CALL XABORT('MPOGEP: NO DEPLETION OBJECT DEFINED.')
         ELSE IF(.NOT.C_ASSOCIATED(IPEDIT)) THEN
           CALL XABORT('MPOGEP: NO EDITION OBJECT DEFINED.')
         ENDIF
         CALL LCMGET(IPDEPL,'STATE-VECTOR',ISTATE)
         NBURN=ISTATE(3)
         NBISO=ISTATE(4)
...
*        RECOVER LOCAL VARIABLES FROM THE MICROLIB IN EDIT OBJECT.
         ...
         MAXNBI=ISTATE(2)
         CALL LCMINF(IPMICR,NAMLCM,NAMMY,EMPTY,ILONG,LCM)
         IPLB3=C_NULL_PTR
         DO 60 IBM=1,NMIL
         CALL COMBIB(IPMICR,IPLB3,PARTYL(IPAR),IBM,NAMLCM,TEXT8,MAXNBI,
     1   VALPAR)
         RVALO((IBM-1)*NVLC+LOCADR(IPAR))=VALPAR
   60    CONTINUE
      ELSE IF(PARTYL(IPAR).EQ.'EQUI') THEN
*        RECOVER A SET OF SPH EQUIVALENCE FACTORS.
         CALL SAPSPH(IPMICR,NG,NMIL,LOCADR(IPAR),NVLC,RVALO)
      ELSE
