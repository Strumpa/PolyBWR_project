...
SUBROUTINE MUSACG(ITRACK,IFTRK,IPRINT,IMACRO,NBSLIN,RCUTOF,GG,LGINF,NBNODE_MACRO,NSURF_MACRO)
  ...
  USE SAL_GEOMETRY_MOD,  ONLY : SAL130_2,SAL130_4,SAL130_6,SAL130_8,SAL130_10,SAL131_2, &
                                & SAL140,SAL160_2,SAL170,SALFOLD_0,FINDLC,DET_ROSETTA
  ...
  REAL(PDB) :: DGMESHX(2),DGMESHY(2),AXIS_XY(4)
  REAL :: GALBED(NALBG)
  ...
  !----
  IF(IPRINT.GT.0) WRITE(FOUT,*) '************* IMACRO=',IMACRO,' *************'
  ...
  ENDDO ITER0
  !----
  !  Find if the perimeter is an unfolding axis
  !----
  DO J=1,NPERIM
    LFOLD(J)=.FALSE.
    DO I=1,NELEM_MACRO
      ELEM = ELEM_MACRO(I)
      IF(GG%IPAR(1,ELEM)==3) THEN
        CALL SAL141(3,GG%RPAR(:,ELEM),X1,Y1,1)
        CALL SAL141(3,GG%RPAR(:,ELEM),X2,Y2,2)
        ALIGN(3,1,J)=X1; ALIGN(3,2,J)=Y1;
        DET1 = DET_ROSETTA(ALIGN(1,1,J),3)
        ALIGN(3,1,J)=X2; ALIGN(3,2,J)=Y2;
        DET2 = DET_ROSETTA(ALIGN(1,1,J),3)
        LFOLD(J)=((ABS(DET1).LE.1.0E-4).OR.(ABS(DET2).LE.1.0E-4))
        IF(LFOLD(J)) EXIT
      ENDIF
    ENDDO
  ENDDO
  ...
  ENDDO
  IF(IPRINT.GT.2) THEN
    WRITE(FOUT,'(/36H MUSACG: PERIMETERS BEFORE UNFOLDING)')
    ...
    WRITE(FOUT,'(7H UNFOLD,1P,10L12/(6X,10L12))') LFOLD(:NPERIM)
  ENDIF
  !----
  !  Unfold macro geometry
  !----
  DO J=1,NPERIM
    IF(LFOLD(J)) THEN
      AXIS_XY(1)=ALIGN(1,1,J) ; AXIS_XY(2)=ALIGN(1,2,J) ; AXIS_XY(3)=ALIGN(2,1,J) ; AXIS_XY(4)=ALIGN(2,2,J)
      CALL SALFOLD_0(GG_MAC,AXIS_XY)
    ENDIF
  ENDDO
  ...
  IF(GG_MAC%NALBG > 6) CALL XABORT('MUSACG: Albedo array overflow(1).')
  DO I=1,NSOUT
    KEYMRG(-I)=-I
