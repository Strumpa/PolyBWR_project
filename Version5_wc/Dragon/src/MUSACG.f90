...
SUBROUTINE MUSACG(ITRACK,IFTRK,IPRINT,IMACRO,NBSLIN,RCUTOF,GG,LGINF,NBNODE_MACRO,NSURF_MACRO)
  ...
  !----
  MAXGLOB=2*NPERIM*GG_MAC%NB_ELEM
  ...
  DO IPASS=1,2 ! two passes are required to get rid of unfolding axis
    ...
    OUT1: DO IB=1,GG_MAC%NBBCDA
      ...
      DO I=1,GG_MAC%BCDATAREAD(IB)%NBER
        ...
        IF(INDBC==0) CYCLE
        IF(INDBC.GT.MAXGLOB) CALL XABORT('MUSACG: IFOLD overflow(1)')
        IGLOB=ELEM_MACRO(IFOLD_GLOB(INDBC))
        IF((GG%IPAR(2,IGLOB).GT.0).AND.(GG%IPAR(3,IGLOB).GT.0)) CYCLE OUT1
    ...
    DO IB=1,GG_MAC%NBBCDA
      IF(LFOLD(IB)) THEN
        ...
        DO ELEM=1,GG_MAC%NB_ELEM
          IF(IFOLD(ELEM).GT.MAXGLOB) CALL XABORT('MUSACG: IFOLD overflow(2)')
