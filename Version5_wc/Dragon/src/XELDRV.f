...
*  LOCAL VARIABLES
*----
      INTEGER         NSTATE,NBCDA
      PARAMETER      (NSTATE=40,NBCDA=6)
     ...
     >                IOUT,ITGEO,IUSED(NBCDA),ICMAX,ICODE(NBCDA),
     >                ICOLD(NBCDA),NANGLO,MXSUB,MXSUBO,MXSEGO,ILONG,
     >                IFMT,I,ISIDE
      LOGICAL         LEAKSW, LELCHK, SWNOGE, SWCONS, EMPTY, LCM
      REAL            ALBEDO(NBCDA),ALBOLD(NBCDA),EXTKOP(NSTATE),
     >                ZERO,ONE,DENS,PCORN
...
*----
   ...
   10    CONTINUE
     ...
     >                         NCOR,NANGL,MXSUB,MXSEG
         IF( NALBGO.LE.0.OR.NALBGO.GT.NBCDA )THEN
...
*     COMPUTE LEAKAGE SWITCH
      LEAKSW=.TRUE.
      DO 80 IR= -NSOUT, -1
         ISIDE=-MATMRG(IR+NSOUT+1)
         IF(ISIDE.GT.6) CALL XABORT('XELDRV: NBCDA>6.')
         IUSED(ISIDE)= 1
         LEAKSW= LEAKSW .AND. ALBEDO(ISIDE).EQ.ONE
   80 CONTINUE
      LEAKSW=.NOT.LEAKSW
...
*     SAVE GENERAL TRACKING INFORMATION.
  ...
  100 CONTINUE
     ...
     >  ISTATE(16),ISTATE(27),ISTATE(30),ISTATE(39),ISTATE(40)
