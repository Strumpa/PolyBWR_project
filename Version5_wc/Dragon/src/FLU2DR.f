*DECK FLU2DR
     ...
     4 KEYFLX,VOL,XSTOT,XSTRC,XSDIA,XSNUF,XSCHI,LREBAL,INITFL,NMERG,
     5 IMERG)
*
*-----------------------------------------------------------------------
*
...
*
      ...
      AKEEP(6)=1.0D0
      AKEEP(7)=1.0D0
      CALL XDRSET(DIFHET,NMERG*NGRP,0.0)
...
*
     ...
     >      //'GROUP PROBLEMS. CONSIDER ADDING THAT FUNCTIONALITY. ')
         ...
         SUMXSTOT=0.0
         DO IR=1,NREG
            SUMXSTOT = SUMXSTOT + XSTOT(MATCOD(IR),1)
         ENDDO
         AVXSTOT = (SUMXSTOT/NREG)

         ALLOCATE(XXX(LX+1))
         CALL LCMGET(IPTRK,'XXX',XXX)
         CALL LCMGET(IPTRK,'XLEN',XLEN)

         OMEGA = (2*PI)/(XLEN*AVXSTOT)
         ...
         IF(ITYPE.EQ.2)THEN
            ...
            DO IX=1,LX
               ...
               IND=KEYFLX(IX,1,1)
               IF(IND.GT.0)
     >            FLUX(IND,IGR,2) = COS(EKKK*OMEGA*AVXSTOT*PARTX)
            ENDDO
         ELSE
...
*----
     ...
     1   VOL,KEYFLX(1,1,1),XSTOT,XSNUF,XSCHI,NMERG,IMERG,DIFHET,
     2   FLUX(1,1,2),B2,ILEAK,LEAKSW,OLDBIL,AKEFF,AFLNOR)
         AKEFFO=AKEFF
         AKEEP(2)=AKEFF
      ENDIF
      B2VALO=B2(4)
...
*           FUNDAMENTAL MODE CONDITION.
     ...
     2      IPRT,DIFHET,GAMMA,AKEFF,INORM,B2)
         ENDIF
...
*         NO B-N LEAKAGE CALCULATION REQUIRED
          ...
          CALL FLUKEF(IPRT,IPMACR,NGRP,NREG,NUNKNO,NMAT,NIFIS,MATCOD(1),
     1    VOL,KEYFLX(1,1,1),XSTOT,XSNUF,XSCHI,NMERG,IMERG,DIFHET,
     2    FLUX(1,1,3),B2,ILEAK,LEAKSW,OLDBIL,AKEFF,AFLNOR)
