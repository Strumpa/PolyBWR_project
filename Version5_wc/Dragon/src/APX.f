...
*----
     ...
     1 MAXLIN=50,MAXMAC=2)
      ...
      TYPE(C_PTR) IPAPX,IPLB1,IPDEPL,IPEDIT,IPMICR
     ...
     2 NOMREA(NREAK)*4,HSMG*131,NOMMAC(MAXMAC)*8
      ...
      CHARACTER REV*48,DATE*64,HEQUI*80,CDIRO*12
*----
*  ALLOCATABLE ARRAYS
*----
...
*----
  350 IF(NPAR.GT.0) DEALLOCATE(PARFMT,PARNAM)
      ...
      ELSE IF(C_ASSOCIATED(IPLB1)) THEN
        IPMICR=IPLB1
      ELSE
        CALL XABORT('APX: NO EDITION OBJECT OR MICROLIB DEFINED.')
      ENDIF
*     -------------------------------------------
      CALL APXCAL(IMPX,IPAPX,IPDEPL,IPMICR,HEQUI)
*     -------------------------------------------
