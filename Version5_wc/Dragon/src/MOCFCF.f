...
*----
     ...
     1 OMEGAX,OMEGAY,OMG2,PHIU,FLM,FLP,CYM,CYP,DFLM,DFLP,CYM2,CYP2,
     2 FLUV,DFLUV
...
*----
     ...
     1 KANGL(MXSUB,NBATCH),NRSEG(MXSEG,NBATCH),SEGLEN(MXSEG,NBATCH))
      ALLOCATE(PHIU(KPN,NGEFF),FLM(NGSS,MXSEG),FLP(NGSS,MXSEG),
     1 CYM(NGSS,MXSEG),CYP(NGSS,MXSEG),EXPT(NGSS*MXSEG))
...
*$OMP END PARALLEL DO
        ENDDO ! IBATCH
        DEALLOCATE(EXP2,FLUX,TRHAR,RHARM)
...
*----
      DEALLOCATE(EXPT,CYP,CYM,FLP,FLM,PHIU)
