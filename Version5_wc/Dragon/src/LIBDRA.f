...
*----
      ...
      REAL, ALLOCATABLE, DIMENSION(:) :: AWR,DELTA,TOTAL,GOLD,ENER,
     1 BIN,EBIN,SIGS2,SCAT2,TOTAL2,SIGF2,CHI2,SADD2,GOLD2,BIN2,CHI4G2
      REAL, ALLOCATABLE, DIMENSION(:,:) :: SIGS,SIGF,CHI,SADD,CHI4G
      REAL, ALLOCATABLE, DIMENSION(:,:,:) :: SCAT
      DOUBLE PRECISION, ALLOCATABLE, DIMENSION(:) :: ZNPHI,ZNPHI2
      LOGICAL, ALLOCATABLE, DIMENSION(:) :: LSCAT,LADD
      CHARACTER(LEN=72), ALLOCATABLE, DIMENSION(:) :: HTITLE
      REAL NFTOT(NGRO),HFACT(NGRO),KAPF
*----
...
*----
     ...
     1  15HRE DEPRECIATED.)')
      ...
      IF((IMPX.GT.0).AND.(LENGT.GT.0)) THEN
         ...
         CALL LCMGTC(IPDRL,'README',72,LENGT,HTITLE)
         WRITE (IOUT,940)
         DO 10 J=1,LENGT
...
*----
      ...
      IF(MASKI(IMX)) THEN
         ...
         PRINT *, "IMX =", IMX
         PRINT *, "Contents of ISONAM"
         PRINT '(A4)', ISONAM(1,IMX)
         PRINT '(A4)', ISONAM(2,IMX)
         PRINT '(A4)', ISONAM(3,IMX)
         PRINT *, HNAMIS
...
*
         ...
         IF(LTITLE.GT.0) THEN
           LTITLE=(LTITLE-1)/18+1
           ALLOCATE(HTITLE(LTITLE))
           CALL LCMGTC(IPDRL,'README',72,LTITLE,HTITLE)
           IF(IMPX.GT.0) THEN
             WRITE (IOUT,930)
             DO 20 J=1,LTITLE
             WRITE (IOUT,'(1X,A)') TRIM(HTITLE(J))
...
*----
            ...
            SIGF(:NGRO,:NDEL0)=0.0
            CHI(:NGRO,:NDEL0)=0.0
            CHI4G(:NGRO,:NBESP)=0.0
            GOLD(:NGRO)=0.0
            SIGS(:NGRO,:NL)=0.0
            SCAT(:NGRO,:NGRO,:NL)=0.0
            SADD(:NGRO,:NED)=0.0
            IF(LBIN.GT.0) BIN(:3*LBIN)=0.0
     ...
     >      1P,E12.4,18H KELVIN. FACTOR = ,E12.4)') TEMP(ITM),TERPM
            ...
            PRINT *, "LIBDRA: CALL LIBDRB #2 for HNAMIS=", HNAMIS
            CALL LIBDRB (IPDRL,HNAMIS,NGRO,NL,NDEL0,NBESP,ENER,
     1      SN(1,IMX),SB(1,IMX),NED,HVECT,DELTA,LBIN,NFS,EBIN,AWR(IMX),
  ...
  210       CONTINUE
     ...
     >      SCAT2,SIGS2)
         ...
         IF((HNAMIS(1:4).EQ."U235").OR.(HNAMIS(1:4).EQ."U238")) THEN
         PRINT *, "LIBDRA after LIBDRB, for ISO=", HNAMIS
         DO 215 IGR=1,NGRO
         IF(SADD(IGR,18)/SADD(IGR,1).GT.200000000.0) THEN
         PRINT *, "WARNING: KappaF=", SADD(IGR,18) / SADD(IGR,1)
         PRINT *, "IN GROUP IGR", IGR
         ELSEIF(SADD(IGR,18)/SADD(IGR,1).LT.180000000.0) THEN
         PRINT *, "WARNING: KappaF=", SADD(IGR,18) / SADD(IGR,1)
         PRINT *, "IN GROUP IGR", IGR
         ENDIF
  215    CONTINUE
...
*----
         ...
         IF(LTITLE.GT.0) THEN
            CALL LCMPTC(KPLIB,'README',72,LTITLE,HTITLE)
            DEALLOCATE(HTITLE)
         ENDIF
         DO 220 IG=1,NGRO
  ...
  220    CONTINUE
         ...
         TOTAL(:NGRO)=REAL(ZNPHI(:NGRO))
         CALL LCMPUT(KPLIB,'NWT0',NGRO,2,TOTAL)
  ...
  340    CONTINUE
        IF((HNAMIS(1:4).EQ."U235").OR.(HNAMIS(1:4).EQ."U238")) THEN
         PRINT *, "At the end of LIBDRA, for ISO=", HNAMIS
         DO 345 IGR=1,NGRO
         IF((SADD(IGR,18)/SADD(IGR,1)).GT.200000000.0) THEN
            PRINT *, "IN GROUP IGR", IGR
            PRINT *, "WARNING FOR KappaF=", SADD(IGR,18) / SADD(IGR,1)
         ELSEIF((SADD(IGR,18)/SADD(IGR,1)).LT.180000000.0) THEN
            PRINT *, "IN GROUP IGR", IGR
            PRINT *, "WARNING FOR KappaF=", SADD(IGR,18)/SADD(IGR,1)
         ENDIF
  ...
  400 CONTINUE
      IF((HNAMIS(1:4).EQ."U235").OR.(HNAMIS(1:4).EQ."U238")) THEN
      PRINT *, "LIBDRA: RETRIEVE H-FACTORS"
      CALL LCMGET(KPLIB,"NFTOT",NFTOT)
      CALL LCMGET(KPLIB,"H-FACTOR",HFACT)
      DO 500 IGR=1,NGRO
        KAPF=HFACT(IGR)/NFTOT(IGR)
        IF((KAPF.GT.200000000.0).OR.(KAPF.LT.180000000.0)) THEN
          PRINT *, "WARNING: KAPF=", KAPF
          PRINT *, "FOR IG=", IGR
      ENDIF
  500    CONTINUE
      ENDIF
