*DECK EDIDRV
     ...
     >                  IADJ,NOUT,HVOUT,BB2,IEDCUR,IGOVE,IREB,INORM,
     >                  IONEF)
*
*-----------------------------------------------------------------------
*
...
*----
     ...
     >            NGROUP,NGCOND,NBMIX,NREGIO,NMERGE,ILEAKS,ILUPS,INORM,
     >            NW,MATCOD,VOLUME,KEYFLX,IGCOND,IMERGE,FLUXES,AFLUXE,
...
*----
     ...
     >              NDFP,ILEAKS,ILUPS,INORM,NW,MATCOD,VOLUME,KEYFLX,
     >              CURNAM,IGCOND,IMERGE,FLUXES,AFLUXE,EIGENK,EIGINF,
     >              B2,DEN,ITYPE,IDEPL,LSISO,EMEVF,DECAY,YIELD,FIPI,
...
*----
     ...
     >             ILUPS,INORM,NW,MATCOD,VOLUME,KEYFLX,CURNAM,IGCOND,
     >             IMERGE,FLUXES,AFLUXE,EIGENK,EIGINF,B2,DEN,ITYPE,
     >             LSISO,EMEVF,DECAY,YIELD,FIPI,FIFP,PYIELD,ITRANC,
     >             LISO,NMLEAK)
      ENDIF
*----
*  REBUILD THE MACROLIB FROM CONDENSED/HOMOGENIZED MICROLIB
*----
      IF(IREB.EQ.1) THEN
        IF(IPRINT.GT.0) WRITE(6,'(30H EDIDRV: REBUILD THE MACROLIB.)')
        IF((CURNAM.EQ.' ').OR.(NBMICR.EQ.0).OR.(NBISO.EQ.0)) THEN
          WRITE(HSMG,'(44HEDIDRV: UNABLE TO REBUILD THE MACROLIB. (NBM,
     >    4HICR=,I5,7H NBISO=,I5,2H).)') NBMICR,NBISO
          CALL XABORT(HSMG)
        ENDIF
        CALL LCMSIX(IPEDIT,CURNAM,1)
        CALL LCMGET(IPEDIT,'STATE-VECTOR',IPAR)
        NBNISO=IPAR(2)
        IF(NBNISO.EQ.0) CALL XABORT('EDIDRV: UNABLE TO REBUILD THE MAC'
     >  //'ROLIB.')
        ALLOCATE(INNAM(3*NBNISO),NMIX(NBNISO),NDEN(NBNISO),MASK(NMERGE),
     >  MASKL(NGCOND))
        CALL LCMGET(IPEDIT,'ISOTOPESUSED',INNAM)
        CALL LCMGET(IPEDIT,'ISOTOPESMIX',NMIX)
        CALL LCMGET(IPEDIT,'ISOTOPESDENS',NDEN)
        ...
        MASKL(:NGCOND)=.TRUE.
        ITSTMP=0
        CALL LCMLEN(IPEDIT,'MACROLIB',ILCMLN,ITYLCM)
        CALL LCMSIX(IPEDIT,'MACROLIB',1)
        CALL LCMGET(IPEDIT,'TIMESTAMP',TIMEF)
        CALL LCMSIX(IPEDIT,' ',2)
        IF(ILCMLN.EQ.-1) CALL LCMDEL(IPEDIT,'MACROLIB')
        CALL LIBMIX(IPEDIT,NMERGE,NGCOND,NBNISO,INNAM,NMIX,NDEN,MASK,
     >  MASKL,ITSTMP,TIMEF)
        CALL LCMSIX(IPEDIT,'MACROLIB',1)
        ...
        CALL LCMSIX(IPEDIT,' ',2)
        CALL LCMSIX(IPEDIT,' ',2)
        DEALLOCATE(MASKL,MASK,NDEN,NMIX,INNAM)
      ENDIF
*----
*  AVERAGE MACROLIB FISSION INFORMATION TO A UNIQUE FISSILE ISOTOPE
*----
      IF(IONEF.EQ.1) THEN
        IF(CURNAM.EQ.' ') CALL XABORT('EDIDRV: MISSING SAVE DIRECTORY.')
        CALL LCMSIX(IPEDIT,CURNAM,1)
        CALL LCMLEN(IPEDIT,'MACROLIB',ILCMLN,ITYLCM)
        IF(ILCMLN.EQ.0) CALL XABORT('EDIDRV: MISSING MACROLIB.')
        CALL LCMSIX(IPEDIT,'MACROLIB',1)
        CALL EDIFIS(IPEDIT)
        CALL LCMSIX(IPEDIT,' ',2)
        CALL LCMSIX(IPEDIT,' ',2)
...
*
      ...
      DEALLOCATE(DECAY,EMEVF,SCATS,SIGS,FADJC,TAUXT,WE,WLETY)
      IF(HSIGN.EQ.'L_LIBRARY') THEN
