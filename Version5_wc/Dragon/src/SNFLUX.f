...
*----
      ...
      INTEGER     IPAR(NSTATE),NCODE(6),BSINFO(2),EELEM,ESCHM,P
      INTEGER     APPCELLCNT,MAXWAVECNT,MAXCELLCNT,PLNWAVECNT,PLNCELLCNT
      REAL        ZCODE(6),SIDE
...
*---
      ...
      INTEGER, ALLOCATABLE, DIMENSION(:) :: TASKSINWAVE,TASKLST
...
*------------
  ...
  440   CONTINUE ! END OF ENERGY LOOP
     ...
     1      NBMIX,NPQ,NSCT,MAT,VOL,SGAR,QEXT,DU,DE,W,DB,DA,CONNEC,
     2      IZGLOB,SIDE,CONFROM,FUNKNO,MN,DN)
     ...
     2      IZGLOB,SIDE,CONFROM,FUNKNO,MN,DN)
        ...
        ELSE IF(NKBA.GE.1)THEN
          ...
          ALLOCATE(TASKSINWAVE(MAXWAVECNT)) 
          ALLOCATE(CON_WHOAMI(2,(NHEX*3)*2,6))
          ALLOCATE(CON_TOWHOM(2,(NHEX*3)*2,6))
          CON_WHOAMI(:2,:((NHEX*3)*2),:6)=0
          CON_TOWHOM(:2,:((NHEX*3)*2),:6)=0
          CALL LCMGET(IPTRK,'TASKLIST',TASKLST)
          CALL LCMGET(IPTRK,'TASKSINWAVE',TASKSINWAVE)
          CALL LCMGET(IPTRK,'CON_WHOAMI',CON_WHOAMI)
          CALL LCMGET(IPTRK,'CON_TOWHOM',CON_TOWHOM)
          IF(ISCHM.EQ.1)THEN
            CALL SNFDH1(NUN,NGEFF,IMPX,INCONV,NGIND,NHEX,NBC,IELEM,
     1      ISPLH,NBMIX,NPQ,NSCT,MAT,VOL,SGAR,QEXT,DU,DE,W,DB,DA,
     2      CONNEC,SIDE,CONFROM,TASKLST,TASKSINWAVE,CON_WHOAMI,
     3      CON_TOWHOM,FUNKNO,MN,DN)
          ELSE IF(ISCHM.EQ.2)THEN
            CALL SNFDH2(NUN,NGEFF,IMPX,INCONV,NGIND,NHEX,NBC,IELEM,
     1      ISPLH,NBMIX,NPQ,NSCT,MAT,VOL,SGAR,QEXT,DU,DE,W,DB,DA,
     2      CONNEC,SIDE,CONFROM,TASKLST,TASKSINWAVE,CON_WHOAMI,
     3      CON_TOWHOM,FUNKNO,MN,DN)
          ENDIF
          DEALLOCATE(CON_TOWHOM,CON_WHOAMI,TASKSINWAVE,TASKLST)
        ENDIF
...
*------------
  ...
  490   CONTINUE ! END OF ENERGY LOOP
     ...
     2      W,MRMZ,DC,DB,DA,CONNEC,IZGLOB,CONFROM,FUNKNO,MN,DN)
          ELSEIF(ISCHM.EQ.2)THEN
     ...
     2      W,MRMZ,DC,DB,DA,CONNEC,IZGLOB,CONFROM,FUNKNO,MN,DN)
        ...
        ELSE IF(NKBA.GE.1)THEN
          ...
          CALL LCMGET(IPTRK,'CON_TOWHOM',CON_TOWHOM)
          IF(ISCHM.EQ.1)THEN
            CALL SNFDH3(NKBA,NUN,NGEFF,IMPX,INCONV,NGIND,NHEX,LZ,NBC,
     1      IELEM,ISPLH,SIDE,NBMIX,NPQ,NSCT,MAT,VOL,NCODE,ZCODE,SGAR,
     2      QEXT,DU,DE,DZ,W,MRMZ,DC,DB,DA,CONNEC,CONFROM,TASKLST,
     3      TASKSINWAVE,CON_WHOAMI,CON_TOWHOM,FUNKNO,MN,DN)
          ELSEIF(ISCHM.EQ.2)THEN
            CALL SNFDH4(NKBA,NUN,NGEFF,IMPX,INCONV,NGIND,NHEX,LZ,NBC,
     1      IELEM,ISPLH,SIDE,NBMIX,NPQ,NSCT,MAT,VOL,NCODE,ZCODE,SGAR,
     2      QEXT,DU,DE,DZ,W,MRMZ,DC,DB,DA,CONNEC,CONFROM,TASKLST,
     3      TASKSINWAVE,CON_WHOAMI,CON_TOWHOM,FUNKNO,MN,DN)
          ENDIF
