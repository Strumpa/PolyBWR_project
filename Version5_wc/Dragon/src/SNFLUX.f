...
*------------
  ...
  200   CONTINUE ! END OF ENERGY LOOP
     ...
     2      DU,DE,W,MRM,MRMY,DB,DA,PL,FUNKNO,ISBS,SUM(NBS),ISBSM,BS,
     3      MAXL,WX,WX,CST,LADPT(2:3))
          ENDIF
...
*----
  ...
  340   CONTINUE ! END OF ENERGY LOOP
     ...
     1      NBMIX,NPQ,NSCT,MAT,VOL,SGAR,NCODE,ZCODE,QEXT,LFIXUP,DU,DE,
     2      DZ,W,MRMX,MRMY,MRMZ,DC,DB,DA,PL,FUNKNO,ISBS,SUM(NBS),ISBSM,
     3      BS,MAXL)
          ELSE IF(ISCHM.EQ.2)THEN
            CALL SNFT23(NUN,NGEFF,IMPX,INCONV,NGIND,LX,LY,LZ,IELEM,
     1      NBMIX,NPQ,NSCT,MAT,VOL,SGAR,NCODE,ZCODE,QEXT,DU,DE,DZ,W,
     2      MRMX,MRMY,MRMZ,DC,DB,DA,PL,FUNKNO,ISBS,SUM(NBS),ISBSM,BS,
     3      MAXL)
          ELSE IF(ISCHM.EQ.3)THEN
     ...
     3      ISBS,SUM(NBS),ISBSM,BS,MAXL,WX,WX,WX,CST,LADPT(2:4))
          ENDIF
        ELSE IF(NKBA.GT.0)THEN
          IF(ISCHM.EQ.1)THEN
            IF(ISBS.EQ.1) CALL XABORT('SNFLUX: BOUNDARY SOURCES NOT YE'
     ...
     1      NBMIX,NPQ,NSCT,MAT,VOL,SGAR,NCODE,ZCODE,QEXT,LFIXUP,DU,DE,
     2      DZ,W,MRMX,MRMY,MRMZ,DC,DB,DA,PL,FUNKNO)
          ELSE IF(ISCHM.EQ.2) THEN
            IF(ISBS.EQ.1) CALL XABORT('SNFLUX: BOUNDARY SOURCES NOT YE'
     1      //'T IMPLEMENTED WITH KBA ALGORITHM.')
            CALL SNFD23(NKBA,NUN,NGEFF,IMPX,INCONV,NGIND,LX,LY,LZ,IELEM,
     1      NBMIX,NPQ,NSCT,MAT,VOL,SGAR,NCODE,ZCODE,QEXT,DU,DE,DZ,W,
     2      MRMX,MRMY,MRMZ,DC,DB,DA,PL,FUNKNO)
          ELSE IF(ISCHM.EQ.3)THEN
            CALL XABORT('SNFLUX: KBA ALGORITHM NOT AVAILABLE.')
          ENDIF
        ENDIF
...
*------------
  ...
  490   CONTINUE ! END OF ENERGY LOOP
     ...
     2      QEXT,DU,DE,DZ,W,MRMZ,DC,DB,DA,PL,CONNEC,CONFROM,TASKLST,
     3      TASKSINWAVE,CON_WHOAMI,CON_TOWHOM,FUNKNO)
          ELSEIF(ISCHM.EQ.2)THEN
