...
!
  USE GANLIB
  USE SAL_GEOMETRY_TYPES,  ONLY : T_G_BASIC,LMERGM,ISPEC,NANIS
  USE SALGET_FUNS_MOD
  IMPLICIT     NONE
  !----
  ...
  IF(IPRINT .GT. 1) THEN
     ...
     WRITE(IOUT,130) ISTATT(23:24),ISTATT(26:28),ISTATT(40)
  ENDIF
  RETURN
  !----
  ! Formats
  ...
  100 FORMAT(/14H STATE VECTOR:/ &
     ...
     & 7H GEOT  ,I9,21H   (TYPE OF GEOMETRY)/ &
  ...
  130 FORMAT( &
     ...
     & 7H MERGMX,I9,33H   (0/1= MERGMIX ACTICATION FLAG)/ &
     & 7H NBATCH,I9,41H   (NUMBER OF TRACKS IN EACH OPENMP CORE)/ &
     & 7H NUNCUR,I9,54H   (NUMBER OF ADDITIONAL INTERFACE CURRENT COMPONENTS)/ &
