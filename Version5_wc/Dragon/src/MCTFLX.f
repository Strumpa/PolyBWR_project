...
*----
     ...
     <        ISAXIS(3),NOCELL(3),NUCELL(3),MXMSH,MAXREG,NBTCLS,
     <        MAXPIN,MAXMSP,MAXRSP,MXGSUR,MXGREG,NUNK,MAXMSH,NDIM,
     <        ESTATE(NSTATE),GSTATE(NSTATE),ITALLY,IND,IOF,IGR,ILON1,
     <        ILON2,ITYLCM,IBANK1,IBANK2,NBCDA
      REAL    RAND,NUCALL,NULIMIT,SCORE1(3),FACT1,FACT2
      LOGICAL LKEEP
...
*----
     ...
     1 IDREG,ITPIN,INDGRP,IMERGE,IGCR,ICODE
      REAL, ALLOCATABLE, DIMENSION(:) :: NUCYCLE,ALBEDO
...
*----
      ALLOCATE(INMIX(NFREG),IBCRT(NFSUR),ALBEDO(NBCDA),ICODE(NBCDA))
...
*----
            ...
            POS(2) = DNGEN1(3,NTRK)
            POS(3) = DNGEN1(4,NTRK)
            CALL MCTRK(IPTRK,IPRINT,NFREG,NFSUR,NDIM,NBCDA,NMIX,ANGBC,
     1      ITYPBC,MAXMSH,NUCELL,MXGSUR,MXGREG,MAXPIN,IBCRT,ICODE,
     2      ALBEDO,IUNFLD,DGMESH,XYZL,INDEX,IDREG,DCMESH,ITPIN,DRAPIN,
     3      ISEED,NGRP,NL,NFM,NDEL,NED,INMIX,XSTOT,XSS,XSN2N,XSN3N,
     4      XSSNN,XSNUSI,XSCHI,XSEDI,MIX,ISONBR,NUCALL,POS,ITALLY,
     5      NBSCO,NMERGE,NGCOND,IMERGE,INDGRP,SCORE1,SCORE2)
            IF(ISONBR.GT.0) THEN
              IBANK2=IBANK2+1
...
*     TRACKING RELATED
      CALL LCMSIX(IPTRK,' ',2)
      DEALLOCATE(DRAPIN,ITPIN,DCMESH,IDREG,INDEX,DGMESH,IUNFLD,ICODE,
     1 ALBEDO,IBCRT,INMIX)
*----
