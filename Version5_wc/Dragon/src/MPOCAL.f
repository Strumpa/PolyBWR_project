*DECK MPOCAL
      SUBROUTINE MPOCAL(IMPX,IPMPO,IPDEPL,IPMICR,HEDIT)
*
*-----------------------------------------------------------------------
*
*Purpose:
...
*----
      TYPE(C_PTR) IPMPO,IPDEPL,IPMICR,IPSPH
      INTEGER IMPX
...
*----
     ...
     1 DIMS_MPO)
      ...
      CALL MPOCA2(IPMPO,IPMICR,HEDIT,NREA,NISO,NADRX,NED,NPRC,ILEAK,
...
*----
      IF((IDF.EQ.2).OR.(IDF.EQ.3).OR.(IDF.EQ.4).OR.(NALBP.GT.0)) THEN
        CALL MPOIDF(IPMPO,IPMICR,HEDIT,NG,NMIL,ICAL,IDF,NALBP,FNORM,
     1  VOLMIL,FLXMIL)
      ENDIF
...
*----
      IF(NISFS*NISPS.GT.0) THEN
         CALL MPOGEY(IPMPO,IPMICR,HEDIT,NISO,NG,NMIL,NBISO,ICAL,NDFI,
     1   NISFS,NISPS)
      ENDIF
      RETURN
*
