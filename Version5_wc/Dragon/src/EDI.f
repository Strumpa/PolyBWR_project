...
*----
      ...
      IF(NENTRY.GT.4) THEN
        DO 70 IEN=5,NENTRY
          IF((IENTRY(IEN).EQ.4).AND.(JENTRY(IEN).EQ.2)) THEN
            IFGEO=FILUNIT(KENTRY(IEN))
            IF(IEN+1.GT.NENTRY) THEN
              CALL XABORT('EDI: A MACROGEOMETRY SHOULD FOLLOW.')
            ENDIF
         ELSE IF((IENTRY(IEN).EQ.1).OR.(IENTRY(IEN).EQ.2)) THEN
            CALL LCMGTC(KENTRY(IEN),'SIGNATURE',12,HSIGN)
            IF(HSIGN.EQ.'L_GEOM') THEN
              IF((IKGEO1.EQ.0).AND.(IFGEO.EQ.0)) THEN
              ...
              ELSE IF(IKGEO2.EQ.0) THEN
                ...
                MACGEO=HENTRY(IEN)
                IKGEO2=IEN
                CALL LCMPTC(IPEDIT,'LINK.MACGEOM',12,MACGEO)
...
*----
  ...
  107       CONTINUE
      ...
      IGCR(:NGRP+1)=NGRP
      ECR(:NGRP+1)=0.0
      CALL EDIGET(IPEDIT,IPGEO2,IFGEO,NGRP,NGCR,NREG,NBMIX,MAT,ITMERG,
     1 NMERGE,IHF,IFFAC,ILUPS,NSAVES,NSTATS,IGCR,ECR,IMERGE,CURNAM,
     2 OLDNAM,IADF,NW,ICURR,NBMICR,CARISO,NACTI,IACTI,IPRINT,MAXPTS,
     3 ICALL,ISOTXS,LISO,LDEPL,LMACR,IADJ,MACGEO,IEUR,NOUT,HVOUT,BB2,
     4 IEDCUR,IGOVE,IREB,INORM,IONEF)
...
*----
     ...
     4 LMACR,IADJ,NOUT,HVOUT,BB2,IEDCUR,IGOVE,IREB,INORM,IONEF)
*----
*  PRINT THE STATE-VECTOR
*----
      IF(IPRINT.GT.0) THEN
        CALL LCMGET(IPEDIT,'STATE-VECTOR',ISTATE)
        WRITE(IOUT,200) ISTATE(1:10)
        WRITE(IOUT,210) ISTATE(11:12),ISTATE(17:22)
        WRITE(IOUT,220) ISTATE(23:28)
      ENDIF
*----
...
*----
      ...
      IF(IPRINT.GT.3) CALL LCMLIB(IPEDIT)
*----
...
*
  ...
  200 FORMAT(/24H EDITION-RELATED OPTIONS/1X,23(1H-)/
     ...
     7  7H IADF  ,I8,46H   (=0: NO ADF; =1: ALBS INFO; =2: BOUNDARY FL,
     8  59HUX INFO; =3: DISCONTINUITY FACTORS; =4: MATRIX DISCONTINUIT,
     9  10HY FACTORS)/
  ...
  220 FORMAT(
     1  7H NOUT  ,I8,47H   (=0: OUTPUT ALL REACTIONS; >0: NUMBER OF OUT,
     2  14HPUT REACTIONS)/
     3  7H IEDCUR,I8,40H   (=0/1: FLUX/FLUX AND CURRENT EDITION)/
     4  7H GOLVER,I8,38H   (=0/1: GOLFIER-VERGAIN FLAG OFF/ON)/
     5  7H IREB  ,I8,37H   (=0/1: MACROLIB REBUILDING OFF/ON)/
     6  7H INORM ,I8,47H   (=0/1: WITHIN-GROUP SCATTERING NORMALIZATION,
     7  8H OFF/ON)/
     8  7H IONEF ,I8,47H   (=0/1: ONE-FISSILE ISOTOPE AVERAGING OFF/ON))
      END
