...
*----
      ...
      TYPE(VECTOR_ARRAY), ALLOCATABLE, DIMENSION(:) :: SIGT_V,WEIGHT_V,
     1 GAMMA_V
...
*----
      ALLOCATE(JPLIB2(NIRES),JPLIB3(NIRES))
      ALLOCATE(NJJ(NGRP,NIRES),NPSYS(MAXNOR*NGRP),MRANK(NGRP))
      ALLOCATE(SIGT_V(NGRP),SIGT_M(NGRP,NIRES),SCAT_M(NGRP,NGRP,NIRES),
     1 WEIGHT_V(NGRP),GAMMA_V(NGRP),PSI_M(NGRP),SIGTAD(NIRES,NGRP))
      ...
      DO JRES=1,NIRES
        ...
        CALL LCMSIX(IPLIB,TEXT12,1)
          IF(JRES.NE.IRES) THEN
            JPLIB2(JRES)=LCMGID(IPLIB,'SIGT_M') ! holds SIGT_M information
          ELSE
            JPLIB2(JRES)=C_NULL_PTR
          ENDIF
          JPLIB3(JRES)=LCMGID(IPLIB,'SCAT_M') ! holds SCAT_M information
...
*----
*  RECOVER RSE INFORMATION FROM MICROLIB (PART 2)
*----
        MI=MRANK(IG)
        ...
        CALL C_F_POINTER(IOFSET,WEIGHT_V(IG)%VECTOR,(/MI/))
        CALL LCMGPD(KPLIB,'GAMMA_V',IOFSET)
        CALL C_F_POINTER(IOFSET,GAMMA_V(IG)%VECTOR,(/MI/))
        DO JRES=1,NIRES
          IF(JRES.NE.IRES) THEN
            CALL LCMGPL(JPLIB2(JRES),IG,IOFSET)
          ...
          DO JG=IG-NJJ(IG,JRES)+1,IG
            MJ=MRANK(JG)
            CALL LCMGPL(JPLIB3(JRES),IPOS+IG-JG,IOFSET)
...
*----
        IF(NIRES.GT.1) THEN
          ...
          DO JRES=1,NIRES
            ...
            IF(JRES.NE.IRES) THEN
              DO IM=1,MI
                DXI=WEIGHT_V(IG)%VECTOR(IM)
                T1=T1+DXI*XFLUX(IND,IM)
                DO JM=1,MI
                  IF((JM.EQ.IM).AND.(ICORR.EQ.0)) CYCLE
...
*----
   30   IF(IMPX.GT.0) THEN
     ...
     1  NBMIX,NREG,NUN,IPHASE,MAT,VOL,KEYFLX,IREX,SIGGAR,TITR,NIRES,
     2  IRES,NBNRS,MRANK,CONR,GOLD,IPPT1,IPPT2,VOLMER,XFLUX2,UNGAR)
*----
   ...
   40   CALL LCMPDL(JPLI0,IG,NBNRS*MI,2,XFLUX2(1,1,IG))
        IF(IMPX.GT.2) THEN
          DO IND=1,NBNRS
            T1=0.0D0
            DO IM=1,MI
              T1=T1+WEIGHT_V(IG)%VECTOR(IM)*XFLUX2(IND,IM,IG)
            ENDDO
...
*----
      ...
      ENDDO
   50 DEALLOCATE(SIGTAD,PSI_M,GAMMA_V,WEIGHT_V,SCAT_M,SIGT_M,SIGT_V)
      DEALLOCATE(MRANK,NPSYS,NJJ)
      DEALLOCATE(JPLIB3,JPLIB2)
      RETURN
      END
