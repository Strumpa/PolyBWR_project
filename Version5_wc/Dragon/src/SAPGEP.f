...
*
     ...
     1   IDEBAR,IARBVA)
      ...
      ENDIF
      IORIGI(NCALAR)=NORIG
      CALL LCMPUT(IPSAP,'ORIGIN',NCALAR,1,IORIGI)
      DEALLOCATE(IORIGI)
      CALL LCMSIX(IPSAP,' ',2)
*----
...
*        RECOVER LOCAL VARIABLES FROM THE MICROLIB IN EDIT OBJECT.
         ...
         TEXT8=' '
         IF(IPCAD.GT.0) TEXT8=PARCHR(IPCAD)
         CALL LCMGET(IPMICR,'STATE-VECTOR',IDATA)
         MAXNBI=IDATA(2)
         CALL LCMINF(IPMICR,NAMLCM,NAMMY,EMPTY,ILONG,LCM)
         IPLB3=C_NULL_PTR
         DO 60 IBM=1,NMIL
         CALL COMBIB(IPMICR,IPLB3,PARTYP(IPAR),IBM,NAMLCM,TEXT8,MAXNBI,
     1   VALPAR)
         IF(PARTYP(IPAR).EQ.'TEMP') VALPAR=VALPAR-273.16
