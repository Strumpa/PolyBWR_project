...
*----
     ...
     1 NDEL,NED,NESP,NFISS0,NFISSI,NW,NL,NPART,STERN
...
*----
   ...
   20 CONTINUE
      ...
      IF(LENGTH.EQ.-1) THEN
         ...
         ENDIF
         CALL LCMGET(IPLIB,'STATE-VECTOR',ISTATE)
         MAXNFI=MAX(MAXNFI,ISTATE(4)/NESP)
         CALL LCMSIX(IPLIB,' ',2)
      ENDIF
      CALL LIBNFI (IPLIB,NGROUP,NBISO,NBMIX,NDEL,NESP,IPISO,MIX,MAXNFI,
     1 NFISS0,NFISSI,LSAME)
*----
*  BUILD THE MACROSCOPIC CROSS SECTIONS.
*----
      PRINT *, "LIBMIX : CALL LIBDEN" 
      CALL LIBDEN (IPLIB,NGROUP,NBISO,NBMIX,NW,NL,NDEL,NESP,ISONAM,
     1 IPISO,MIX,DEN,MASK,MASKL,NED,JNED,ITRANC,NFISS0,NFISSI,LSAME,
     2 NPART,ITSTMP,TMPDAY,STERN)
*----
...
*----
      ...
      IF(LENGTH.GT.0) THEN
         ...
         LSTOPW=((HPRT1.EQ.'B').OR.(HPRT1.EQ.'C').OR.(HPRT1.EQ.'P'))
      ENDIF
      IF(LSTOPW) THEN
