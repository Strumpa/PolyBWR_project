...
*----
     ...
     1 MAXLIN=50,MAXMAC=2)
      TYPE(C_PTR) IPSAP,IPLB1,IPLB2,IPDEPL,IPEDIT,IPFLUX,IPMICR
      CHARACTER TEXT4*4,TEXT8*8,TEXT12*12,TEXT20*20,HSSAP*80,HSIGN*12,
     1 CDIRO*12,KEYWRD(NKEYS)*4,PARNAM(MAXPAR)*80,PARKEY(MAXPAR)*4,
...
*----
  350 IF(NENTRY.GE.2) THEN
      ...
      IF(.NOT.C_ASSOCIATED(IPEDIT)) THEN
         CALL XABORT('SAP: NO EDITION OBJECT DEFINED.')
      ENDIF
      CALL LCMGTC(IPEDIT,'LAST-EDIT',12,CDIRO)
      IPMICR=LCMGID(IPEDIT,CDIRO)
*     -------------------------------------------
      CALL SAPCAL(IMPX,IPSAP,IPDEPL,IPMICR,LCRON)
*     -------------------------------------------
*----
