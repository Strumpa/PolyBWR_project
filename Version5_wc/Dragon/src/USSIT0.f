...
*----
     ...
     3 VOLMER(0:NBNRS),UNGAR(NUN,NIRES,NGRP)
      LOGICAL LEAKSW,MASKG(NGRP)
      CHARACTER CDOOR*12,TITR*72
*----
*  LOCAL VARIABLES
*----
      TYPE(C_PTR) IPSYS,KPSYS,JPLIB,KPLIB,JPLI0,IPMACR,IPSOU
...
*----
      ...
      REAL, ALLOCATABLE, DIMENSION(:) :: SIGTXS,SIGS0X,SIGG,AWPHI,FUN,
     1 SUN
...
*----
     ...
     1 SIGS0X(0:NBMIX),SIGG(0:NBMIX),AWPHI(0:NBNRS),WEIGH(MAXNOR,NIRES),
...
*        USE A COLLISION PROBABILITY DOOR.
     ...
     1   NBMIX,NANI,MAT,VOL,KNORM,IPIJK,LEAKSW,ITPIJ,.FALSE.,TITR,NALBP)
      ENDIF
      DEALLOCATE(NPSYS)
*----
*  LOOP OVER THE ENERGY GROUPS.
...
*
  ...
  280   CONTINUE
        ...
        XFLUX(:NBNRS,:MAXNOR,IRES)=0.0
        DO 295 IND=1,NBNRS
        DO 290 INOR=1,NORI
        I1=INM(IND,INOR,NBNRS)
        XFLUX(IND,INOR,IRES)=REAL(MATRIX(I1,NBNRS*NORI+1))
  290   CONTINUE
  295   CONTINUE
...
*----
         ...
         ELSE
            FUN((IASM+INOR-1)*NUN+1:(IASM+INOR)*NUN)=0.0
         ENDIF
         SIGG(0)=0.0
         DO 370 IBM=1,NBMIX
...
*----
      DEALLOCATE(MATRIX)
      DEALLOCATE(SIGX,PAV,SIGWS,WSLD,TOTPT,WEIGH,AWPHI,SIGG,SIGS0X,
     1 SIGTXS,XFLUX)
      RETURN
      END
