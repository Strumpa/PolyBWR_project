...
*----
   ...
   80 CONTINUE
      ...
      PRINT *, "HVECT(1)", HVECT(1)
      PRINT *, "HVECT(18)", HVECT(18)
      PRINT *, "LIBEXT: BEFORE SETTING SIGNIFICANT DILUTIONS"
      DO 81 IDIL=1,NDIL
       DO 82 IGR=1,NGRO
       IF(SADD(IGR,18,IDIL)/SADD(IGR,1,IDIL).GT.200000000.0) THEN
        PRINT *, "WARNING: KappaF =", SADD(IGR,18,IDIL)/SADD(IGR,1,IDIL)
        PRINT *, "For IDIL=", IDIL
        PRINT *, "For IGR=", IGR
       ELSEIF(SADD(IGR,18,IDIL)/SADD(IGR,1,IDIL).LT.180000000.0) THEN
        PRINT *, "WARNING: KappaF =", SADD(IGR,18,IDIL)/SADD(IGR,1,IDIL)
        PRINT *, "For IDIL=", IDIL
        PRINT *, "For IGR=", IGR
       ENDIF
   ...
   85   CONTINUE
      ELSE
        ...
        ELSE
          MDIL=MDIL+1
          IPDIL(MDIL)=IDIL
          PRINT *, "LIBEXT: associate IPDIL(MDIL), MDIL=", MDIL
          PRINT *, "WITH IDIL=", IDIL
        ENDIF
   90   CONTINUE
      ENDIF
      IPDIL(MDIL+1)=NDIL+1
