...
SUBROUTINE SALEND(GG)
  ...
  !----
  DEALLOCATE(GG%IPAR,GG%RPAR,GG%VOL_NODE,GG%MED,STAT =OK)
  ...
  IF(ASSOCIATED(GG%IBC2_ELEM)) THEN
    DEALLOCATE(GG%IBC2_ELEM,GG%ISURF2_ELEM,GG%PPERIM_NODE,GG%TYPE_BC2,GG%IDATA_BC2, &
    & GG%PERIM_MAC2,GG%PERIM_NODE,STAT =OK)
    IF(OK /= 0) CALL XABORT('SALEND: failure to deallocate GG members(2)')
  ENDIF
  IF(ASSOCIATED(GG%IBC2_SURF2))  THEN
     DEALLOCATE(GG%IBC2_SURF2,GG%IELEM_SURF2,GG%SURF2,STAT =OK)
     IF(OK /= 0) CALL XABORT('SALEND: failure to deallocate GG surf members')
  ENDIF
  IF(ASSOCIATED(GG%BCDATAREAD)) THEN
    DO ELEM=1,GG%NBBCDA
