...
*  LOCAL VARIABLES
*----
      INTEGER NSTATE,IOUT,IMPX
      PARAMETER (NSTATE=40,IOUT=6,IMPX=1)
...
*----
      ...
      REAL, ALLOCATABLE, DIMENSION(:) :: DENWRK,GA1,ENER
...
*----
      ...
      IF((ILONG.GE.2).AND.(ILONG.LE.6)) THEN
        IF(ITSTMP.EQ.0) THEN
          CALL XABORT('LIBEST: XS PERTURBATION FORBIDDEN.')
...
*----
      ...
      IF((HPRT1.NE.'B').AND.(HPRT1.NE.'C').AND.(HPRT1.NE.'P')) THEN
         CALL XABORT('LIBEST: INVALID PARTICLE TYPE. B, C OR P EXPECTE'
     1   //'D.')
      ENDIF
...
*----
      ...
      IF((CV(:3).EQ.'BST').OR.(CV(:3).EQ.'CST')
     1   .OR.(CV(:3).EQ.'PST')) THEN
         ICV=0
...
*----
      IF(EXIST) THEN
         ...
         ENDDO
      ENDIF
*  RECOVER MIXTURES DENSITIES IF THEY ARE NOT ALREADY AVAILABLE.
*----
      CALL LCMLEN(IPLIB,'MIXTURESDENS',ILONG,ITYLCM)
      IF(ILONG.EQ.0) THEN
         ALLOCATE(DENWRK(NBISO))
         DENWRK(:NBISO)=DEN(:NBISO)
         DO IBM=1,NBMIX
            CALL LIBCON(IPLIB,IBM,NBISO,MIX,DENWRK,DENMIX(IBM),2,IMPX)
         ENDDO
