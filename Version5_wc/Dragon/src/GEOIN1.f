...
*  LOCAL VARIABLES
*----
      PARAMETER (MAXCOD=21,MAXHEX=9,MAXTEX=4,MAXTUR=12,MAXTYP=31,
     1 MXCL=500,MAXNOD=20,NSTATE=40,IOUT=6)
...
*----
     ...
     1 CELL,MIX,ISECT,IMESH,MERGE,ITURN,NS,MILIE,MIXDL,MIXGR,ITNODE,
     2 IREMIX
      REAL, ALLOCATABLE, DIMENSION(:) :: ZCODE,MESH,CYL,CENT,XR0,RR0,
     1 ANG,ARPIN,RS,FRACT,POURC,PROCE
      REAL, ALLOCATABLE, DIMENSION(:,:) :: NODE,ZNODE
*----
*  Data
*----
...
*       MODIFY AN EXISTING GEOMETRY.
     ...
     3       (ISTATE(1).EQ.26) .OR. (ISTATE(1) .EQ.27)
      ...
      ENDIF
*
   30 CALL REDGET(ITYPL,INTLIR,REALIR,CARLIR,DBLLIR)
      IF(ITYPL.NE.3) CALL XABORT('GEOIN1: CHARACTER DATA EXPECTED(1).')
      ...
      ELSE IF(CARLIR.EQ.'CAR1D') THEN
        ISTATE(1)=2
        CALL REDGET(ITYPL,LX,REALIR,CARLIR,DBLLIR)
        IF(ITYPL.NE.1) CALL XABORT('GEOIN1: INTEGER DATA EXPECTED.')
        LREG=LX
      ELSE IF(CARLIR.EQ.'SPHERE') THEN
        ISTATE(1)=4
        CALL REDGET(ITYPL,LR,REALIR,CARLIR,DBLLIR)
        IF(ITYPL.NE.1) CALL XABORT('GEOIN1: INTEGER DATA EXPECTED.')
        LREG=LR
      ELSE IF(CARLIR.EQ.'CAR2D') THEN
        ...
        CALL REDGET(ITYPL,LX,REALIR,CARLIR,DBLLIR)
        IF(ITYPL.NE.1) CALL XABORT('GEOIN1: INTEGER DATA EXPECTED.')
        CALL REDGET(ITYPL,LY,REALIR,CARLIR,DBLLIR)
        IF(ITYPL.NE.1) CALL XABORT('GEOIN1: INTEGER DATA EXPECTED.')
        LREG=LX*LY
      ELSE IF(CARLIR.EQ.'CAR3D') THEN
        ...
        CALL REDGET(ITYPL,LY,REALIR,CARLIR,DBLLIR)
        IF(ITYPL.NE.1) CALL XABORT('GEOIN1: INTEGER DATA EXPECTED.')
        CALL REDGET(ITYPL,LZ,REALIR,CARLIR,DBLLIR)
        IF(ITYPL.NE.1) CALL XABORT('GEOIN1: INTEGER DATA EXPECTED.')
        LREG=LX*LY*LZ
      ...
      ELSE IF(CARLIR.EQ.'HEXT') THEN
        ...
        ISTATE(1)=12
        LHEX=.TRUE.
        CALL REDGET(ITYPL,INTLIR,REALIR,CARLIR,DBLLIR)
        IF(ITYPL.NE.1) CALL XABORT('GEOIN1: INTEGER DATA EXPECTED.')
        NRINGH=MAX(1,INTLIR)
        LX=NRINGH
      ...
      ELSE IF(CARLIR(1:4).EQ.'TUBE') THEN
        ...
        IF(DIR.EQ.' ') THEN
          ...
          ELSE
            LX=INTLIR
            CALL REDGET(ITYPL,LY,REALIR,CARLIR,DBLLIR)
            IF(ITYPL.NE.1) CALL XABORT('GEOIN1: INTEGER DATA EXPECTE'
     1      //'D.')
          ENDIF
          LREG=LR*LY*LX
     ...
     1      //'D.')
      ...
      ELSE IF(CARLIR(1:7).EQ.'HEXTCEL') THEN
        ...
        LHEX=.TRUE.
        CALL REDGET(ITYPL,INTLIR,REALIR,CARLIR,DBLLIR)
        IF(ITYPL.NE.1) CALL XABORT('GEOIN1'//
...
*       DO-IT-YOURSELF OPTION FOR THE INTERFACE CURRENT METHOD.
        ISTATE(1)=30
        CALL REDGET(ITYPL,LX,REALIR,CARLIR,DBLLIR)
        IF(ITYPL.NE.1) CALL XABORT('GEOIN1: INTEGER DATA EXPECTED.')
        LREG=LX
      ELSE IF(CARLIR.EQ.'MACRO') THEN
*       DO-IT-YOURSELF OPTION FOR MACRO-GEOMETRIES.
        ISTATE(1)=31
        CALL REDGET(ITYPL,LX,REALIR,CARLIR,DBLLIR)
        IF(ITYPL.NE.1) CALL XABORT('GEOIN1: INTEGER DATA EXPECTED.')
        LREG=LX
...
*       DIRECTORY OF THE LCM OBJECT POINTED BY IPLIST.
     ...
     3       (ISTATE(1).EQ.26) .OR. (ISTATE(1) .EQ.27)
      ...
      ENDIF
*
   50 CALL REDGET(ITYPL,INTLIR,REALIR,CARLIR,DBLLIR)
      IF(ITYPL.NE.3) CALL XABORT('GEOIN1: CHARACTER DATA EXPECTED(2).')
   60 IF(CARLIR.EQ.'EDIT') THEN
        CALL REDGET(ITYPL,IMPX,REALIR,CARLIR,DREALIR)
        IF(ITYPL.NE.1) CALL XABORT('GEOIN1: INTEGER DATA EXPECTED.')
...
*       LOCATIONS.
   ...
   90   CONTINUE
     ...
     1       //'ER OF TIMES.')
           ...
           I=JREP+1
           CALL REDGET(ITYPL,INTLIR,REALIR,CARLIR,DBLLIR)
           IF(ITYPL.NE.3) CALL XABORT('GEOIN1: CHARACTER DATA EXPECTE'
     1     //'D(3).')
        ENDIF
...
*       INPUT MERGED MIXTURE NUMBERS for homogenization by geometry.
  ...
  550   CONTINUE
     ...
     1      //'r of times')
          ...
          I=JREP+1
          CALL REDGET(ITYPL,INTLIR,REALIR,CARLIR,DBLLIR)
          IF(ITYPL.NE.3) CALL XABORT('GEOIN1: CHARACTER DATA EXPECTED'
     1    //'(4).')
        ENDIF
...
*       INPUT CARTESIAN COORDINATES.
        ...
        ALLOCATE(MESH(LMESH))
        DO 100 I=1,LMESH
        CALL REDGET(ITYPL,INTLIR,MESH(I),TEXT12,DBLLIR)
        IF(ITYPL.NE.2) THEN
...
*       INPUT TUBE RADIUS.
        ...
        ALLOCATE(CYL(LCYL))
        DO 110 I=1,LCYL
        CALL REDGET(ITYPL,INTLIR,CYL(I),CARLIR,DBLLIR)
        IF(ITYPL.NE.2) CALL XABORT('GEOIN1: REAL DATA EXPECTED(2).')
        IF(I.GT.1) THEN
...
*       INPUT TUBE CENTER LOCATION (USE FOR CARCEL* ONLY).
        ...
        CENT(:3)=0.0
        DO 120 I=1,3
          CALL REDGET(ITYPL,INTLIR,CENT(I),CARLIR,DBLLIR)
          IF(ITYPL.NE.2) GO TO 130
  120   CONTINUE
        CALL REDGET(ITYPL,INTLIR,CENT(I),CARLIR,DBLLIR)
...
*        INPUT THE SIDE LENGTH IN TRIANGULAR OR HEXAGONAL GEOMETRY.
     ...
     1   //'HIBITED.')
         CALL REDGET(ITYPL,INTLIR,REALIR,CARLIR,DBLLIR)
         IF(ITYPL.NE.2) CALL XABORT('GEOIN1: REAL DATA EXPECTED(3).')
         SIDE=REALIR
...
*       INPUT MESH SPLITTING FACTORS.
        ...
        DO 140 I=1,LMESH
        CALL REDGET(ITYPL,IMESH(I),REALIR,TEXT12,DBLLIR)
        IF(ITYPL.NE.1) CALL XABORT('GEOIN1: INTEGER DATA EXPECTED.')
  ...
  140   CONTINUE
      ...
      ELSE IF(CARLIR.EQ.'HTMERGE') THEN
        CALL REDGET(ITYPL,INTLIR,REALIR,CARLIR,DBLLIR)
        IF(ITYPL.NE.1) CALL XABORT('GEOIN1: HTMERGE LEVEL EXPECTED.')
        IF((INTLIR.LT.0).OR.(INTLIR.GT.2)) INTLIR=0
        MRGHT=INTLIR
      ELSE IF(CARLIR.EQ.'MERGE') THEN
*       INPUT CELL-MERGING ITYPLES.
        ISTATE(10)=1
        ALLOCATE(MERGE(LREG))
        I=0
  150   I=I+1
        CALL REDGET(ITYPL,INTLIR,REALIR,CARLIR,DBLLIR)
        IF(ITYPL.EQ.3) GO TO 160
        IF(ITYPL.NE.1) CALL XABORT('GEOIN1: INTEGER DATA EXPECTED.')
...
*         INPUT DENSITY OF PIN IN CLUSTER.
          CALL REDGET(ITYPL,INTLIR,REALIR,CARLIR,DBLLIR)
          IF(ITYPL.NE.2) CALL XABORT('GEOIN1: REAL DATA EXPECTED FOR '
     1    //NAMT//' KEYWORD.')
          NPIN=-1
          DPIN=REALIR
          ISTATE(18)=-1
...
*         INPUT RADIUS/ANGLE OF CLUSTER RING.
          CALL REDGET(ITYPL,INTLIR,REALIR,CARLIR,DBLLIR)
          IF(ITYPL.NE.2) CALL XABORT('GEOIN1: REAL DATA EXPECTED(7).')
          ARPIN(1)=REALIR
          CALL REDGET(ITYPL,INTLIR,REALIR,CARLIR,DBLLIR)
          IF(ITYPL.EQ.3) THEN
     ...
     1    //'mitted.')
          ...
          DO IPIN=3,NPIN
            CALL REDGET(ITYPL,INTLIR,REALIR,CARLIR,DBLLIR)
            IF(ITYPL.NE.2) CALL XABORT('GEOIN1: REAL DATA EXPECTED('
     1       //'8).')
            ARPIN(IPIN)=REALIR
          ENDDO
...
*         INPUT Cartesian positions of pins.
          DO IPIN=0,NPIN-1
            CALL REDGET(ITYPL,INTLIR,REALIR,CARLIR,DBLLIR)
            IF(ITYPL.NE.2) CALL XABORT('GEOIN1: REAL CPIN DATA EXPECT'
     1      //'ED.')
            ARPIN(IPIN+1)=REALIR
          ENDDO
          CALL LCMPUT(IPLIST,NAMT,NPIN,2,ARPIN)
...
*       DOUBLE HETEROGENEITY OPTION.
     ...
     1  //'7).')
        ...
        ENDIF
        CALL REDGET(ITYPL,NG,REALIR,CARLIR,DBLLIR)
        IF(ITYPL.NE.1) CALL XABORT('GEOIN1: INTEGER DATA EXPECTED.')
        CALL REDGET(ITYPL,NMILG,REALIR,CARLIR,DBLLIR)
        IF(ITYPL.NE.1) CALL XABORT('GEOIN1: INTEGER DATA EXPECTED.')
*
        ALLOCATE(NS(NG))
        NSMAX=0
        DO 210 I=1,NG
        CALL REDGET(ITYPL,NS(I),REALIR,CARLIR,DBLLIR)
        IF(ITYPL.NE.1) CALL XABORT('GEOIN1: INTEGER DATA EXPECTED.')
...
*
  ...
  220   CONTINUE
        ...
        DO 230 J=1,NS(I)+1
        IOFJ=(I-1)*(NSMAX+1)+J
        CALL REDGET(ITYPL,INTLIR,RS(IOFJ),CARLIR,DBLLIR)
        IF(ITYPL.NE.2) CALL XABORT('GEOIN1: REAL DATA EXPECTED(9).')
  230   CONTINUE
  235   CONTINUE
...
*
        ...
        DO 240 I=1,NMILG
        CALL REDGET(ITYPL,MILIE(I),REALIR,CARLIR,DBLLIR)
        IF(ITYPL.NE.1) CALL XABORT('GEOIN1: INTEGER DATA EXPECTED.')
  240   CONTINUE
...
*
        ...
        DO 250 I=1,NMILG
        CALL REDGET(ITYPL,MIXDL(I),REALIR,CARLIR,DBLLIR)
        IF(ITYPL.NE.1) CALL XABORT('GEOIN1: INTEGER DATA EXPECTED.')
  250   CONTINUE
...
*
  ...
  260   CONTINUE
        ...
        DO 270 J=1,NG
        IOFJ=(I-1)*NG+J
        CALL REDGET(ITYPL,INTLIR,FRACT(IOFJ),CARLIR,DBLLIR)
        IF(ITYPL.NE.2) CALL XABORT('GEOIN1: REAL DATA EXPECTED(10).')
  270   CONTINUE
        DO 290 J=1,NG
        IOFJ=(I-1)*NG+J
        IF(FRACT(IOFJ).GT.0.0) THEN
          ...
          CALL REDGET(ITYPL,MIXGR(IOFK),REALIR,CARLIR,DBLLIR)
          IF(ITYPL.NE.1) CALL XABORT('GEOIN1: INTEGER DATA EXPECTED.')
  280     CONTINUE
        ENDIF
  290   CONTINUE
...
*       CELL PROPORTIONS FOR DO-IT-YOURSELF OPTION.
     ...
     1  //'MITED TO DO-IT-YOURSELF GEOMETRY WITH IC.')
        ALLOCATE(POURC(LX))
        DO 310 I=1,LX
        CALL REDGET(ITYPL,INTLIR,POURC(I),CARLIR,DBLLIR)
        IF(ITYPL.NE.2) CALL XABORT('GEOIN1: REAL DATA EXPECTED(11).')
  310   CONTINUE
...
*       CELL PROBABILITIES FOR DO-IT-YOURSELF OPTION.
     ...
     1  //'MITED TO DO-IT-YOURSELF GEOMETRY WITH IC.')
        ...
        DO 320 J=1,LX
        IOFJ=(J-1)*LX+I
        CALL REDGET(ITYPL,INTLIR,PROCE(IOFJ),CARLIR,DBLLIR)
        IF(ITYPL.NE.2) CALL XABORT('GEOIN1: REAL DATA EXPECTED(12).')
  320   CONTINUE
  325   CONTINUE
        CALL LCMPUT(IPLIST,'PROCEL',LX*LX,2,PROCE)
        DEALLOCATE(PROCE)
      ELSE IF(CARLIR.EQ.'REGIONS') THEN
*       DEFINE DO-IT-YOURSELF MACRO-REGIONS.
        IF(ISTATE(1).NE.31) CALL XABORT('GEOIN1: REGIONS - KEYWORD LI'
     1  //'MITED TO DO-IT-YOURSELF MACRO-GEOMETRY.')
        CALL LCMSIX(IPLIST,'MACRO-GEOM',1)
        ALLOCATE(NODE(MAXNOD,LX),ZNODE(2,LX),ITNODE(LX))
        CALL REDGET(ITYPL,INTLIR,REALIR,CARLIR,DBLLIR)
        IF(ITYPL.NE.3) CALL XABORT('GEOIN1: CHARACTER DATA EXPECTED('
     1  //'8).')
        JSTATE(:NSTATE)=0
        ...
        DO IM=1,LX
          ...
          IF(CARLIR.EQ.'RECT') THEN
            ITNODE(IM)=1
            DO I=1,4
              CALL REDGET(ITYPL,INTLIR,NODE(I,IM),CARLIR,DBLLIR)
              IF(ITYPL.NE.2) CALL XABORT('GEOIN1: REAL DATA EXPECTED'
     1        //'(13).')
            ENDDO
     ...
     1        //'16).')
            ENDDO
          ELSE IF(CARLIR.EQ.'POLY') THEN
            CALL REDGET(ITYPL,ITNODE(IM),REALIR,CARLIR,DBLLIR)
            IF(ITYPL.NE.1) CALL XABORT('GEODIN: INTEGER DATA EXPECTED.')
            IF(ITNODE(IM).LT.3) THEN
              CALL XABORT('GEOIN1: NCORN SHOULD BE >= 3.')
            ELSE IF(2*ITNODE(IM).GT.MAXNOD) THEN
              CALL XABORT('GEOIN1: MAXNOD OVERFLOW.')
            ENDIF
            DO I=1,2*ITNODE(IM)
              CALL REDGET(ITYPL,INTLIR,NODE(I,IM),CARLIR,DBLLIR)
              IF(ITYPL.NE.2) CALL XABORT('GEOIN1: REAL DATA EXPECTED('
     1        //'17).')
            ENDDO
          ELSE
            CALL XABORT('GEOIN1: *RECT*, *TRIA*, *QUAD*, *HEXA* OR *PO'
     1      //'LY* KEYWORD EXPECTED.')
          ENDIF
          CALL REDGET(ITYPL,INTLIR,REALIR,CARLIR,DBLLIR)
          IF(ITYPL.NE.3) CALL XABORT('GEOIN1: CHARACTER DATA EXPECTED'
     1    //'(9).')
          ...
          IF(CARLIR.EQ.'AXIAL') THEN
            JSTATE(5)=3
            CALL REDGET(ITYPL,INTLIR,ZNODE(1,IM),CARLIR,DBLLIR)
            IF(ITYPL.NE.2) CALL XABORT('GEOIN1: REAL DATA EXPECTED('
     1      //'18).')
            CALL REDGET(ITYPL,INTLIR,ZNODE(2,IM),CARLIR,DBLLIR)
            IF(ITYPL.NE.2) CALL XABORT('GEOIN1: REAL DATA EXPECTED('
     1      //'19).')
            CALL REDGET(ITYPL,INTLIR,REALIR,CARLIR,DBLLIR)
            IF(ITYPL.NE.3) CALL XABORT('GEOIN1: CHARACTER DATA EXPECT'
     1      //'ED(10).')
          ENDIF
        ENDDO
        CALL LCMPUT(IPLIST,'NODE',MAXNOD*LX,2,NODE)
        IF(JSTATE(5).EQ.3) CALL LCMPUT(IPLIST,'ZNODE',2*LX,2,ZNODE)
        CALL LCMPUT(IPLIST,'ITNODE',LX,1,ITNODE)
        DEALLOCATE(ITNODE,ZNODE,NODE)
*
        IF(CARLIR.EQ.'CENTER') THEN
          CALL REDGET(ITYPL,INTLIR,CENTER(1),CARLIR,DBLLIR)
          IF(ITYPL.NE.2) CALL XABORT('GEOIN1: REAL DATA EXPECTED('
     1    //'20).')
          CALL REDGET(ITYPL,INTLIR,CENTER(2),CARLIR,DBLLIR)
          IF(ITYPL.NE.2) CALL XABORT('GEOIN1: REAL DATA EXPECTED('
     1    //'21).')
          CALL REDGET(ITYPL,INTLIR,REALIR,CARLIR,DBLLIR)
          IF(ITYPL.NE.3) CALL XABORT('GEOIN1: CHARACTER DATA EXPECTED'
     1    //'(11).')
          IF(CARLIR.EQ.'NONE') THEN
            JSTATE(3)=1
          ELSE IF(CARLIR.EQ.'REFL') THEN
            JSTATE(3)=2
          ELSE IF(CARLIR.EQ.'ROTA') THEN
            JSTATE(3)=3
          ELSE
            CALL XABORT('GEOIN1: NONE, REFL OR ROTA KEYWORD EXPECTED.')
          ENDIF
          CALL LCMPUT(IPLIST,'CENTER',2,2,CENTER)
          CALL REDGET(ITYPL,INTLIR,REALIR,CARLIR,DBLLIR)
          IF(ITYPL.NE.3) CALL XABORT('GEOIN1: CHARACTER DATA EXPECTED'
     1    //'(12).')
        ENDIF
*
        IF(CARLIR.EQ.'REMIX') THEN
          ALLOCATE(IREMIX(LX))
          DO II=1,LX
            CALL REDGET(ITYPL,IREMIX(II),REALIR,CARLIR,DBLLIR)
            IF(ITYPL.NE.1) CALL XABORT('GEOIN1: INTEGER DATA EXPECTED'
     1      //'.')
          ENDDO
          CALL LCMPUT(IPLIST,'IREMIX',LX,1,IREMIX)
          JSTATE(4)=MAXVAL(IREMIX)
          DEALLOCATE(IREMIX)
          CALL REDGET(ITYPL,INTLIR,REALIR,CARLIR,DBLLIR)
          IF(ITYPL.NE.3) CALL XABORT('GEOIN1: CHARACTER DATA EXPECTED'
     1    //'(13).')
...
*       INPUT BOUNDARY CONDITIONS.
  ...
  360     CALL LCMPUT(IPLIST,'ITRI',1,1,ITRI)
        ...
        ENDIF
        CALL REDGET(ITYPL,INTLIR,REALIR,TEXT4,DBLLIR)
        IF(ITYPL.NE.3) CALL XABORT('GEOIN1: CHARACTER DATA EXPECTED('
     1  //'16).')
        DO 370 I=1,MAXCOD
...
*       INPUT A SUB GEOMETRY.
     ...
     1  //'17).')
        CALL REDGET(ITYPL,INTLIR,REALIR,TEXT4,DBLLIR)
        IF((ITYPL.NE.3).OR.(TEXT4.NE.':=')) CALL XABORT('GEOIN1: := TO'
     1  //'KEN EXPECTED.')
        CALL REDGET(ITYPL,INTLIR,REALIR,TEXT4,DBLLIR)
        IF((ITYPL.NE.3).OR.(TEXT4.NE.'GEO:')) THEN
          WRITE(HSMG,'(36HGEOIN1: GEO: TOKEN EXPECTED (CARLIR=,A,
     1    7H ITYPL=,I2,7H TEXT4=,A,1H))') CARLIR,ITYPL,TEXT4
          CALL XABORT(HSMG)
        ENDIF
        CALL LCMLEN(IPLIST,CARLIR,ILONG,ITYX)
        IF(ILONG.NE.0) THEN
  ...
  410 CARLIR='L_GEOM'
     ...
     >  CALL XABORT('GEOIN1: ALBEDO NUMBER MUST BE GREATER THAN 0')
      ...
      IF(IMPX.GT.0) THEN
        ...
        WRITE (IOUT,510) LEVEL,GEONAM,CARLIR,TRIM(TYPE(ISTATE(1)))
      ENDIF
      IF(IMPX.GT.1) THEN
...
*
  ...
  510 FORMAT(/20H CREATION OF A LEVEL,I3,27H GEOMETRY ON THE DIRECTORY ,
     1 7HNAMED ',A12,21H' OF THE LCM OBJECT ',A12,12H' WITH TYPE ,A,
     2 1H.)
