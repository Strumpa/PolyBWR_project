...
SUBROUTINE SALMUS(FGEO ,IPTRK, IFTRK, RCUTOF, IPRINT, NBSLIN, GG)
  ...
  INTEGER      ISTATT(NSTATE),I,J,I1,I2,IMACRO,NCOUR,NMIX,NREG,OK
  REAL         RSTATT(NSTATE)
  LOGICAL      LGINF
  !----
  !  Allocatable arrays
  ...
  & NMC_NODE,NMC_SURF,IFR,MIX,ICNT,SURF_MACRO
  REAL, ALLOCATABLE, DIMENSION(:) :: VOLUME,ALB,DVX
  !----
  ...
  NMC_NODE(1)=0
  NMC_SURF(1)=0
     print *,'SALMUS: aa NMACRO=',GG%NB_MACRO
  ...
  ISTATT(28)=NMC_SURF(GG%NB_MACRO+1) ! number of current unknowns
  CALL LCMPUT(IPTRK,'STATE-VECTOR',NSTATE,1,ISTATT)
  !----
  !  Create connectivity data
  !----
  NMIX=NMC_SURF(GG%NB_MACRO+1)
  ALLOCATE(IFR(NMIX),ALB(NMIX),MIX(NMIX),DVX(NMIX),ICNT(NMIX))
  NMIX=0
  DO IMACRO=1,GG%NB_MACRO
    KPTRK=LCMDIL(JPTRK,IMACRO)
    NCOUR=NSURF_MACRO(IMACRO)
    ALLOCATE(SURF_MACRO(NCOUR))
    CALL LCMSIX(KPTRK,'GEOMETRY  ',1)
    CALL LCMGET(KPTRK,'SURF_MACRO',SURF_MACRO)
    CALL LCMSIX(KPTRK,'          ',2)
    ICNT(NMIX+1:NMIX+NCOUR)=SURF_MACRO(:NCOUR)
    DEALLOCATE(SURF_MACRO)
    MIX(NMIX+1:NMIX+NCOUR)=(/ (I, I=NMIX+1,NMIX+NCOUR) /)
    ALB(NMIX+1:NMIX+NCOUR)=1.0
    DVX(NMIX+1:NMIX+NCOUR)=1.0
    NMIX=NMIX+NCOUR
  ENDDO
  DO I=1,NMIX
    DO J=1,NMIX
  ...
  IF(IPRINT .GT. 1) THEN
    ...
    DO IMACRO=1,GG%NB_MACRO
      NCOUR=NSURF_MACRO(IMACRO)
      WRITE (IOUT,50) (ICNT(I),I=NMIX+1,NMIX+NCOUR)
      WRITE (IOUT,60) (MIX(I),IFR(I),I=NMIX+1,NMIX+NCOUR)
      WRITE (IOUT,70) (ALB(I),I=NMIX+1,NMIX+NCOUR)
      WRITE (IOUT,80) (DVX(I),I=NMIX+1,NMIX+NCOUR)
      WRITE (IOUT,90) ('----------------',I=1,MIN(8,NCOUR))
      NMIX=NMIX+NCOUR
    ENDDO
  ENDIF
  ...
  !  Formats
  !----
   20 FORMAT (//11H REGION    ,8(I8,6X,1HI))
   30 FORMAT (  11H MIXTURE   ,8(I8,6X,1HI))
   40 FORMAT (  11H VOLUME    ,8(1P,E13.6,2H I))
   50 FORMAT (/9H ELEMENT:,8(3H  S,I6.6,6X,1HI,:)/(9X,8(3H  S,I6.6,6X,1HI,:)))
