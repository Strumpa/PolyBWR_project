...
SUBROUTINE SALMUS(FGEO ,IPTRK, IFTRK, RCUTOF, IPRINT, NBSLIN, GG)
  ...
  INTEGER      ISTATT(NSTATE),I,J,I1,I2,J1,J2,IMACRO,J3,NMIX,IMIX,NREG,OK, &
               & NINST,NBCDA,ILONG,ITYLCM,IMERGE,MMAX
  REAL         RSTATT(NSTATE)
  LOGICAL      LGINF
  ...
  !  Allocatable arrays
  !----
  INTEGER, ALLOCATABLE, DIMENSION(:) :: NSURF_MACRO,NBFLUX_MACRO,NBNODE_MACRO,ITAB, &
  & NMC_NODE,NMC_FLUX,NMC_SURF,IFR,MIX,ICNT,MERGE_MACRO,SURF_MACRO,PERIM,IDMA,IMAC
  REAL, ALLOCATABLE, DIMENSION(:) :: VOLUME,ALB,DVX,SUR,GALBED
  REAL(PDB), ALLOCATABLE, DIMENSION(:) :: SURF2
  INTEGER, POINTER, DIMENSION(:) :: NEW_MACRO
  !----
  ...
  !----
  IF(LMERGM) THEN
    ALLOCATE(MERGE_MACRO(GG%NB_NODE))
    MERGE_MACRO(:GG%NB_NODE)=0
    IMERGE=0
    DO IMACRO=1,GG%NB_MACRO
      DO J=1,GG%NB_NODE
        IF(GG%NUM_MACRO(GG%NUM_MERGE(J))==IMACRO) MERGE_MACRO(J)=IMERGE+GG%MED(J)
      ENDDO
      MMAX=MAXVAL(MERGE_MACRO(:))
      DO I=1,MMAX
        1000 IF(I>MAXVAL(MERGE_MACRO(:))) EXIT
        DO J=1,GG%NB_NODE
          IF(MERGE_MACRO(J).EQ.I) GO TO 2000
        ENDDO
        DO J=1,GG%NB_NODE
          IF(MERGE_MACRO(J)>=I) MERGE_MACRO(J)=MERGE_MACRO(J)-1
        ENDDO
        GO TO 1000
        2000 CONTINUE
      ENDDO
      IMERGE=MAXVAL(MERGE_MACRO(:))
    ENDDO
    ALLOCATE(NEW_MACRO(IMERGE))
    DO J=1,GG%NB_NODE
      NEW_MACRO(MERGE_MACRO(J))=GG%NUM_MACRO(GG%NUM_MERGE(J))
    ENDDO
    DEALLOCATE(GG%NUM_MACRO)
    GG%NUM_MACRO=>NEW_MACRO
    GG%NUM_MERGE(:)=MERGE_MACRO(:)
    DEALLOCATE(MERGE_MACRO)
    IF(IPRINT>0) WRITE(IOUT,'(/40h SALMUS: MERGMIX reduces GG%NB_FLUX from,i5, &
  ...
  ! perform tracking
  !----
  ALLOCATE(NSURF_MACRO(GG%NB_MACRO),NBFLUX_MACRO(GG%NB_MACRO),NBNODE_MACRO(GG%NB_MACRO))
  ALLOCATE(NMC_NODE(GG%NB_MACRO+1),NMC_FLUX(GG%NB_MACRO+1),NMC_SURF(GG%NB_MACRO+1))
  NSURF_MACRO(:GG%NB_MACRO) = 0
  NBFLUX_MACRO(:GG%NB_MACRO) = 0
  ...
  DO IMACRO=1,GG%NB_MACRO
    ...
    & NBNODE_MACRO(IMACRO),NBFLUX_MACRO(IMACRO),NSURF_MACRO(IMACRO))
    NMC_NODE(IMACRO+1)=NMC_NODE(IMACRO)+NBNODE_MACRO(IMACRO)
    NMC_FLUX(IMACRO+1)=NMC_FLUX(IMACRO)+NBFLUX_MACRO(IMACRO)
    NMC_SURF(IMACRO+1)=NMC_SURF(IMACRO)+NSURF_MACRO(IMACRO)
  ENDDO
  CALL LCMPUT(IPTRK,'NMC_NODE',GG%NB_MACRO+1,1,NMC_NODE)
  CALL LCMPUT(IPTRK,'NMC_FLUX',GG%NB_MACRO+1,1,NMC_FLUX)
  ...
  IMIX=0
  DO IMACRO=1,GG%NB_MACRO
    J2=NBFLUX_MACRO(IMACRO)
    IF(J2.EQ.0) CYCLE
    J3=NSURF_MACRO(IMACRO)
    KPTRK=LCMDIL(JPTRK,IMACRO)
    ALLOCATE(MERGE_MACRO(J2),SURF_MACRO(J3),SURF2(J3))
    CALL LCMSIX(KPTRK,'SURFACIC_TMP',1)
      CALL LCMLEN(KPTRK,'GLOBAL_MERGE',ILONG,ITYLCM)
      IF(ILONG.NE.J2) CALL XABORT('SALMUS: wrong number of nodes')
      CALL LCMGET(KPTRK,'GLOBAL_MERGE',MERGE_MACRO)
    ...
    CALL LCMSIX(KPTRK,' ',2)
    IF(J1+J2>NREG) CALL XABORT('SALMUS: NREG overflow')
    ...
    NMIX=NMIX+J3
  ENDDO
  IF(J1/=NREG) CALL XABORT('SALMUS: inconsistent NREG')
  CALL LCMPUT(IPTRK,'GLOBAL_MERGE',NREG,1,IMAC)
  DEALLOCATE(IMAC)
  OUT2: DO I=1,NMIX
    DO J=1,NMIX
  ...
  DEALLOCATE(NMC_SURF,NMC_FLUX,NMC_NODE)
  DEALLOCATE(NBNODE_MACRO,NBFLUX_MACRO,NSURF_MACRO)
  !----
  !  Formats
  !----
