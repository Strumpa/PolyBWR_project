...
*----
     ...
     >  SBISO2(NBISO2),GIR2(NBISO2),DENMIX2(MAXMIX2),ENERGY(NGRO+1))
        ALLOCATE(MASKI2(NBISO2))
        ALLOCATE(JPISO2(NBISO2))
        CALL LCMGET(IPLIBX,'ENERGY',ENERGY)
        CALL LCMPUT(IPLIB,'ENERGY',NGRO+1,2,ENERGY)
        CALL LIBINF(IPLIBX,NBISO2,MAXLIB,MAXED,MAXMIX2,NBISO2,NGRO,NL2,
     1  NW2,ITRANC,NLIB2,NCOMB2,NEDMAC2,NBMIX2,ISONAM2,ISONRF2,ISOMIX2,
   ...
   70   IF(ITYPLU.NE.3) CALL XABORT('LIBCTL: CHARACTER DATA EXPECTED.')
     ...
     >    //' BE GIVEN.')
          ...
          IF(ITYPLU.EQ.1) THEN
            ...
            ENDIF
            LOCUPD(NNMIX)=INTLIR
            DENMIX(NNMIX)=DENMIX2(INTLIR)
            CALL REDGET(ITYPLU,INTLIR,REALIR,CARLIR,DBLLIR)
            IF(ITYPLU.EQ.2) THEN
              IF(DENMIX(NNMIX).EQ.-1.0) THEN
                CALL LIBCON(IPLIBX,INTLIR,NBISO2,ISOMIX2,WORK,
     >          DENMIX(NNMIX),2,IMPX)
              ENDIF
              DENMIX(NNMIX)=REALIR
              CALL REDGET(ITYPLU,INTLIR,REALIR,CARLIR,DBLLIR)
            ENDIF
            GO TO 70
          ELSE IF((ITYPLU.EQ.3).AND.(CARLIR(1:4).EQ.'COMB')) THEN
*           THIS MIXTURE IS A COMBINATION OF OTHER MIXTURES.
            VOLTOT=0.0
            DENMIX(NNMIX)=-1.0
  75        VOLFRA=0.0
            MIXCMB=0
            CALL REDGET(ITYPLU,MIXCMB,REALIR,CARLIR,DBLLIR)
            IF(ITYPLU.EQ.3) THEN
               IF(VOLTOT.EQ.0.0) CALL XABORT('LIBCTL: TOTAL VOLUME F'
     >         //'RACTION OF 0.0 IS ILLEGAL.')
               GO TO 70
            ENDIF
            IF(ITYPLU.EQ.2) CALL XABORT('LIBCTL: MIXTURE NUMBER MISS'
     >      //'ING FOR COMBINATION.')
            LOCUPD(NNMIX)=LOCUPD(MIXCMB)
            CALL REDGET(ITYPLU,INTLIR,VOLFRA,CARLIR,DBLLIR)
            IF((ITYPLU.EQ.1).OR.(ITYPLU.EQ.3)) CALL XABORT('LIBCTL: '
     >      //'VOLUME FRACTION MISSING FOR COMBINATION.')
            IF(VOLFRA.EQ.0.0) CALL XABORT('LIBCTL: INDIVIDUAL VOLUME'
     ...
     3      IEVOL,ITYP,TMPNEW)
            GO TO 75
          ELSE IF((ITYPLU.EQ.3).AND.(CARLIR(1:4).EQ.'NOEV')) THEN
            KEVOL(NNMIX)=1
          ELSE IF((ITYPLU.EQ.3).AND.(CARLIR(1:4).EQ.'EVOL')) THEN
            KEVOL(NNMIX)=2
          ELSE
            CALL XABORT('LIBCTL: WRONG INPUT VALUE.')
          ENDIF
...
*----
      ...
      ENDDO
      IF(NBMIX.GT.MAXMIX) THEN
        CALL XABORT('LIBCTL: MAXMIX TOO SMALL.')
      ELSE IF(NBISO.EQ.0) THEN
        CALL XABORT('LIBCTL: NO ISOTOPES IN THE MICROLIB.')
      ENDIF
      TEXT12='L_LIBRARY'
...
*----
      ...
      DEALLOCATE(WORK,KEVOL,DENMIX,GIR,SBISO,SNISO,TMPISO,DENISO)
