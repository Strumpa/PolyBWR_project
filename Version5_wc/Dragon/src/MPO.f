...
*----
     ...
     1 MAXMAC=2,MAXREA=50,MAXCAD=153)
      TYPE(C_PTR) IPMPO,IPLB1,IPLB2,IPDEPL,IPEDIT,IPMICR
...
*----
      IF(C_ASSOCIATED(IPEDIT)) THEN
        CALL LCMGTC(IPEDIT,'LAST-EDIT',12,CDIRO)
        IPMICR=LCMGID(IPEDIT,CDIRO)
      ELSE IF(C_ASSOCIATED(IPLB1)) THEN
        IPMICR=IPLB1
      ELSE
        CALL XABORT('MPO: NO EDITION OBJECT OR MICROLIB DEFINED(1).')
      ENDIF
      CALL LCMSIX(IPMICR,'MACROLIB',1)
        ...
        CALL LCMGET(IPMICR,'ENERGY',ENERG)
        WRITE(RECNAM,'(23H/energymesh/energymesh_,I0,1H/)') NENERG-1
        ENERG(:NG+1)=ENERG(:NG+1)*1.0E-6
...
*----
  360 IF(IMPX.GT.0) WRITE(6,420) NCALS+1
      ...
      ELSE IF(C_ASSOCIATED(IPLB1)) THEN
        IPMICR=IPLB1
      ELSE
        CALL XABORT('MPO: NO EDITION OBJECT OR MICROLIB DEFINED(2).')
      ENDIF
*     -------------------------------------------
      CALL MPOCAL(IMPX,IPMPO,IPDEPL,IPMICR,HEDIT)
*     -------------------------------------------
...
*----
      ...
      CALL MPOGEP(IPMPO,IPDEPL,IPLB1,IPLB2,IPMICR,IPEDIT,HEDIT,IMPX,
     1 ITIM,NPAR,NLOC,MUPLET,LGNEW,NMIL,NG,NCALS)
