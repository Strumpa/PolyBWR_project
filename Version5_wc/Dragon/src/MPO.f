...
*----
      IF(IPICK.EQ.1) THEN
        CALL REDGET(ITYP,NITMA,FLOTT,TEXT24,DFLOTT)
        IF(ITYP.NE.-1) CALL XABORT('MPO: OUTPUT INTEGER EXPECTED.')
        ITYP=1
        CALL REDPUT(ITYP,NCALS,FLOTT,TEXT24,DFLOTT)
        CALL REDGET(ITYP,NITMA,FLOTT,TEXT24,DFLOTT)
        IF((ITYP.NE.3).OR.(TEXT24.NE.';')) THEN
          CALL XABORT('MPO: ; CHARACTER EXPECTED.')
        ENDIF
      ENDIF
      GO TO 390
*----
