...
*----
     ...
     2 KSPH,IPROB,MAT(NREG),KEYFLX(NREG),IGRMIN,IGRMAX,MAXX0,ITRANZ
      REAL DEN(NBISO),SN(NGRO,NBISO),VOL(NREG),EPS,TCEPS
      LOGICAL LEAKSW
...
*----
  ...
  200 CONTINUE
     ...
     2 VOL,KEYFLX,LEAKSW,TITR,START,SIGT2,SIGT3,NOCONV,TCEPS,ICPIJ,
     3 TK3,TK4)
  210 CONTINUE
      ZZMAX=0.0
      LNGRO=0
...
*----
  ...
  340    CONTINUE
     ...
     >   ITSTMP,TMPDAY)
      ...
      CALL KDRCPU(TK2)
      IF(IMPX.GT.1) THEN
        WRITE(6,'(/9H TONDRV: ,F8.1,18H SECOND TO PERFORM,
     ...
     3  25HFLUX SOLUTION DOOR CALLS.)') TK2-TK1,TK3,TK4,ICPIJ
        IF(TC_TOTAL.GT.0) THEN
          WRITE(6,'(9X,A,I7,1H/,I7,A,F5.1,A)')
     1    'PIJ/ARM CACHE: ',TC_HITS,TC_TOTAL,' GROUPS SKIPPED (',
     2    100.0*REAL(TC_HITS)/REAL(TC_TOTAL),'%)'
        ENDIF
      ENDIF
      CALL TC_RESET()
*----
...
*----
      ...
      CALL LCMPUT(IPLIB,'TCEPS-SHIBA',1,2,TCEPS)
      CALL LCMSIX(IPLIB,' ',2)
*----
