...
SUBROUTINE MUSP(IPTRK,IFTRAK,IMPX,NREGIO,NBMIX,MAT,VOL,SIGT0,SIGW0,NELPIJ, &
  ...
  !  ALLOCATABLE ARRAYS
  !----
  INTEGER, ALLOCATABLE, DIMENSION(:) :: MATALB,NMC_FLUX,NMC_SURF,MAT2,IGEN,INUM, &
    & IFR,MIX,IMAC
  ...
  ALLOCATE(NMC_FLUX(NMACRO+1),NMC_SURF(NMACRO+1))
  JPTRK=LCMGID(IPTRK,'MACRO-TRACK')
  CALL LCMGET(IPTRK,'NMC_FLUX',NMC_FLUX)
  ...
  NPSS=0
  DO IMACRO=1,NMACRO
    J2=NMC_FLUX(IMACRO+1)-NMC_FLUX(IMACRO)
    IF(J2.EQ.0) CYCLE
  ...
  CALL LCMGET(IPTRK,'GLOBAL_MERGE',IMAC)
  PP(:NREGIO,:NREGIO)=0.0
  IPIJ=0
  DO JKG=1,NGEN
    J2=NMC_FLUX(JKG+1)-NMC_FLUX(JKG)
    I1=0
    DO IKK=1,NMERGE
      IKG=IGEN(IKK)
      I2=NMC_FLUX(IKG+1)-NMC_FLUX(IKG)
      IF(IKG.EQ.JKG) THEN
        DO J=1,J2
          DO I=1,J2
  ...
  DO IKK=1,NMERGE
    ...
    IPIS=0
    DO IK=1,IKG-1
      IPIS=IPIS+(NMC_FLUX(IK+1)-NMC_FLUX(IK))*(NMC_SURF(IK+1)-NMC_SURF(IK))
    ENDDO
    DO I=1,I2
      DO IC=1,I3
        ICC=MIX(IT+IC)
        ...
        DO JKK=1,NMERGE
          ...
          IPSJ=0
          DO IK=1,JKG-1
            IPSJ=IPSJ+(NMC_FLUX(IK+1)-NMC_FLUX(IK))*(NMC_SURF(IK+1)-NMC_SURF(IK))
          ENDDO
          DO J=1,J2
            DO JC=1,J3
              JCC=MIX(JT+JC)
