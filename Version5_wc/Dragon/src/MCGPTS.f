*DECK MCGPTS
      SUBROUTINE MCGPTS(SUBPJJ,NFI,NREG,NBCDA,M,NANI,NFUNL,NANGL,NMU,
     1                  NMOD,LPS,NPJJM,NGEFF,IANGL,NSEG,ISGNR,NZON,
     2                  NOM2D,IS,JS,PJJIND,W2D,XMU,CAZ1,CAZ2,ZMU,WZMU,
     3                  SIGAL,T2D,PSJ,PJJD,LPJJAN,NR2D,NMAX,NZP,N2REG,
     4                  N2SOU,DELU,INDREG,Z,VNORF,CMU,CMUI,SMU,SMUI,
     5                  TMU,TMUI,SSYM)
*
*-----------------------------------------------------------------------
*
...
*----
      INTEGER NFI,NREG,NBCDA,M,NANI,NFUNL,NANGL,NMU,NMOD,LPS,NPJJM,
     1 NGEFF,IANGL,NSEG,ISGNR(NMOD,NFUNL),NZON(NFI),NOM2D(NSEG),JS(LPS),
     2 IS(NFI-NREG+1),PJJIND(NPJJM,2),NR2D,NMAX,NZP,N2REG,N2SOU,
     3 INDREG(-N2SOU:N2REG,0:NZP+1),SSYM
      REAL XMU(NMU),ZMU(NMU),WZMU(NMU),SIGAL(-NBCDA:M,NGEFF),
     1 PSJ(LPS,NGEFF),DELU,Z(0:NZP)
...
*        this track has encountered the top boundary -> it is reflected
     ...
     1        KANGL,TRHARP,H3D,ZMU,WZMU,NOM3D,NZON,NFI,NREG,3,NBCDA,M,
     2        IS,JS,PJJD,PSJ,LPJJAN,NPJJM,PJJIND,SIGAL,IMU,1)
         T=TP
...
*        ---
     ...
     1        TIN,CPOI,SPOI,TPO,TPOI,N3D,NOM3D,H3D)
         ...
         ENDDO
         CALL MCGDS4(SUBPJJ,N3D,NSUB,NMU,LPS,NFUNL,NANGL0,NGEFF,W3D,
     1        KANGL,TRHARM,H3D,ZMU,WZMU,NOM3D,NZON,NFI,NREG,3,NBCDA,M,
     2        IS,JS,PJJD,PSJ,LPJJAN,NPJJM,PJJIND,SIGAL,IMU,1)
*        ---
         T=TP
 10   CONTINUE
