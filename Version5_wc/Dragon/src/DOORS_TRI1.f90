SUBROUTINE DOORS_TRI1(IPTRK,NANIS,NREG,NMAT,NUN,MATCOD,VOL,SIGG,SUNKNO)
  ...
  IF((ITYPE.EQ.2).OR.(ITYPE.EQ.5).OR.(ITYPE.EQ.7)) THEN
    ...
    CALL TRI1GSO(IPTRK,NREG,NMAT,NUN,MATCOD,VOL,SIGG,SUNKNO)
  ...
  RETURN
CONTAINS
  SUBROUTINE TRI1GSO(IPTRK,NREG,NMAT,NUN,MATCOD,VOL,SIGG,SUNKNO)
    !
    !-----------------------------------------------------------------------
    !
    !Purpose:
    ...
    IF((ITYPE.NE.2).AND.(ITYPE.NE.5).AND.(ITYPE.NE.7)) THEN
      CALL XABORT('TRI1GSO: INVALID CARTESIAN GEOMETRY.')
    ELSE IF((IELEM.LT.0).OR.(ICOL.GT.3)) THEN
      CALL XABORT('TRI1GSO: RAVIART-THOMAS METHOD EXPECTED.')
    ELSE IF(ISCAT.GT.1) THEN
      WRITE(HSMG,'(54HTRI1GSO: MACRO-CALCULATION WITH ANISOTROPIC SCATTERING, &
      & 63H CURRENTLY NOT IMPLEMENTED; USE SCAT 1 KEYWORD IN TRIVAT: DATA.)')
      CALL XABORT(HSMG)
    ELSE IF(LX*LY*LZ.NE.NREG) THEN
      CALL XABORT('TRI1GSO: INVALID NREG.')
    ENDIF
  ...
  SUBROUTINE TRI1GSR(IPTRK,NREG,NMAT,NUN,MATCOD,SIGG,SUNKNO)
    ...
    IF((ITYPE.NE.8).AND.(ITYPE.NE.9)) THEN
      CALL XABORT('TRI1GSR: INVALID HEXAGONAL GEOMETRY.')
    ELSE IF((IELEM.LT.0).OR.(ICOL.GT.3)) THEN
      CALL XABORT('TRI1GSR: RAVIART-THOMAS METHOD EXPECTED.')
    ELSE IF(ISCAT.GT.1) THEN
      WRITE(HSMG,'(54HTRI1GSR: MACRO-CALCULATION WITH ANISOTROPIC SCATTERING, &
      & 63H CURRENTLY NOT IMPLEMENTED; USE SCAT 1 KEYWORD IN TRIVAT: DATA.)')
      CALL XABORT(HSMG)
    ELSE IF(3*NBLOS.NE.NREG) THEN
      CALL XABORT('TRI1GSR: INVALID NREG.')
    ENDIF
    CALL LCMLEN(IPTRK,'KN',MAXKN,ITYLCM)
