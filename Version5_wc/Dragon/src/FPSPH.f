...
*----
     ...
     > XMAX,P,FF,UD
      DOUBLE PRECISION, ALLOCATABLE, DIMENSION(:,:) :: DFF,TDFF,XX,GG
*----
*  CYCLIC INDEXING
*----
      ID(I,IS)=1+MOD(I-1,IS)
*----
...
*       Convergence test
        IPICK=1
        GO TO 50
      ELSE IF(TEXT12.EQ.'ANDERSON') THEN
*       READ THE TYPE OF ANDERSON WINDOW SIZE.
        CALL REDGET(INDIC,M,FLOTT,TEXT12,DFLOTT)
        IF(INDIC.NE.1) CALL XABORT('FPSPH: INTEGER DATA EXPECTED(6).')
...
*----
        IF(M.GT.0) THEN
          ALLOCATE(XX(NPERT,M+1),GG(NPERT,M+1))
          IF(ITER.EQ.1) THEN
            XX(:NPERT,:M+1)=0.0D0
            GG(:NPERT,:M+1)=0.0D0
          ELSE
            CALL LCMGET(IPOPT,'ANDERSON-XX',XX)
            CALL LCMGET(IPOPT,'ANDERSON-GG',GG)
          ENDIF
          GG(:NPERT,ID(ITER,M+1))=VARV(:NPERT)-XX(:NPERT,ID(ITER,M+1))
          IF(ITER.GT.1) CALL FPAND(NPERT,M,ITER,XX,GG,VARV)
          XX(:NPERT,ID(ITER+1,M+1))=VARV(:NPERT)
          CALL LCMPUT(IPOPT,'ANDERSON-XX',NPERT*(M+1),4,XX)
          CALL LCMPUT(IPOPT,'ANDERSON-GG',NPERT*(M+1),4,GG)
          DEALLOCATE(GG,XX)
        ENDIF
*----
...
*----
  ...
  160   CONTINUE
        ...
        IF(IPRINT.GT.0) WRITE(6,240) ITER,M,ERROR,ERR2
        IF(ERR2.LT.EPSPH) THEN
          ICONV=1
          IF(IPRINT.GT.0) WRITE(6,230) ITER
        ENDIF
      ELSE
        ERR2=1.0E10
      ENDIF
...
*----
      ...
      IF(JENTRY(1).EQ.0)THEN
        ...
        ISTATE(15)=M ! Anderson window size
      ...
      ELSE
        ...
        CALL LCMPUT(IPOPT,'STATE-VECTOR',NSTATE,1,ISTATE)
      ENDIF
      IF(IPRINT.GT.0) WRITE(6,220) ISTATE(1),ISTATE(5),ISTATE(8),
     1 ISTATE(15)
*----
...
*
  ...
  220 FORMAT(/21H STATE VECTOR OPTIONS/21H --------------------/
     1 7H NPERT ,I8,26H   (NUMBER OF SPH FACTORS)/
     2 7H ITER  ,I8,25H   (NUMBER OF ITERATIONS)/
     3 7H IMETH ,I8,28H   (3/4: FIXED POINT/NEWTON)/
     4 7H M     ,I8,25H   (ANDERSON WINDOW SIZE))
  230 FORMAT(/39H FPSPH: CONVERGENCE OF SPH ALGORITHM IN,I5,
     > 12H ITERATIONS.)
  240 FORMAT(/13H FPSPH: ITER=,I3,4X,12HWINDOW SIZE=,I3,4X,6HERROR=,1P,
     > E10.3,1X,5HERR2=,E10.3)
      END
