...
*----
      ...
      IF(IPRINT .GE. 1) THEN
        ...
        IF(IPRINT .GE. 10) THEN
          WRITE(IOUT,6002) (IOPT(IKI),IKI=1,2)
        ENDIF
      ENDIF
...
*----
      CALL FMTGIS(IPRINT,NBISO,NAMISO,MISPRT,NAMRD,
     >            NOPT,IOPT,ISOPRT)
*----
...
*----
     ...
     >    (KEFF(ISTEP),ISTEP=1,NSTEP),'];'
        WRITE(FMT2,'(A17,I3,A9)') '(A3,2A4,A3,I4,A3,1P,',
     >    NSTEP,'E15.8,A2)'
        DO ISOR=1,MISPRT
     ...
     >        (ISODEN(ISOT,ISTEP),ISTEP=1,NSTEP),'];'
        ...
        ENDDO
      ELSE
        WRITE(FMT1,'(A8,I3,A20)') '(A16,1P,',NSTEP-1,
     >     '(E15.8,A1),E15.8,A2)'
        WRITE(IKMAT,FMT1) 'Time =np.array([',
     >    (TIME(ISTEP)*TIMEFS,',',ISTEP=1,NSTEP-1),
     >     TIME(NSTEP)*TIMEFS,'])'
        WRITE(IKMAT,FMT1) 'Keff =np.array([',
     >    (KEFF(ISTEP),',',ISTEP=1,NSTEP-1),
     >     KEFF(NSTEP),'])'
        WRITE(FMT2,'(A21,I3,A20)') '(A3,2A4,A1,I4,A12,1P,',
     >    NSTEP-1,'(E15.8,A1),E15.8,A2)'
        DO ISOR=1,MISPRT
          WRITE(IKMAT,'(A3,2A4,A11,I4,A1,I4,A2)') 'Den',
     >    NAMRD(1,ISOR),NAMRD(2,ISOR),
     >    '=np.zeros((',NBMIXB,',',NSTEP,'))'
          DO ISOT=1,NBISO
            IF(ISOPRT(ISOT).EQ.ISOR) THEN
              WRITE(IKMAT,FMT2) 'Den',
     >        NAMRD(1,ISOR),NAMRD(2,ISOR),
     >        '[',ISOMIX(ISOT)-1,']=np.array([',
     >        (ISODEN(ISOT,ISTEP),',',ISTEP=1,NSTEP-1),
     >        ISODEN(ISOT,NSTEP),'])'
            ENDIF
          ENDDO
        ENDDO
