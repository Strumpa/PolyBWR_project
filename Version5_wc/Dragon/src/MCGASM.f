*DECK MCGASM
     ...
     1                  IFTRAK,NANI,NGEFF,NBCDA,NFI,NREG,NLONG,M,NMU,
     2                  NANGL,N2MAX,LC,NDIM,NGIND,CYCLIC,ISCR,CAZ0,CAZ1,
     3                  CAZ2,CPO,LC0,PACA,LPS,LTMT,NPJJM,LACA,LPJJ,
     4                  LPJJAN,SIGAL,LPRISM,N2REG,N2SOU,NZP,DELU,FACSYM,
     5                  ISTRM)
*
*-----------------------------------------------------------------------
*
...
*----
      ...
      INTEGER IPRINT,IFTRAK,NGEFF,NBCDA,NFI,NREG,NLONG,M,NMU,NANGL,
     1 N2MAX,LC,NDIM,NGIND(NGEFF),LC0,PACA,LPS,NPJJM,N2REG,N2SOU,NZP,
     2 ISTRM
      REAL CPO(NMU),SIGAL(-NBCDA:M,NGEFF),DELU,FACSYM
...
*        ----------------------------
            ...
            ENDDO
            CALL MCGPTS(SUBPJJ,NFI,NREG,NBCDA,M,NANI,NFUNL,NANGL,NMU,
     1           NMOD,LPS,NPJJM,NGEFF,IANGL,NSEG,ISGNR,NZON,NOM,IS,JS,
...
*        ---------------------------------
            ...
            DO II=0,NR2SE
               HH(II+2)=HH(II+1)+H(II+2)
            ENDDO
            CALL MCGPTA(NFI,NREG,NLONG,NBCDA,M,NANGL,NMU,LC,NGEFF,
...
*     process last integration line for ACA TMT
     ...
     1        WEIGHT0,H,H0,LFORC,NTPROC) 
         ...
         DO II=0,NR2SE
            HH(II+2)=HH(II+1)+H(II+2)
         ENDDO
         CALL MCGPTA(NFI,NREG,NLONG,NBCDA,M,NANGL,NMU,LC,NGEFF,IANGL,
     1        NSEG,NOM,NZONA,IPERM,KM,IM,MCU,PREV,NEXT,WEIGHT,ZMU,WZMU,
...
*        ----------------------------
     ...
     2           NBCDA,M,IS,JS,PJJD,PSJ,LPJJAN,NPJJM,PJJIND,SIGAL,1)
            ELSE IF(ISTRM.EQ.3) THEN
*             TIBERE model
              CALL MCGDSD(NSEG,NSUB,NMU,LPS,NFUNL,NANGL,NGEFF,WEIGHT,
     1           TRHAR,H,ZMU,WZMU,KANGL,NOM,NZON,NFI,NREG,NDIM,NBCDA,M,
     2           IS,JS,PJJD,PSJ,LPJJAN,NPJJM,PJJIND,SIGAL,1,CAZ1(IANGL),
...
*        ---------------------------------
     ...
     1           ZMUA,WZMUA,NOM,NZONA,NLONG,NFI,NDIM,LC,NBCDA,M,KM,IM,
...
*---
      IF(LACA) THEN
         DO II=1,NGEFF
           ...
           CALL MCGDS3(NLONG,PACA,M,SIGAL(0,II),XSW((II-1)*(M+1)+1),
     1          VA,NZONA,LC,MCU,IM,JU,LC0,IM0,MCU0,DIAGF(1,II),
     2          CF(1,II),DIAGQ((II-1)*NLONG+1),DIAGFR,CFR,LUDF,LUCF)
*     
           IF(IPRINT.GT.3) THEN
