...
def POSTPROC(pyCOMPOs,ListeCOMPO,ListeAUTOP,name_geom,bu_autop_list,VISU_param,Nmin,S2_libs,ssh_module,ssh_method,CORR,DEPL_SOL,SAT):
    ...
    if visu_SERPENT==1 or visu_COMP==1 or visu_DELTA==1 :
        ...
        for test_name in DRAGON_results_ALL.keys():
            ...
            elif "HOM_Gd157_PCC" in test_name:
                test_name_file = "HOM_UOX_Gd157"
    ...
    if visu_DELTA==1 :
        ...
        for test_name in DRAGON_results_ALL.keys():
            ...
            for lib_name in S2_libs:
                ...
                for k in range(2,len(isotopes_SOUHAITES)+2):
                    print(f"Isotope = {isotopes_SOUHAITES[k-2]}")
                    print(f"ERRORS = {ERRORS_ALL[test_name][lib_name][k]}")
                    print(f"Len of errors on iso = {len(ERRORS_ALL[test_name][lib_name][k])}")
