...
def PREPARE_GEO(TRACK, MaterialIndexation, pitch, height, I_z, ene_groups, 
                Tf_ini, Tc_ini, DensC_ini):
    """
    ...
    myLifo.pushEmpty(MaterialIndexation, "LCM")
    myLifo.push(pitch)
    myLifo.push(height)
    myLifo.push(I_z)
    ...
    myLifo.push(Tf_ini)
    myLifo.push(Tc_ini)
    myLifo.push(DensC_ini)
    

    myLifo.lib()
    calcINIT = cle2000.new('PREPARE_GEO',myLifo,1)
    calcINIT.exec()
