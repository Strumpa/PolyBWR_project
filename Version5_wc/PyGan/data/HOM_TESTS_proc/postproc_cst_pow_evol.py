...
class multiD5S2_comparisons:
    ...
    def compare_keffs(self):
        for case in self.D5_cases:
            ...
            plt.legend()
            plt.grid()
            plt.savefig(f"{self.save_dir}/keff_{self.comparison_name}_{self.burnup_points}.png")
            plt.close()
    ...
    def compare_Ni(self):
        for iso in self.tracked_nuclides:
            ...
            for case in self.D5_cases:
                ...
                plt.xlabel(f"Burnup [{self.S2_case.unitsBU}]")
                plt.ylabel(f"{iso} density")
                plt.title(f"{iso} density evolution for {self.comparison_name} case")
                plt.legend()
                plt.grid()
                plt.savefig(f"{self.save_dir}/dens_{iso}_{self.comparison_name}_{self.burnup_points}.png")
                plt.close()
    ...
    def plot_delta_Ni(self):
        ...
        for iso in self.tracked_nuclides:
            ...
            plt.legend()
            plt.grid()
            plt.savefig(f"{self.save_dir}/relative_delta_{iso}_{self.comparison_name}_{self.burnup_points}.png")
            plt.close()
            ...
            plt.legend()
            plt.grid()
            plt.savefig(f"{self.save_dir}/abs_delta_{iso}_{self.comparison_name}_{self.burnup_points}.png")
            plt.close()
        return
