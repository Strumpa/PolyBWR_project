...
class compare_D5_OMC_rates_XS:
    def __init__(self, case_name, study_type, D5_case, OMC_case, OMC_XS_libs, compo_keywords, isotopes, self_shielding_methods, save_path):
        self.E0 = 1.0E+07 # eV --> 10 MeV
        self.case_name = case_name
        self.study_type = study_type
        self.D5_case = D5_case
        self.OMC_case = OMC_case

        # Data used to initialize the nested dictionaries for the D5-OMC differences
        self.OMC_XS_libs = OMC_XS_libs
        self.compo_keywords = compo_keywords
        self.isotopes = isotopes
        self.self_shielding_methods = self_shielding_methods
        ...
        # Initialize the nested dictionaries for the D5-OMC differences
        self.D5_OMC_rates_diff = self.initialize_nested_dict(OMC_XS_libs, compo_keywords, isotopes, reactions, self_shielding_methods)
        #self.D5_OMC_XS_diff =  self.initialize_nested_dict(OMC_XS_libs, compo_keywords, isotopes, reactions, self_shielding_methods)

        return

    def initialize_nested_dict(self, OMC_XS_libs, keywords, isotopes, reactions, ssh_methods):
        """
        Initializes a nested dictionary with the structure:
        OMC_XS_libs -> keywords -> isotopes -> reactions -> ssh_methods -> empty list.
        
        Parameters:
            OMC_XS_libs (list): List of keys for the first level.
            keywords (list): List of keys for the second level.
            isotopes (list): List of keys for the third level.
            reactions (list): List of keys for the fourth level.
            ssh_methods (list): List of keys for the innermost level.
    ...
    def compare_reaction_rates(self, reaction_id, keyword, SSH_methods):
        iso = reaction_id.split("_")[0]
        reaction = reaction_id.split("_")[1]
        for library in self.OMC_XS_libs:
            # for now only consider BU step = 0
            for SSH in SSH_methods:
                self.D5_OMC_rates_diff[library][keyword][iso][reaction][SSH] = (self.D5_case.reaction_data_pair[reaction_id][keyword][SSH]["rates"] - self.OMC_case.rates["SHEM295"][f"{iso}_{reaction}_{library}_0"])*100/self.OMC_case.rates["SHEM295"][f"{iso}_{reaction}_{library}_0"]
        return
    
    def renorm_rates(self, reaction_id, keyword, mesh_name):
        """
        Since DRAGON5 and OpenMC do not use the same normalization for fluxes, 
        this function renormalizes the rates in order to allow for a direct D5-OpenMC comparison.
        """
        # Renormalize DRAGON5 and OpenMC rates to 1
        # For all OpenMC libraries and DRAGON5 SSH methods
        for SSH in self.D5_case.reaction_data_pair[reaction_id][keyword].keys():
            self.D5_case.reaction_data_pair[reaction_id][keyword][SSH]["rates"] = self.D5_case.reaction_data_pair[reaction_id][keyword][SSH]["rates"]/np.sum(self.D5_case.reaction_data_pair[reaction_id][keyword][SSH]["rates"])
        for library in self.OMC_XS_libs:
            self.OMC_case.rates[mesh_name][f"{reaction_id}_{library}_0"] = self.OMC_case.rates[mesh_name][f"{reaction_id}_{library}_0"]/np.sum(self.OMC_case.rates[mesh_name][f"{reaction_id}_{library}_0"])
        return
    
    def plot_rates_D5_OMC(self, reaction_id, keyword, mesh_name, SSH_methods, bu_step = 0):
        iso = reaction_id.split("_")[0]
        reaction = reaction_id.split("_")[1]
        if reaction == "ngamma":
            reaction_print = "$(n,\\gamma)$"
        ...
        for library in self.OMC_XS_libs:
            ...
            u = []
            OMC_rates_to_plot = []
            OMC_rates = self.OMC_case.rates[mesh_name][f"{iso}_{reaction}_{library}_{bu_step}"]
            for i in range(len(OMC_rates)):
                u.extend([u_mesh[i],u_mesh[i+1]])
                OMC_rates_to_plot.extend([OMC_rates[i],OMC_rates[i]])
            plt.step(u, OMC_rates_to_plot, where='post', label=f"OMC: $\\tau$ {reaction_print} {library} for {iso}")
        ssh_id = ""
        for SSH in SSH_methods:   
            ssh_id += SSH 
            u = []   
        ...
        plt.ylabel(f"$\\tau$ {reaction_print} {iso}")
        plt.yscale("log")
        plt.title(f"Comparison of {iso} {reaction_print} reaction rates between DRAGON5 and OpenMC")
        plt.grid()
        plt.legend()
        plt.savefig(f"{self.save_path}/{iso}_{reaction}_rates_D5_{keyword}_{ssh_id}_OMC_{lib_id}_{mesh_name}.png")
        plt.close()
        return
    ...
    def plot_zoom_rates_and_errors(self, library, rection_id, keyword, mesh_name, SSH_methods, grmin, grmax):
        """
        use subplot to plot 1) rates (D5 and OpenMC) and 2) relative errors (D5-OpenMC)/OpenMC for each SSH method
        grmin and grmax are the group indices to zoom in
        """
        ssh_id = ""
        iso = rection_id.split("_")[0]
        ...
        fig,ax = plt.subplots(number_of_subplot,1,figsize=(20, 3*number_of_subplot))
        lib_id = library
        OMC_rates = self.OMC_case.rates[mesh_name][f"{iso}_{reaction}_{library}_0"][grmin-1:grmax]
        OMC_rates_to_plot = []
        for i in range(len(OMC_rates)):
            OMC_rates_to_plot.extend([OMC_rates[i],OMC_rates[i]])
        ax[0].step(u, OMC_rates_to_plot, where='post', label=f"OpenMC: $\\tau$ {reaction_print} {library} for {iso}")
        for SSH in SSH_methods:
            D5_rates_to_plot = []     
            D5_rates = self.D5_case.reaction_data_pair[rection_id][keyword][SSH]["rates"][grmin-1:grmax]
            for i in range(len(D5_rates)):
        ...
        ax[0].grid()
        ax[0].legend()
        ax[0].set_title(f"Comparison of {iso} {reaction_print} reaction rates between DRAGON5 {keyword} and OpenMC")

        for i, SSH in enumerate(SSH_methods):
            ssh_id += SSH
            D5_OMC_rates_diff_to_plot = []     
            D5_OMC_rates_diff = self.D5_OMC_rates_diff[library][keyword][iso][reaction][SSH][grmin-1:grmax]
            for j in range(len(D5_OMC_rates_diff)):
                D5_OMC_rates_diff_to_plot.extend([D5_OMC_rates_diff[j],D5_OMC_rates_diff[j]])
            ax[i+1].step(u, D5_OMC_rates_diff_to_plot, label=f"{SSH} - {library} : {iso} {reaction_print}")
            ax[i+1].set_xlabel("Lethargy")
            ax[i+1].set_ylabel(f"$\\Delta \\tau$ {reaction_print} {iso}  [%]")
            ax[i+1].set_title(f"Relative difference on $\\tau$ {reaction_print} {iso} between DRAGON5 {SSH} and OpenMC ({library}) on {mesh_name}")
            ax[i+1].grid()
            ax[i+1].legend()
        plt.tight_layout()
        #plt.title(f"Comparison of {iso} {reaction_print} reaction rates between DRAGON5 and SERPENT2")
        plt.savefig(f"{self.save_path}/{iso}_{reaction}_rates_D5_{keyword}_{ssh_id}_OMC_{lib_id}_{mesh_name}_zoomed_{grmin}_{grmax}.png")
        plt.close()
        return
