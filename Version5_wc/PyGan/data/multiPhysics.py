...
def convergence(Field, OldField, tol):
    """
    check the converegence of the field at all heigths,
    tol is in %
    """
    conv=False
    for i in range(len(Field)):
        if np.abs(Field[i] - OldField[i])*100/OldField[i] > tol:
            conv = False
            break
        else:
            conv = True
...
tol_POW = 1e-2 # Convergence criterion for the power axial distribution, in %
