...
from Dragon5_cpo_parsing import  parse_DRAGON_DIAG_rates_regi_num, parse_DRAGON_SCHEME
...
def analyze_computational_schemes():
    ...
    computational_schemes = ["1L_MOC_SC_ILU0"] # ["2L_IC_MOC_SC_ILU0", "2L_PIJ_MOC_SC_ILU0"] #  #["2L_IC_MOC_SC", "2L_PIJ_MOC_SC"]#["2L_IC_MOC", "2L_IC_MOC_SC_GRMAX15", "2L_IC_MOC_SC_GRMAX16", "2L_IC_MOC_SC_GRMAX17", "2L_IC_MOC_SC_GRMAX18","2L_IC_MOC_SC_GRMAX19", "2L_IC_MOC_SC_GRMAX20", "2L_IC_MOC_SC_GRMAX21", "2L_IC_MOC_SC_GRMAX22", "2L_IC_MOC_SC_GRMAX23"]
    ...
    composition_option = "AT10_void_0"
    evaluation = "J311_295" #"endfb8r1_295" #
        ...
        for ssh_method in ssh_methods:
            for ssh_sol in ssh_sol_doors:
                ...
                if transport_correction == "NONE":
                    CPO_name = f"CPO_n{num_angles}_ld{int(line_density)}_n8_ld25_TSPC_{anisotropy_level}_{ssh_sol}_MOC_GAUS_{anisotropy_level}_{batch}_200"
                else:
                    CPO_name = f"CPO_n{num_angles}_ld{int(line_density)}_n8_ld25_TSPC_{anisotropy_level}_{ssh_sol}_MOC_GAUS_{anisotropy_level}_{batch}_200_{transport_correction}"
                print(f"computational_scheme : {computational_scheme}, CPO_name = {CPO_name}")
                keff_D5, fiss_rates_D5, fiss_over_abs_rates_D5, D5_abs_U238, FLUX_groups_D5, times, energy_mesh = parse_DRAGON_SCHEME(name_case, CPO_name, composition_option, evaluation, ssh_method, correlation_option, refinement_opt_name, fission_isotopes , n_gamma_isotopes = ["U238", "Gd155", "Gd157"], bu=0, computational_scheme=computational_scheme)
                keff_S2, fission_rates_S2, ngamma_rates_S2, fiss_over_abs_S2, fluxes_S2, S2_abs_U238 = parse_S2_pin_mat_det(name_case=S2_name_case, XS_lib_S2="jeff311_pynjoy2016", fission_isotopes=fission_isotopes, ngamma_isotopes=["U238", "Gd155", "Gd157"], bu=0)
                ...
                plot_spectrum_comparison(energy_mesh, FLUX_groups_D5, fluxes_S2, name_case, CPO_name, calculation_opt = f"{computational_scheme}_{geometry_type}_{ssh_method}_{ssh_sol}_{correlation_option}", evaluation=evaluation)
