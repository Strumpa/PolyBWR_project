...
def POSTPROC_hom(pyCOMPO,ListeCOMPO,name_geom,name_mix,suffixe,VISU_param,form,Nmin):
    ...
    #SERPENT_path=f'/home/p117902/working_dir/Serpent2_para_bateman/Linux_aarch64/PyNjoy2016_results/' #for results with JEFF311_oldlib2016 acelib <--- working on fixing that.
    ...
    if visu_SERPENT==1 or visu_COMP==1 or visu_DELTA==1 :
        ...
        # 1) results from PyNjoy2016
        # --- Keff
        #res=serpentTools.read(SERPENT_path+name_mix+"_mc_res.m")
        res_pynjoy=serpentTools.read("/home/p117902/working_dir/Serpent2_para_bateman/Linux_aarch64/AT10_C7_hom_mc_res.m")

        serpent_keff_pynjoy=res_pynjoy.resdata["absKeff"]
        np.savetxt('serpent_keff.txt',serpent_keff_pynjoy)
        ...
        SERPENT_BU=np.loadtxt('serpent_BU_pynjoy.txt',dtype=float)
        ...
        for k in range(lenBU_SERPENT):
            ...
            serpent_keff_pynjoy[k]=SERPENT_keff[k][0]
        ...
        SERPENT_ALL_pynjoy=[
           SERPENT_BU,
           serpent_keff_pynjoy,
           SERPENT_ISOTOPESDENS_pynjoy[0,:],
           SERPENT_ISOTOPESDENS_pynjoy[1,:],
           SERPENT_ISOTOPESDENS_pynjoy[2,:],
           SERPENT_ISOTOPESDENS_pynjoy[3,:],
           SERPENT_ISOTOPESDENS_pynjoy[4,:],
           SERPENT_ISOTOPESDENS_pynjoy[5,:],
           SERPENT_ISOTOPESDENS_pynjoy[6,:],
           SERPENT_ISOTOPESDENS_pynjoy[7,:],
           SERPENT_ISOTOPESDENS_pynjoy[8,:],
           SERPENT_ISOTOPESDENS_pynjoy[9,:],
           SERPENT_ISOTOPESDENS_pynjoy[10,:],
           ]
        #print("$$$ ---------------- SERPENT_ALL",SERPENT_ALL)
        # 2) results from Serpent2 with sss_jeff311 library
        # --- Keff
        #res=serpentTools.read(SERPENT_path+name_mix+"_mc_res.m")
        res_oldlib=serpentTools.read("/home/p117902/working_dir/Serpent2_para_bateman/Linux_aarch64/AT10_C7_hom_oldlib_mc_res.m")

        serpent_keff_oldlib=res_oldlib.resdata["absKeff"]
        np.savetxt('serpent_keff_oldlib.txt',serpent_keff_oldlib)
        SERPENT_keff=np.loadtxt('serpent_keff_oldlib.txt',dtype=float)
            
        # --- BU
        #depFile = SERPENT_path+name_mix+"_mc_dep.m"
        depFile = "/home/p117902/working_dir/Serpent2_para_bateman/Linux_aarch64/AT10_C7_hom_oldlib_mc_dep.m"
        ...
        SERPENT_BU=np.loadtxt('serpent_BU_oldlib.txt',dtype=float)
        
        # --- ISOTOPES DENSITIES
        serpent_ISOTOPESDENS=fuel.toDataFrame("adens",names=isotopes_SOUHAITES)
        np.savetxt('serpent_ISOTOPESDENS_oldlib.txt',serpent_ISOTOPESDENS)
        SERPENT_ISOTOPESDENS=np.loadtxt('serpent_ISOTOPESDENS_oldlib.txt',dtype=float)
        SERPENT_ISOTOPESDENS_oldlib=np.transpose(SERPENT_ISOTOPESDENS)
        ...
        for k in range(lenBU_SERPENT):
            SERPENT_BU[k]=1000*SERPENT_BU[k]
            serpent_keff_oldlib[k]=SERPENT_keff[k][0]
        ...
        SERPENT_ALL_oldlib=[
           SERPENT_BU,
           serpent_keff_oldlib,
           SERPENT_ISOTOPESDENS_oldlib[0,:],
           SERPENT_ISOTOPESDENS_oldlib[1,:],
           SERPENT_ISOTOPESDENS_oldlib[2,:],
           SERPENT_ISOTOPESDENS_oldlib[3,:],
           SERPENT_ISOTOPESDENS_oldlib[4,:],
           SERPENT_ISOTOPESDENS_oldlib[5,:],
           SERPENT_ISOTOPESDENS_oldlib[6,:],
           SERPENT_ISOTOPESDENS_oldlib[7,:],
           SERPENT_ISOTOPESDENS_oldlib[8,:],
           SERPENT_ISOTOPESDENS_oldlib[9,:],
           SERPENT_ISOTOPESDENS_oldlib[10,:],
           ]
        #print("$$$ ---------------- SERPENT_ALL",SERPENT_ALL)
    ...
    if visu_DELTA==1 :
        ...
        ERROR_pynjoy=np.zeros((lenISOT_DRAGON,lenBU_DRAGON))
        ERROR_oldlib=np.zeros((lenISOT_DRAGON,lenBU_DRAGON))
        ...
        for k in range(lenISOT_DRAGON):
            for j in range(Nmin,lenBU_DRAGON):
                ...
                if k==0: # Vecteur BU
                    ...
                    if SERPENT_ALL_pynjoy[k][j]==0 and SERPENT_ALL_oldlib[k][j]==0:
                        ERROR_pynjoy[k][j-Nmin]=0
                        ERROR_oldlib[k][j-Nmin]=0
                    else:
                        #print(f"DRAGON_ALL[k][j] = {DRAGON_ALL[k][j]}")
                        #print(f"SERPENT_ALL[k][j] = {SERPENT_ALL_pynjoy[k][j]}")
    ...
    if visu_SERPENT==1:
        ...
        for k in range(lenISOT_SERPENT):
            ...
            plt.xlabel('BU (MWj/t)')
            plt.grid()
            plt.legend(['Serpent2_oldlib','Serpent2_pynjoy'])

            if k == 0: # Comparaison des Keff
                plt.ylabel('Keff')
                save_name=name_mix+'_Serpent2_Keff_pynjoy_oldlib_serpent'
                fig_name=name_mix+' - Keff'
            else : # Erreur sur isotopes
                plt.ylabel('Concentration atomique (a/barn.cm)')
                save_name=name_mix+'_Serpent2_pynjoy_oldlib_'+isotopes_SOUHAITES[k-1]
                fig_name=name_mix+' - '+isotopes_SOUHAITES[k-1]
    ...
    if visu_COMP==1:
        ...
        for k in range(lenISOT_DRAGON):
            ...
            plt.xlabel('BU (MWj/t)')
            plt.grid()
            plt.legend(['DRAGON5','Serpent2_oldlib','Serpent2_pynjoy'])

            if k == 0: # Comparaison des Keff
                plt.ylabel('Keff')
                save_name=name_mix+'_COMP_pynjoy_oldlib_Keff'
                fig_name=name_mix+' - Keff'
            else : # Erreur sur isotopes
                plt.ylabel('Concentration atomique (a/barn.cm)')
                save_name=name_mix+'_COMP_'+isotopes_SOUHAITES[k-1]
                fig_name=name_mix+' - '+isotopes_SOUHAITES[k-1]
