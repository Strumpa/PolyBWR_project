...
from utilities import sum_S2rates_over_iso, define_OECD_NEA_PHASE_IIIB_mixes
...
def parse_Spectrum_detector(path_to_S2, name_case, bu_step=0):
    """
    Parse the spectrum detector from Serpent2 results
    path_to_S2 : (str) path to the Serpent2 output files
    name_case : (str) name of the Serpent2 output file
    bu_step : (int) burnup step
    """
    ...
    # Extracting the spectrum detector response
    n_groups = detector.detectors['spectrum_295G'].tallies.shape[0]
    multi_group_spectrum = []
    
    print(n_groups)
    print(detector.detectors['spectrum_295G'].tallies)
    for i in range(n_groups):
        multi_group_spectrum.append(detector.detectors['spectrum_295G'].tallies[i])
    
    multi_group_spectrum = np.array(multi_group_spectrum)
    # Renormalize the spectrum to 1
    multi_group_spectrum /= np.sum(multi_group_spectrum)
    return keff, multi_group_spectrum


def parse_eighth_lattice_S2(path_to_S2, name_case, bu_step=0):
    """
    Parse lattice detector from Serpent2 results and extract rates for the 8th of the lattice
    path_to_S2 : (str) path to the Serpent2 output files
    name_case : (str) name of the Serpent2 output file
    bu_step : (int) burnup step
    """
    ...
    # lattice_2G
    n_groups = detector.detectors['_lattice_2G'].tallies.shape[0]
    n_cells = detector.detectors['_lattice_2G'].tallies.shape[1]
    n_tallies = detector.detectors['_lattice_2G'].tallies.shape[2]
    print(f"lattice detector shape : {detector.detectors['_lattice_2G'].tallies.shape}")
    print(f"n_groups = {n_groups}, n_cells = {n_cells}, n_tallies = {n_tallies}")
    # the lattice detector is a 10x10 lattice, however fuel array is only 8x8 : 1 cell on each side / top and bottom is water
    # Extracting the detector response
    tally_index_to_react = { 0: "U235_ngamma", 1 : "U238_ngamma",
                             2 : "Gd155_ngamma", 3: "Gd157_ngamma", 
                             4 : "U234_fiss", 5 : "U235_fiss", 6 : "U238_fiss"}
    ...
    vol = np.pi * 0.529 ** 2
    mix_desc = define_OECD_NEA_PHASE_IIIB_mixes()
    eighth_cells_lattice_numbering = [11, 12, 13, 14, 22, 23, 24, 33, 34]
    S2_numbering_to_POS_ID_IDX = {11: 1, 
                                 12: 2, 
                                 13: 3, 
                                 14: 4, 
                                 22: 5, 
                                 ...
                                 33: 8, 
                                 34: 9}
    cells_on_diag_lattice_numbering = [11, 22, 33]

    tally_indices_to_fiss_iso = {12:"U234", 
                                 13:"U235", 
                                 14:"U236", 
                                 15:"U238"
                                 }
    fiss_rates_S2_lat_det = np.zeros((n_groups, len(eighth_cells_lattice_numbering)))  # Initialize the array for fission rates
    OECD_NEA_cells_desc = { 11: {"ROD_ID": 4, "POSITION": 1},
                            12: {"ROD_ID": 3, "POSITION": 2},
                            13: {"ROD_ID": 2, "POSITION": 3},
                            14: {"ROD_ID": 2, "POSITION": 4},
                            22: {"ROD_ID": 2, "POSITION": 5},
                            23: {"ROD_ID": 5, "POSITION": 6},
                            24: {"ROD_ID": 1, "POSITION": 7},
                            33: {"ROD_ID": 1, "POSITION": 8},
                            34: {"ROD_ID": 1, "POSITION": 9} }



    for i in range(n_groups):
        for j in eighth_cells_lattice_numbering:
            rates = 0
            iso_dens_U235 = mix_desc[OECD_NEA_cells_desc[j]["ROD_ID"]]["U235"]
            print(f"i={i}, j={j}, iso_dens_U235={iso_dens_U235}")
            for tally_idx in tally_indices_to_fiss_iso.keys():
                iso_dens = mix_desc[OECD_NEA_cells_desc[j]["ROD_ID"]][tally_indices_to_fiss_iso[tally_idx]]
                rates += detector.detectors['_lattice_2G'].tallies[i,j,tally_idx] * iso_dens  * vol
                print(f"vol = {vol}")
            fiss_rates_S2_lat_det[i][S2_numbering_to_POS_ID_IDX[j]-1] = rates

    return fiss_rates_S2_lat_det, mix_desc

def parse_material_det(path_to_S2, name_case, bu_step=0):
    """Parse material detector from Serpent2 results.

    Args:
        path_to_S2 (str): Path to the Serpent2 output files.
        name_case (str): Name of the Serpent2 output file.
        bu_step (int, optional): Burnup step. Defaults to 0.
    """
    ...
    mix_desc = define_OECD_NEA_PHASE_IIIB_mixes()
    POS_TO_ROD_ID = {1:4, 2:3, 3:2, 4:2, 5:2, 6:5, 7:1, 8:1, 9:1}
    POS_ID_TO_NUM = {1: 4, 2: 8, 3: 8, 4: 8, 5: 4, 6: 8, 7: 8, 8: 4, 9: 8}
    ...
    tally_index_to_react = {0 : "U234_fission", 1 : "U235_fission",
                            ...
                            6 : "U235_ngamma", 7 : "U238_ngamma",
                            8 : "Pu239_ngamma", 9 : "Pu241_ngamma",
                            10 : "Xe135_ngamma", 11 : "Sm149_ngamma",
                            12 : "Gd154_ngamma", 13 : "Gd155_ngamma", 
                            14 : "Gd156_ngamma", 15 : "Gd157_ngamma", 
                            16 : "Gd158_ngamma", 17 : "Gd160_ngamma"
            }
    n_cells = 9
        ...
        if detector_name in detector.detectors.keys():
            ...
            for g in range(n_goups):
                for r in range(n_reactions):
                    ...
                    pos_id = cell_idx + 1  # since cell_idx is 0-based and ROD_ID starts from 1
                    rod_id = POS_TO_ROD_ID[pos_id]
