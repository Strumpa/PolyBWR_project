...
class multiD5S2_comparisons:
    ...
    def compare_keffs(self):
        for case in self.D5_cases:
            ...
            plt.plot(self.S2_case.BU, case.keff[:len(self.S2_case.BU)], label = f"D5", marker = "x", linestyle = "--")
            plt.plot(self.S2_case.BU, self.S2_case.keff, label = f"S2", marker = "x", linestyle = "--")
            plt.xlabel(f"Burnup [{self.S2_case.unitsBU}]")
            plt.ylabel(f"keff")
            plt.title(f"keff for {self.comparison_name} case")
            plt.legend()
            plt.grid()
            plt.savefig(f"{self.save_dir}/keff_{self.comparison_name.replace(" ", "_")}_{self.burnup_points}.png")
            plt.close()
        ...
        for iso in self.tracked_nuclides:
            ...
            for case in self.D5_cases:
                if len(case.DRAGON_ISOTOPESDENS[iso]) == len(self.S2_case.Ni[iso]):
                    ...
                    delta_Niso_abs = [(case.DRAGON_ISOTOPESDENS[iso][idx] - self.S2_case.Ni[iso][idx]) for idx in range(len(self.S2_case.Ni[iso]))]
                else:
                    ...
                    delta_Niso = [(interp_Niso[idx] - self.S2_case.Ni[iso][idx]) * 100 / self.S2_case.Ni[iso][idx]
                        if self.S2_case.Ni[iso][idx] != 0 else 0
                        for idx in range(len(self.S2_case.Ni[iso]))]
                    delta_Niso_abs = [(interp_Niso[idx] - self.S2_case.Ni[iso][idx]) for idx in range(len(self.S2_case.Ni[iso]))]
                ...
                delta_Niso_case_abs[f"{case.draglib_name}_{case.ssh_opt}_{case.correlation}_{case.extrapolation_type}_to_S2_edep{self.S2_case.edep_id}"] = delta_Niso_abs

                plt.plot(self.S2_case.BU, case.DRAGON_ISOTOPESDENS[iso][:len(self.S2_case.BU)], label = f"D5", marker = "x", linestyle = "--")
                plt.plot(self.S2_case.BU, self.S2_case.Ni[iso], label = f"S2", marker = "x", linestyle = "--")
                plt.xlabel(f"Burnup [{self.S2_case.unitsBU}]")
            ...
            self.delta_Niso[f"{iso}_rel"] = delta_Niso_case
            self.delta_Niso[f"{iso}_abs"] = delta_Niso_case_abs
        return
    ...
    def plot_delta_Ni(self):
        ...
        for iso in self.tracked_nuclides:
            plt.figure(figsize=(10, 6))
            for comparison_case in self.delta_Niso[f"{iso}_rel"].keys():
                plt.plot(self.S2_case.BU, self.delta_Niso[f"{iso}_rel"][comparison_case], label = f"{comparison_case}".replace("_"," "), marker = "x", linestyle = "--")
            plt.xlabel(f"Burnup [{self.S2_case.unitsBU}]")
            ...
            plt.figure(figsize=(10, 6))
            for comparison_case in self.delta_Niso[f"{iso}_abs"].keys():
                plt.plot(self.S2_case.BU, self.delta_Niso[f"{iso}_abs"][comparison_case], label = f"{comparison_case}".replace("_"," "), marker = "x", linestyle = "--")
            plt.xlabel(f"Burnup [{self.S2_case.unitsBU}]")
