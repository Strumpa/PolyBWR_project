...
\OptDirEnr
 {P-KINETICS\blank{2}}{*}
 {Sub-directory containing point kinetics information.
  This sub-directory follows the specification presented in \Sect{pkinetics}.}
...
\RealEnr
 {STEP-CM\blank{5}}{$1$}{cm} {Length of one rod insertion step.}
...
\subsubsection{The \moc{P-KINETICS} sub-directory}\label{sect:pkinetics}

The \moc{P-KINETICS} sub-directory contains the information collected by the {\tt PKINI:} module of DONJON and updated at
each call of the {\tt PKINS:} module.

\vskip 0.08cm

The dimensioning parameters $\mathcal{K}_i$, which are stored in the state vector for this data structure, represent:

\begin{itemize}

\item The multiphysics time stage index ($\mathcal{K}_1$). This parameter is set to 1 by module {\tt PKINI:} and is
increased by module {\tt PKINS:}.

\item The number of delayed precursor groups ($N_{\rm group}=\mathcal{K}_2$).

\item The number of feedback parameters ($N_{\rm alpha}=\mathcal{K}_3$). Currently set to a maximum of 10.

\item The number of time laws for some feedback parameters ($\mathcal{K}_4$).

\end{itemize}

\begin{DescriptionEnregistrement}{Records in \moc{P-KINETICS} sub-directory}
{7.0cm} \label{tabl:tabpkinetics}

\IntEnr
  {STATE-VECTOR}{$40$}
  {Vector describing the various parameters associated with this data structure $\mathcal{K}_i$}

\RealEnr
 {T-VALUE-INIT}{$1$}{s} {Initial time.}

\RealEnr
 {LAMBDA\blank{6}}{$1$}{s} {Prompt neutron generation time.}

\RealEnr
 {EPSILON\blank{5}}{$1$}{s} {Rugge-Kutta epsilon.}

\RealEnr
 {BETAI\blank{7}}{$N_{\rm group}$}{} {Delayed neutron fraction vector.}

\RealEnr
 {LAMBDAI\blank{5}}{$N_{\rm group}$}{s} {Delayed neutron time-constant vector.}

\DirlEnr
  {ALPHA\blank{7}}{$N_{\rm alpha}$}
  {List of feedback parameters. Each component of the list is a directory containing
   the information relative to a feedback parameter.}

\RealEnr
 {P-VALUE-INIT}{$N_{\rm alpha}$}{}
 {Initial {\tt P-VALUE} of feedback parameters.}

\CharEnr
 {P-NAME\blank{6}}{$(N_{\rm alpha})*12$} {{\tt P-NAME} of feedback parameters.}
          
\DirEnr
 {PKIN-DAT0001}
 {Sub-directory containing beginning-of-stage information.}
          
\OptDirEnr
 {PKIN-DAT0002}{*}
 {Sub-directory containing end-of-stage information. Set by module {\tt PKINS:}}

\RealEnr
 {PKIN-TIMES}{$2$}{s} {Beginning and end of stage times.}
