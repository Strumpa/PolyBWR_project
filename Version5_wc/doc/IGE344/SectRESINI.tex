...
each channel refuelling, or following a specific pattern. The direction of refuelling
corresponds to the direction of coolant flow along the channel. 
...
$[$ \moc{REF-SHIFT} $\{$ \dusa{ishift} $|$ \moc{COMB} (\dusa{ishift}(i) ,  i = 1, \dusa{ncomb} ) $|$ \moc{SHUFF} (\dusa{ipattern}(i) ,  i = 1, \dusa{ncomb} ) \\
~~~$[[$ \moc{PATTERN} (\dusa{ipos}(i) ,  i = 1, \dusa{nb} ) $\{$ \moc{UNIDIR} $|$ \moc{BIDIR} $\}$ $]]$ $\}$ $]$ \\
...
\item[\moc{SHUFF}] keyword used to enable the axial shuffling of specific channels, option used in time-averaged calculations only.

\item[\dusa{ipattern}] integer array of the bundle-shuffling pattern indexes. The pattern indexes are associated with the order in which the patterns are declared with the keyword \moc{PATTERN}.

\item[\moc{PATTERN}] keyword to specify a new bundle-shuffling pattern. The number of patterns must be equal to the maximum index \dusa{ipattern} indicated.

\item[\dusa{ipos}] integer array of the bundle positions along the channel before refuelling. Values between 1 and \dusa{nb} indicate the previous positions in the same channel, a value of 0 indicates the entry of a fresh bundle.

\item[\moc{UNIDIR}] keyword to indicate unidirectionnal coolant flow and bundle displacement for all channels in the combustion zone using this shuffling pattern.

\item[\moc{BIDIR}] keyword to indicate bidirectionnal coolant flow and bundle displacement for all channels in the combustion zone using this shuffling pattern. This is the default option when not using the SHUFF keyword.
