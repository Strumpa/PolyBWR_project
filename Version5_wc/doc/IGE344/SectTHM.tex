...
$[$ \moc{FLUID} $\{$ \moc{H2O} $|$ \moc{D2O} $|$ \moc{SALT} \dusa{sname} \dusa{scomp} $|$ \moc{LEAD} $\}~]$ \\
...
\item[\moc{LEAD}] use liquid lead as coolant.
...
\item[\moc{DFM}] keyword used to specify the drift flux model option.

\item[\dusa{idfm}] integer value set to 0 for no drift flux model (default) or 1 to compute EPRI drift flux model (recommended for BWR modelling). 
Other correlations are available under 2 (Modified Bestion), 3 (GE-ramp) or 4 (Chexal). 
