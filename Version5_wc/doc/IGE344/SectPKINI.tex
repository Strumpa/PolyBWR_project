...
\dusa{MAPFL} \moc{:=} \moc{PKINI:} \dusa{MAPFL} $[$ \dusa{MACRO} $]$ \moc{::} \dstr{descpkini}
\end{DataStructure}
...
\item[\dusa{MACRO}] \texttt{character*12} name of an optional read-only \dds{macrolib} 
object containing point kinetics parameters (\moc{LAMBDA}, \moc{BETAI} and \moc{LAMBDAI}).
...
\item[\moc{LAMBDA}] keyword used to set the prompt neutron generation time. This information should not be given if object \dusa{MACRO}
is defined.

\item[\dusa{lambda}] prompt neutron generation time (s).

\item[\moc{NGROUP}] keyword used to set the number of delayed precursor groups. This information should not be given if object \dusa{MACRO}
is defined.

\item[\dusa{ngroup}] number of delayed precursor groups.

\item[\moc{BETAI}] keyword used to set the delayed neutron fraction vector. This information should not be given if object \dusa{MACRO}
is defined.

\item[\dusa{betai}] value of the delayed neutron fraction in a delayed precursor group.

\item[\moc{LAMBDAI}] keyword used to set the delayed neutron time constant vector. This information should not be given if object \dusa{MACRO}
is defined.
