...
\dusa{MACRO2} \moc{:=} \moc{OUT:} $[$ \dusa{MACRO2} $]~\{$ \dusa{FLUX} $|$  \dusa{IFLU} $\}$ \dusa{TRACK} \dusa{MACRO} \dusa{GEOM} \moc{::} \dstr{out\_data}
\end{DataStructure}
...
\item[\dusa{IFLU}] {\tt character*12} name of the \dds{interpflux} data structure ({\tt L\_FVIEW} signature) where the
interpolated flux distribution will be stored. This file is required if and only if the keyword \moc{VAL}
is set in data structure \dstr{out\_data}.
...
\hskip 0.5cm \moc{VAL} \dusa{nhom} $[[$ \moc{RECT} \dusa{xm} \dusa{xp} \dusa{ym} \dusa{yp} $[$ \moc{AXIAL} \dusa{zm} \dusa{zp} $]~]]~[[$ \moc{TRIA} \dusa{x1} \dusa{y1} \dusa{x2} \dusa{y2} \dusa{x3} \dusa{y3} $[$ \moc{AXIAL} \dusa{zm} \dusa{zp} $]~]]$ \\
\hskip 0.5cm $[[$ \moc{QUAD} \dusa{x1} \dusa{y1} \dusa{x2} \dusa{y2} \dusa{x3} \dusa{y3} \dusa{x4} \dusa{y4} $[$ \moc{AXIAL} \dusa{zm} \dusa{zp} $]~]]~[$ \moc{REMIX} (\dusa{imix2}(ii),ii=1,nhom) $]~\}~]$ \\
;
\end{DataStructure}
...
\item[\moc{VAL}] keyword used to specify that the homogenization will be based on the voxel information available in the \dds{interpflux} data structure \dusa{IFLU}. For
hexagonal geometries, the origin of the radial plane is the center of the central hexagon.

\item[\dusa{nhom}] number of homogeneous nodes to be defined using \moc{RECT} and/or \moc{TRIA} and/or \moc{QUAD} data structures. Many homogeneous mixtures can be defined by
repeating the \moc{RECT} and/or \moc{TRIA} and/or \moc{QUAD} data structures.
...
\item[\moc{REMIX}] the homogenization produced by option \moc{VAL} is further homogenized according to \dusa{imix2} indices.

\item[\dusa{imix2}] array of rehomogenized region numbers to which are associated the regions indices created {\sl after} the \moc{VAL} homogenization
was performed. In the editing routines a value of \dusa{imix2}=0 allows the corresponding region to be neglected.
