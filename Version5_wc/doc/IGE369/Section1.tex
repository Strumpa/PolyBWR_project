...
\item[\dusa{NAME1}] {\tt Character*12} name of a \dds{lcm} object.

\item[\dusa{NAME2}] {\tt Character*12} name of an \dds{xsm} file.

\item[\dusa{NAME3}] {\tt Character*12} name of a sequential binary file.

\item[\dusa{NAME4}] {\tt Character*12} name of a sequential \dds{ascii} file.
...
\subsection{The {\tt GEOD:} module}

The {\tt GEOD:} module is used to create or modify a geometry. The geometry definition module in TRIVAC permits all the characteristics (coordinates, material mixture type indices and boundary conditions) of a simple or complex geometry to be specified. The method used to specify the geometry is independent of the discretization module to be used subsequently. Each geometry is represented by a name ({\tt character*12}) and is saved in a \dds{lcm} object or an \dds{xsm} file under its given name. It is always possible to modify a given existing geometry or copy it into a neighbouring \dds{lcm} object under a new name. The calling specifications are:

\begin{DataStructure}{Structure \dstr{GEOD:}}
$\{$ \dusa{GEOM1} \moc{:=} \moc{GEOD: ::} \dstr{geo\_data1} $|$ \\
\dusa{GEOM1} \moc{:=} \moc{GEOD:} $\{$ \dusa{GEOM1} $|$ \dusa{GEOM2} $\}$ \moc{::} \dstr{geo\_data2} $\}$
\end{DataStructure}
...
\item[\dusa{GEOM1}] {\tt character*12} name of the \dds{lcm} object (type {\tt L\_GEOM}) that will contain the geometry.

\item[\dusa{GEOM2}] {\tt character*12} name of a \dds{lcm} object (type {\tt L\_GEOM}) containing the existing geometry. The type and all the characteristics of \dusa{GEOM2} will be copied onto \dusa{GEOM1}.
...
\subsubsection{Data input for module {\tt GEOD:}}\label{sect:geo_data1}
...
~~~~~~ \moc{HEX} \dusa{lh} $|$ \moc{HEXZ} \dusa{lh} \dusa{lz} $|$ \moc{MACRO} \dusa{lp} $\}$ \\
$[$ \moc{EDIT} \dusa{iprint} $]$ \\
$\{$ \dstr{descBC} \dstr{descMC} \dstr{descPOS} $|$ \dstr{descMACRO} $\}$
;
\end{DataStructure}
...
\item[\moc{MACRO}] keyword to specify that a {\sl do-it-yourself} type macro-geometry
description follows.

\item[\dusa{lp}] number of regions in the macro-geometry.

\item[\moc{EDIT}] keyword used to set \dusa{iprint}.

\item[\dusa{iprint}] index used to control the printing in module {\tt GEOD:}. =0 for no print; =1 for minimum printing (default value); =2 for printing the geometry state vector.
...
\item[\dstr{descMACRO}] structure used to specify the properties of {\sl do-it-yourself}
macro-geometries used by edition modules (see \Sect{descMACRO}).
...
\subsubsection{Do-it-yourself macro-geometry}\label{sect:descMACRO}

This {\sl do-it-yourself} macro-geometry is an abstract representation of a 2D or 3D geometry adapted to a simple description of an edition geometry. These geometries cannot be used to perform a tracking using any module in Sects.~\ref{sect:bivact},~\ref{sect:trivact} or \ref{sect:nsst}. The \dusa{lp} regions are defined in sequential order and can be remixed using the \moc{REMIX} keyword to produce fewer mixtures than regions. Structure \dstr{descMACRO} is defined as

\begin{DataStructure}{Structure \dstr{descMACRO}}
\moc{REGIONS} $[[~\{$ \\
\hskip 0.5cm \moc{RECT} \dusa{xm} \dusa{xp} \dusa{ym} \dusa{yp} $[$ \moc{AXIAL} \dusa{zm} \dusa{zp} $]~|$ \moc{TRIA} \dusa{x1} \dusa{y1} \dusa{x2} \dusa{y2} \dusa{x3} \dusa{y3} $[$ \moc{AXIAL} \dusa{zm} \dusa{zp} $]~|$ \\
\hskip 0.5cm \moc{QUAD} \dusa{x1} \dusa{y1} \dusa{x2} \dusa{y2} \dusa{x3} \dusa{y3} \dusa{x4} \dusa{y4} $[$ \moc{AXIAL} \dusa{zm} \dusa{zp} $]~|$ \\
\hskip 0.5cm \moc{HEXA} \dusa{x1} \dusa{y1} \dusa{x2} \dusa{y2} \dusa{x3} \dusa{y3} \dusa{x4} \dusa{y4} \dusa{x5} \dusa{y5} \dusa{x6} \dusa{y6} $[$ \moc{AXIAL} \dusa{zm} \dusa{zp} $]~|$ \\
\hskip 0.5cm \moc{POLY} \dusa{ncorn} (\dusa{x}(i),\dusa{y}(i),i=1,\dusa{ncorn}) $[$ \moc{AXIAL} \dusa{zm} \dusa{zp} $]$ \\
\hskip 1.35cm $\}~]]$ \\
$[$ \moc{CENTER} \dusa{x0} \dusa{y0} $[$ $\{$ \moc{NONE} $|$ \moc{REFL} $|$ \moc{ROTA} $\}~]~]$ \\
$[$ \moc{REMIX} (\dusa{ireg2}(ii),ii=1,\dusa{lp}) $]$
\end{DataStructure}

\noindent where
\begin{ListeDeDescription}{mmmmmmm}

\item[\dusa{lp}] number of homogeneous regions to be defined using \moc{RECT}, \moc{TRIA}, \moc{QUAD}, \moc{HEXA} and/or \moc{POLY} data structures. Many regions can be defined by
repeating the \moc{RECT}, \moc{TRIA}, \moc{QUAD}, \moc{HEXA} and/or \moc{POLY} data structures.

\item[\moc{REGIONS}] keyword used to specify a list of \dusa{lp} user-defined macro regions.

\item[\moc{RECT}] keyword used to specify a unique region based on a rectangular node. 
...
\item[\dusa{zm}] lower limit of the homogeneous node alonx Z--axis.

\item[\dusa{zp}] upper limit of the homogeneous node alonx Z--axis.

\item[\moc{TRIA}] keyword used to specify a unique region based on a triangular node.

\item[\moc{QUAD}] keyword used to specify a unique region based on a quadrilateral node. In this case, the vertex list must be provided in clockwise or anticlockwise order.

\item[\moc{HEXA}] keyword used to specify a unique region based on an hexagonal node. In this case, the vertex list must be provided in clockwise or anticlockwise order.

\item[\dusa{x1}] X--coordinate of the first vertex.

\item[\dusa{y1}] Y--coordinate of the first vertex.

\item[\dusa{x2}] X--coordinate of the second vertex.

\item[\dusa{y2}] Y--coordinate of the second vertex.

\item[\dusa{x3}] X--coordinate of the third vertex.

\item[\dusa{y3}] Y--coordinate of the third vertex.

\item[\dusa{x4}] X--coordinate of the fourth vertex of a quadrilateral or hexagon.

\item[\dusa{y4}] Y--coordinate of the fourth vertex of a quadrilateral or hexagon.

\item[\dusa{x5}] X--coordinate of the fifth vertex of an hexagon.

\item[\dusa{y5}] Y--coordinate of the fifth vertex of an hexagon.

\item[\dusa{x6}] X--coordinate of the sixth vertex of an hexagon.

\item[\dusa{y6}] Y--coordinate of the sixth vertex of an hexagon.

\item[\moc{POLY}] keyword used to specify a unique region based on an arbitrary polygonal node. In this case, the vertex list must be provided in clockwise or anticlockwise order.

\item[\dusa{ncorn}] number of vertices ($\ge 3$) defining the polygon.

\item[\dusa{x}] X--coordinate of a vertex of the polygon.

\item[\dusa{y}] Y--coordinate of a vertex of the polygon.

\item[\moc{AXIAL}] keyword used to set \dusa{zm} and \dusa{zp} in 3D cases.

\item[\moc{CENTER}] keyword used to specify the geometry center in the $X-Y$ plane. This keyword is useful to perform unfolding of a macro-geometry.

\item[\dusa{x0}] center alonx X--axis.

\item[\dusa{y0}] center alonx Y--axis.

\item[\moc{NONE}] no symmetry and no unfolding is to be performed (default option).

\item[\moc{REFL}] reflexion is assumed (adapted to S30 or SA60 symmetries).

\item[\moc{ROTA}] rotation is assumed (adapted to RA60 or R120 symmetries).

\item[\moc{REMIX}] some regions of the macro-geometry are merged according to \dusa{imix2} indices. By default, each region forms a mixture.

\item[\dusa{imix2}] array of redefined mixture indices to which are associated the regions indices created {\sl after} the \moc{MACRO} definition. A value of \dusa{ireg2}=0 allows the corresponding region to be neglected.
...
\item[\dusa{MACR1}] {\tt character*12} name of the \dds{lcm} object (type {\tt L\_MACROLIB}) containing the new Macrolib
...
\item[\dusa{MACR2}] {\tt character*12} name of the \dds{lcm} object (type {\tt L\_MACROLIB}) containing a read-only
...
The {\tt BIVACT:} module is used to perform  a BIVAC-type \dds{tracking} on a 1D/2D geometry.\cite{bivac,benaboud,SVAT1}
...
\item[\dusa{TRACK}] {\tt character*12} name of the \dds{lcm} object (type {\tt L\_BIVAC}) containing the \dds{tracking} information. If \dusa{TRACK} appears on the RHS, the previous settings will be applied by default.

\item[\dusa{GEOM}] {\tt character*12} name of the \dds{lcm} object (type {\tt L\_GEOM}) containing the geometry.
...
GEOD:} module but this value is insufficient if symmetries or mesh-splitting
are specified.
...
\subsection{The {\tt TRIVAT:} module}\label{sect:trivact}

The {\tt TRIVAT:} module is used to perform a TRIVAC-type \dds{tracking} on a
...
\item[\dusa{TRACK}] {\tt character*12} of the \dds{lcm} object (type {\tt L\_TRIVAC}) 
containing the \dds{tracking} information. If \dusa{TRACK} appears on the RHS, the
previous settings will be applied by default.

\item[\dusa{GEOM}] {\tt character*12} of the \dds{lcm} object (type {\tt
...
The {\tt BIVACA:} module is used to compute the finite element system matrices (type {\tt L\_SYSTEM}) corresponding to a BIVAC \dds{tracking} (type {\tt L\_BIVAC}) and to a set of nuclear properties (type {\tt L\_MACROLIB}). The calling specifications are:
...
\item[\dusa{SYST}] {\tt character*12} name of the \dds{lcm} object (type {\tt L\_SYSTEM}) containing the system matrices. If \dusa{SYST} appears on the RHS, the system matrices previously stored in \dusa{SYST} are kept.

\item[\dusa{MACRO}] {\tt character*12} name of the \dds{lcm} object (type {\tt L\_MACROLIB}) containing the macroscopic cross sections and diffusion coefficients.

\item[\dusa{TRACK}] {\tt character*12} name of the \dds{lcm} object (type {\tt L\_BIVAC}) containing the BIVAC \dds{tracking}.
...
The TRIVAA: module is used to compute the finite element system matrices (type {\tt L\_SYSTEM}) corresponding to a TRIVAC \dds{tracking} (type {\tt L\_TRIVAC}) and to a set of nuclear properties (type {\tt L\_MACROLIB}). The calling specifications are:
...
\item[\dusa{TRACK}] {\tt character*12} name of the \dds{lcm} object (type {\tt L\_TRIVAC}) containing the TRIVAC \dds{tracking}.

\item[\dusa{DMACRO}] {\tt character*12} name of the \dds{lcm} object  (type {\tt
...
\item[\moc{OVEL}] The reciprocal neutron velocities for each material mixture are recovered from the input \dds{macrolib} \dusa{MACRO} and used
...
\item[\dusa{FLUX}] {\tt character*12} name of the \dds{lcm} object (type {\tt L\_FLUX}) containing the solution. If \dusa{FLUX} appears on the RHS, the solution previously stored in \dusa{FLUX} is used to initialize the new iterative process; otherwise, a uniform unknown vector is used.

\item[\dusa{SYST}] {\tt character*12} name of the \dds{lcm} object (type {\tt L\_SYSTEM}) containing the system matrices.

\item[\dusa{TRACK}] {\tt character*12} name of the \dds{lcm} object (type {\tt L\_TRACK}) containing the \dds{tracking}.

\item[\dusa{MACRO}] {\tt character*12} name of the optional \dds{lcm} object (type {\tt L\_MACROLIB}) containing the cross sections. This
object is only used to set a link to the \dds{macrolib} name inside the \dds{flux} object. By default, the name of the \dds{macrolib} is recovered
from the link in the \dds{system} object.
...
recovered in the state vector of the \dds{tracking} object \dusa{TRACK}.
...
\item[\dusa{GPT}] {\tt character*12} name of the \dds{lcm} object (type {\tt L\_SOURCE}) containing the fixed source. If
\dusa{GPT} appears on the RHS, this information is used to initialize the state vector.

\item[\dusa{FLUX0}] {\tt character*12} name of the \dds{lcm} object (type {\tt L\_FLUX}) containing the unperturbed flux.

\item[\dusa{SYST0}] {\tt character*12} name of the \dds{lcm} object (type {\tt L\_SYSTEM}) containing the unperturbed system matrices.

\item[\dusa{DSYST}] {\tt character*12} name of the \dds{lcm} object (type {\tt L\_SYSTEM}) containing a perturbation to the system matrices.
...
\item[\dusa{FLUX\_GPT}] {\tt character*12} name of the \dds{lcm} object (type {\tt L\_FLUX}) containing the GPT solution. If \dusa{FLUX\_GPT} appears on the RHS, the solution previously stored in \dusa{FLUX\_GPT} is used to initialize the new iterative process; otherwise, a uniform unknown vector is used.

\item[\dusa{GPT}] {\tt character*12} name of the \dds{lcm} object (type {\tt L\_SOURCE}) containing the fixed sources.

\item[\dusa{FLUX0}] {\tt character*12} name of the \dds{lcm} object (type {\tt L\_FLUX}) containing the unperturbed flux used to decontaminate the GPT solution.

\item[\dusa{SYST}] {\tt character*12} name of the \dds{lcm} object (type {\tt L\_SYSTEM}) containing the unperturbed system matrices.
...
The {\tt OUT:} module is used to compute the reaction rates and to store them in an extended \dds{macrolib} (type {\tt L\_MACROLIB}) corresponding to a solution (type {\tt L\_FLUX}) of the matrix system. The calling specifications are:

\begin{DataStructure}{Structure \dstr{OUT:}}
\dusa{MACRO2} \moc{:=} \moc{OUT:} $[$ \dusa{MACRO2} $]$ \dusa{TRACK} \dusa{MACRO} \dusa{REFGEO} $\{$ \dusa{FLUX} $|$ \dusa{IFLU} \dusa{MACGEO} $\}$ \\
\hskip 0.5cm $~[$ \dusa{MATEX} $]$ \\
\hskip 0.5cm \moc{::} \dstr{out\_data}
\end{DataStructure}

\noindent where
\begin{ListeDeDescription}{mmmmmm}

\item[\dusa{MACRO2}] {\tt character*12} name of the \dds{lcm} object (type {\tt L\_MACROLIB}) containing the extended \dds{macrolib}.

\item[\dusa{TRACK}] {\tt character*12} name of the \dds{lcm} object (type {\tt L\_TRACK}) containing a \dds{tracking}.

\item[\dusa{MACRO}] {\tt character*12} name of the \dds{lcm} object (type {\tt L\_MACROLIB}) containing the reference \dds{macrolib}.

\item[\dusa{REFGEO}] {\tt character*12} name of the \dds{lcm} object (type {\tt L\_GEOM}) containing the reference \dds{geometry}.

\item[\dusa{FLUX}] {\tt character*12} name of the \dds{lcm} object (type {\tt L\_FLUX}) containing a solution.

\item[\dusa{IFLU}] {\tt character*12} name of the \dds{lcm} object (type {\tt L\_FVIEW}) where the
interpolated flux distribution is recovered. This \dds{lcm} object is required if and only if the keyword \moc{VAL}
is set in data structure \dstr{out\_data} of Sect.~\ref{sect:out_data}.

\item[\dusa{MACGEO}] {\tt character*12} name of the \dds{lcm} object (type {\tt L\_GEOM}) containing the macro-\dds{geometry} used
with the \moc{INTG} \moc{VAL} option. This information is described in Sect.~\ref{sect:descMACRO}.

\item[\dusa{MATEX}] {\tt character*12} name of the optional \dds{lcm} object (type {\tt L\_MATEX}) created by DONJON.
...
$[$ \moc{INTG} $\{$ \moc{NONE} $|$ \moc{COMP} $|$ \moc{MATX} $|$ \moc{IN} $|$ \moc{MIX} $|$ (\dusa{ihom}(i), i=1,nreg) $|$
\moc{VAL} \dusa{MACGEO}~$\}~]$ \\
$[$ \moc{PKIN} $]$ \\
;
\end{DataStructure}
...
\item[\moc{COMP}] keyword used to specify that a complete homogenization is to take place. 

\item[\moc{MATX}] keyword for computing the reaction rates on the geometry mesh before the {\tt USPLIT:} module was called in DONJON. In this case, the RHS object \dusa{MATEX} must be provided.
...
\item[\moc{VAL}] keyword used to specify that the homogenization will be based on the voxel information available in the data structure named \dusa{IFLU} ({\tt L\_FVIEW} signature) and macrogeometry information available in the data structure named \dusa{MACGEO} ({\tt L\_GEOM} signature). For
hexagonal geometries, the origin of the radial plane is the center of the central hexagon.

\item[\moc{PKIN}] keyword used to compute point kinetics parameters for the full core. This option activates option \moc{PROD} and requires an adjoint flux solution.
...
The {\tt ERROR:} module is used to compare reaction rates contained into two extended \dds{macrolibs} and to print statistics regarding the comparison.
...
\noindent where $T_{i,g}^{{\rm norm}*}$ is computed using the reference values (stored in \dusa{MACRO1}) and $N$ is the total number of regions in the \dds{macrolib}.
...
\item[\dusa{MACRO1}] {\tt character*12} name of the \dds{lcm} object (type {\tt L\_MACROLIB}) containing the extended \dds{macrolib} used to compute the reference reaction rates.

\item[\dusa{MACRO2}] {\tt character*12} name of the \dds{lcm} object (type {\tt L\_MACROLIB}) containing the extended \dds{macrolib} used to compute the approximate reaction rates.
...
The delayed neutron information can be provided directly from the input file or recovered from the \dds{macrolib}
data structure.
...
\item[\dusa{KINET}] {\tt character*12} name of the \dds{lcm} object (type {\tt L\_KINET}) to be created by the module.

\item[\dusa{MACRO}] {\tt character*12} name of the \dds{lcm} object (type {\tt L\_MACROLIB}) containing the \dds{macrolib} information.

\item[\dusa{TRACK}] {\tt character*12} name of the \dds{lcm} object (type {\tt L\_TRACK}) containing the \dds{tracking} information.

\item[\dusa{SYST}] {\tt character*12} name of the \dds{lcm} object (type {\tt L\_SYSTEM}) corresponding to \dds{macrolib} \dusa{MACRO}
and \dds{tracking} \dusa{TRACK}.

\item[\dusa{FLUX}] {\tt character*12} name of the \dds{lcm} object (type {\tt L\_FLUX}) containing the initial steady-state solution.
...
If these values are not provided, they should be recorded in the \dds{macrolib} data structure.
...
\item[\dusa{KINET}] {\tt character*12} name of the \dds{lcm} object (type {\tt L\_KINET}) in modification mode.

\item[\dusa{MACRO}] {\tt character*12} name of the \dds{lcm} object (type {\tt L\_MACROLIB}) containing the \dds{macrolib}
...
\item[\dusa{MACRO\_0}] {\tt character*12} name of the \dds{lcm} object (type {\tt L\_MACROLIB}) containing the \dds{macrolib}
...
\item[\dusa{SYST\_0}] {\tt character*12} name of the \dds{lcm} object (type {\tt L\_SYSTEM}) corresponding to \dds{macrolib} \dusa{MACRO\_0}
and \dds{tracking} \dusa{TRACK}.
...
The \moc{VAL:} module supplies an interpolation of the flux in diffusion or $SP_n$ calculations for Cartesian geometries and hexagonal geometries discretized
with a Raviart-Thomas-Schneider approximation. A Cartesian voxel matrix is produced in all cases. This module also provide {\sl flux reconstruction} with nodal
methods. The calling specifications are:
...
\item[\dusa{MACRO}] {\tt character*12} name of the \dds{lcm} object (type {\tt L\_MACROLIB}) containing the cross sections and diffusion coefficients.
...
\subsection{The {\tt NSST:} module}\label{sect:nsst}

The {\tt NSST:} module is used to perform a \dds{tracking} for the Nodal
...
\item[\dusa{FLUX}] {\tt character*12} name of the \dds{lcm} object (type {\tt L\_FLUX}) containing the solution. If \dusa{FLUX} appears on the RHS, the solution previously stored in \dusa{FLUX}
is used to initialize the new iterative process; otherwise, a uniform unknown vector is used.

\item[\dusa{TRACK}] {\tt character*12} name of the \dds{lcm} object (type {\tt L\_TRACK}) containing the \dds{tracking}.

\item[\dusa{MACRO}] {\tt character*12} name of the \dds{lcm} object (type {\tt L\_MACROLIB}) containing the cross sections, diffusion coefficients and discontinuity
factors.
