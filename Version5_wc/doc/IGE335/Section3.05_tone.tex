...
$[$ \moc{MXIT} \dusa{imxit} $]~~[$ \moc{EPS} \dusa{valeps} $]~~[$ \moc{TCEPS} \dusa{vtceps} $]$ \\
$[~\{$ \moc{SPH} $|$ \moc{NOSP} $\}~]$
$[~\{$ \moc{TRAN} $|$ \moc{NOTR} $\}~]$
$[~\{$ \moc{PIJ} $|$ \moc{ARM} $\}~]$ \\
{\tt ;}
...
\item[\moc{TCEPS}] keyword to specify the relative tolerance for skipping DOORPV or
DOORAV calls.

\item[\dusa{vtceps}] the convergence criterion for skipping DOORPV or
DOORAV calls. By default, \dusa{valeps}=$1.0\times 10^{-5}$.
