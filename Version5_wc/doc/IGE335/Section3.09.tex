...
\item[\moc{ENERGY}]  keyword used to set the incident neutron energy for fission yields. This keyword is not used if the Draglib contains a unique set of fission yields. By default, the most thermal set is used.

\item[\dusa{enrgyd}] energy (eV) of the incident neutron for fission yields. Typical values are 2.53e-2 eV for thermal reactors and 4.0e5 or 5.0e5 eV for fast reactors.
