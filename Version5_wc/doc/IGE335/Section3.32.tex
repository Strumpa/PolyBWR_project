...
\item[\moc{PARTICLE}] keyword used to specify the companion particle type
recovered from the {\sc macrolib}, that is, the primary or source state of the
transition. For a transition $q \rightarrow p$, this keyword specifies the
incident particle $q$ whose flux contributes to the source of $p$. The particle
$p$ being solved is defined by the input {\sc macrolib}. This keyword is
repeated for each type of companion particles, in the same order as the
\dusa{FLUX} objects on the RHS.
...
\item[\dusa{int}] source intensity. With \moc{ISO}, it is defined as the number of particles per volume unit per time unit. With \moc{MONO}, it is defined as the number of particles per surface unit per time unit.
