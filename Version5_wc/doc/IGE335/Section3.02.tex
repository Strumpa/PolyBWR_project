...
\item[\moc{PARTICLE}] keyword to specify the particle type associated with the
{\sc microlib} and its embedded {\sc macrolib}. For coupled particle
\moc{MATXS2} libraries, this particle is the secondary state of the transition.
Equivalently, for a transition $q \rightarrow p$, this keyword specifies the
produced particle $p$. Companion particles are the incident or source particles
$q$ whose flux contributes to the source of $p$. This option is only compatible
with a \moc{MATXS2} format library.

\item[\dusa{htype}] character*1 name of the particle. Usual names are {\tt N}:
neutrons, {\tt G}: photons, {\tt B}: electrons, {\tt C}: positrons and
{\tt P}: protons. If the \moc{PARTICLE} keyword is not provided with a
\moc{MATXS2} library, the first supported particle found in the MATXS2 file is
used. For other library formats, the default particle is the neutron.
...
\hskip 1.0cm $|$ \\
\hskip 1.0cm \moc{COMB} $[[~[$ \moc{TEMP} \dusa{tempi} $]$ \dusa{mati} \dusa{relvol} $]]~\}$
\end{DataStructure}
...
\item[\moc{COMB}]  keyword to specify that this mixture is reset with a combination of previously defined mixtures.

\item[\moc{TEMP}]  keyword to assign a different temperature to the new combination of mixtures.
By default, the temperature is not modified.

\item[\dusa{tempi}]  temperature (in Kelvin) assigned to the new combination of mixtures.
...
$\{$ \moc{MIX} \dusa{matnum} $[~\{$ \dusa{matold} $[$ \dusa{denmix} $]~|$ \moc{COMB} $[[$ \dusa{mati} \dusa{relvol} $]]~\}~]~[~\{$ \moc{NOEV} $|$ \moc{EVOL} $\}~]$ \\
\hskip 1.0cm $|$ \moc{ALL} $\}$
\end{DataStructure}
...
\item[\moc{MIX}] keyword to specify the number identifying the next mixture to be updated.
...
\item[\moc{COMB}] keyword to specify that mixture \dusa{matnum} is a combination of previously defined mixtures.

\item[\dusa{mati}] previously defined mixture identifier on \dusa{MICLIB}.

\item[\dusa{relvol}] relative volume $V_{i}$ occupied by mixture \dusa{mati}=$i$ in \dusa{matnum}.
