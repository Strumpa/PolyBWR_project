...
$[$ \moc{ANIS} \dusa{nanis} $]$ \\
$[~\{$  \moc{ONEG} $|$ \moc{ALLG}  $|$ \moc{XCLL} $\}~]~[$ \moc{BATCH} \dusa{nbatch} $]$ \\
$[~\{$  \moc{TREG}  $|$ \moc{TMER} $\}~]$ \\
...
\item[\moc{BATCH}] keyword to specify the number of tracks assigned to each OpenMP core. The default value is \dusa{iquab}$=1$.
