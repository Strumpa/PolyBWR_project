...
$\{$ \dstr{descBC} \dstr{descSP} \dstr{descPP} \\
~~~~$|$ \dstr{descSIJ} \\
~~~~$|$ \dstr{descMACRO} $\}$ \\
...
\item[\moc{GROUP}] keyword to specify that a {\sl do-it-yourself} type geometry for the interface current method
description follows.
...
geometries for the interface current method (see \Sect{descSIJ}).
...
\item[\moc{HTMERGE}] keyword to merge the triangle in hexagonal rings for \moc{HEXT}, \moc{HEXTZ}, \moc{HEXTCEL} and \moc{HEXTCELZ} geometries. 

\item[\dusa{ihtm}] merge level for \moc{HTMERGE}. By default, \dusa{ihtm}=0 and no merging takes place. The value \dusa{ihtm}=1 implies merging the triangle on an hexagonal ring independently for each sector. This is possible only if the mixtures are defined per ring for all sectors or per ring per sector. The value \dusa{ihtm}=2 implies merging the triangle on an hexagonal ring. This is possible only if the mixtures are defined per ring for all sectors.
...
\subsubsection{Do-it-yourself geometry for the interface current method}\label{sect:descSIJ}

This {\sl do-it-yourself} geometry is an abstract representation of an assembly of arbitrary unit-cells defined in term of their probability of presence and of their probability to have a particular neighbor. This definition is adapted a naive implementation of the interface current method. Structure \dstr{descSIJ} is defined as
...
\moc{CELL}  (\dusa{HCELL}(i),i=1,\dusa{lp})
\end{DataStructure}
...
\item[\moc{CELL}] keyword to specify the description of the cells.

\item[\dusa{HCELL}] array of sub-geometry {\tt character*12} names containing the
geometric description of each cell. 
...
This {\sl do-it-yourself} macro-geometry is an abstract representation of a 2D or 3D geometry adapted to a simple description of an edition geometry. These geometries cannot be used to perform a tracking with any module in Sect.~\ref{sect:TRKData}. The \dusa{lp} regions are defined in sequential order and can be remixed using the \moc{REMIX} keyword to produce fewer mixtures than regions. Structure \dstr{descMACRO} is defined as
