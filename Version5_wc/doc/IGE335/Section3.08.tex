...
\hskip 0.5cm $[~\{$ \dusa{REFGEO} $[$ \dusa{MACGEO1} $]~|$ \dusa{SURFIL} \dusa{MACGEO2}~$\}~]~[$ \dusa{REFPIJ} $]$ \moc{::} \dstr{descedi}
\end{DataStructure}
...
\item[\dusa{MACGEO1}] {\tt character*12} optional name of the read-only macro-\dds{geometry} data
...
file used to store the surfacic elements of the geometry. This file is required if and only if \moc{MERG} \moc{G2S}
keywords are set in Sect.~\ref{sect:descedi}.

\item[\dusa{MACGEO2}] {\tt character*12} name of the read-only macro-\dds{geometry} data
structure ({\tt L\_GEOM} signature) containing the information used with the \moc{MERG} \moc{G2S} option. This
information is described in Sect.~\ref{sect:descMACRO}.
...
$[$ \moc{UPS} $]$ \\
$[$ \moc{MERG} $\{$ \moc{NONE} $|$ \moc{COMP} $|$ \moc{GEO} $|$ \moc{HMIX} $|$ \moc{G2S} \dusa{MACGEO2} $|$ \moc{REGI} (\dusa{iregm}(ii),ii=1,nregio) $|$ \\
...
$[$ \moc{EDI\_CURR} $]~[$ \moc{GOLVER} $]~[$ \moc{REBUILD} $]~[$ \moc{WNORM} $]~[$ \moc{ONEFIS} $]$ \\
...
are automatically computed by comparing the reference geometry \dusa{REFGEO} with the macro-geometry \dusa{MACGEO1} ({\tt L\_GEOM} signature).
...
file \dusa{SURFIL} and macro-geometry information available in the data structure named \dusa{MACGEO2} ({\tt L\_GEOM} signature).
...
\item[\moc{REBUILD}] keyword used to rebuild the embedded macrolib from condensed/homogenized microlib information. By default, the embedded macrolib
is build without using microlib information. The "\moc{MICR RES REBUILD}" options are useful to create \dds{macrolib} data with a unique fission spectrum
without making approximations.

\item[\moc{WNORM}] keyword used to normalize the within-group scattering cross sections to be consistent with {\tt SIGS} information. By default,
the within-group scattering cross sections are not renormalized.

\item[\moc{ONEFIS}] keyword used to average the fission information contained in the embedded macrolib so as to obtain a unique fission spectrum. By
default, the macrolib information is generated with different fission spectra, one for each fissile isotope.
...
\item[\dusa{MACGEO1}] character*12 name of the macro-geometry.
