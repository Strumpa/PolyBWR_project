...
\item[\moc{ANDERSON}] keyword to activate Anderson acceleration of the SPH fixed-point iterations.

\item[\dusa{m}] window size of the Anderson acceleration algorithm. \dusa{m}$+1$ values of the previous iterates and gradients are kept
in the \dusa{OPTIM} object. \dusa{m} $=0$ is equivalent to free iterations (default option). \dusa{m} should be equal or smaller than
the total number of SPH factors. A value smaller or equal to 10 is recommended.
