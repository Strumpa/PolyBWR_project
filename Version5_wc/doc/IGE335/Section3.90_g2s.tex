...
structure. This structure may be build using the operator {\tt GEO:} (see \Sect{GEOData}). It is also possible to display
a macro-geometry, as defined in Sect.~\ref{sect:descMACRO}.
...
$[$~\moc{MACRO} $]$ \\
$[~\{$~\moc{DRAWNOD} $|$ \moc{DRAWMIX} $|$ \moc{DRAWELEM} $|$ \moc{DRAWMACRO} $\}~]~[$ \moc{ZOOMX} \dusa{facx1} \dusa{facx2}
$]~[$ \moc{ZOOMY} \dusa{facy1} \dusa{facy2} $]$ \\
\moc{;}
\end{DataStructure}
...
\item[\moc{DRAWMACRO}] keyword used to print the macro indices on the LHS postscript plot \dusa{PSFIL}. By default, no indices are printed.
