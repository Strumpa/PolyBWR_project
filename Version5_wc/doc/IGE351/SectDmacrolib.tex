...
\end{array} \right.
\end{displaymath}
\item Point kinetics information $I_{\rm pkin}=\mathcal{S}^{M}_{18}$:
\begin{displaymath}
I_{\rm pkin} = \left\{
\begin{array}{ll}
0 & \textrm{no point kinetics information available}\\
1 & \textrm{point kinetics information is available.}
\end{array} \right.
\end{displaymath}
\end{itemize}
...
\OptCharEnr
  ...
  {\tt C} (positrons) and {\tt P} (protons). In coupled sets, this is the
  selected secondary particle. If no particle is explicitly selected when
  reading a \moc{MATXS2} library, this record is set to the first supported
  particle found in the file.}
\OptCharEnr
...
\OptRealVar
  ...
  For example, {\tt GENERGY} contains the energy mesh of companion photons ($G_i+1$ values).}
\OptDirlVar
...
\OptDirEnr
  {P-KINETICS\blank{2}}{$I_{\rm pkin} = 1$}
  {Point kinetics--related input data as presented in \Sect{macroPKIN}.}
...
\subsection{The \moc{/P-KINETICS/} sub-directory in \dir{macrolib}}\label{sect:macroPKIN}

The \moc{P-KINETICS} sub-directory contains point kinetics information.

\vskip 0.08cm

\begin{DescriptionEnregistrement}{Records in \moc{P-KINETICS} sub-directory}{7.5cm}

\RealEnr
 {LAMBDA\blank{6}}{$1$}{s} {Prompt neutron generation time.}

\RealEnr
 {BETAI\blank{7}}{$N_{d}$}{} {Delayed neutron fraction vector.}

\RealEnr
 {LAMBDAI\blank{5}}{$N_{d}$}{s} {Delayed neutron time-constant vector.}
