...
\item $\mathcal{S}^{t}_{30}$ is the number of perimeters surrounding the global geometry. For Cartesian 2D and 3D geometries, this value is generally equal to 4 and
6, respectively. It can be higher for hexagonal geometries.
...
\IntEnr
  {ICODE\blank{7}}{$\mathcal{S}^{t}_{30}$}
  {array ${\cal I}_{\beta,k}$ containing the surface albedo index on each perimeter of the geometry (geometric surface albedo
  $\beta_{{\rm g},k}$ are used if ${\cal I}_{\beta,k}< 0$ while physical 
  surface albedo $\beta_{{\rm p},k}$ are used if ${\cal I}_{\beta,k}> 0$).} 
\RealEnr
  {ALBEDO\blank{6}}{$\mathcal{S}^{t}_{30}$}{}
  {array $\beta_{{\rm g},k}$ containing the geometric surface albedo on each perimeter of the geometry (used only if ${\cal I}_{\beta,k}\ge 0$).}
\OptDirEnr
