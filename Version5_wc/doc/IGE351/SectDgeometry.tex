\section{Contents of a \dir{geometry} directory}\label{sect:geometrydir}
...
30 & \textrm{Do-it-yourself geometryfor the interface current method} \\
31 & \textrm{Do-it-yourself macro geometry}
\end{array} \right.
\end{displaymath}
\eject
...
\item The type of sectorization $F_{\mathrm{sec}}=\mathcal{S}^{G}_{14}$.
...
\item  The merge option for triangles $\mathcal{S}^{G}_{19}$. If $=0$, no merging takes place. The value $=1$ implies merging the triangle on an hexagonal ring independently for each sector. This is possible only if the mixtures are defined per ring for all sectors or per ring per sector. The value $=2$ implies merging the triangle on an hexagonal ring.
...
\OptDirEnr
  ...
  {Directory containing double-heterogeneity related data. This directory can only be present on the root directory. This directory is described in Sect.~\ref{sect:geometrybihet}.}
\OptDirEnr
  {MACRO-GEOM\blank{2}}{$F_{t}=40$}
  {Directory containing macro-geometry related data. This directory can only be present on the root directory. This directory is described in Sect.~\ref{sect:geometrymacro}.}
\OptRealEnr
...
\subsection{The \moc{/MACRO-GEOM/} sub-directory in \dir{geometry}}\label{sect:geometrymacro}

The first level of the geometry directory may contains a macro-geometry directory \moc{/MACRO-GEOM/} made of the
following records:

\begin{DescriptionEnregistrement}{Records in the \moc{/MACRO-GEOM/} sub-directory}{7.5cm}
\IntEnr
  {STATE-VECTOR}{$40$}
  {Vector describing the various parameters associated with this data structure $\mathcal{S}^{mg}_{i}$}
\IntEnr
  {ITNODE\blank{6}}{$\mathcal{S}^{mg}_{2}$}
  {The type $I_m$ of each basic mixture $m$. $I_m=1$: simple rectangle of type {\tt RECT} (the sides of a  rectangle are assumed to be parallel to the $X-$ and $Y-$ axes).
   $I_m\ge 3$: number of sides of a polygon ($=3$: {\tt TRIA}, $=4$: {\tt QUAD}, $=6$: {\tt HEXA}).
  }
\RealEnr
  {NODE\blank{8}}{$\mathcal{S}^{mg}_{1},\mathcal{S}^{mg}_{2}$}{cm}
  {The node locations for the basic mixtures in the radial plane. {\tt NODE} is a sequence of $(x,y)$ pairs. If $I_m=1$, four values ($x_-$, $x_+$, $y_-$, $y_+$) are required per mixture.
  If $I_m\ge 3$, $2\times I_m$ values are required per mixture. The sequence of vertices is given in clockwise or anti-clockwise order.}
\OptRealEnr
  {ZNODE\blank{7}}{$2,\mathcal{S}^{mg}_{2}$}{$\mathcal{S}^{mg}_{5}=3$}{cm}
  {The axial node locations ($z_-$, $z_+$) for the basic mixtures in 3D cases}
\OptIntEnr
  {IREMIX\blank{6}}{$\mathcal{S}^{mg}_{2}$}{$\mathcal{S}^{mg}_{4}>0$}
  {The merge index assigned to each basic mixture}
\OptRealEnr
  {CENTER\blank{6}}{2}{*}{cm}
  {Center of the global macro-geometry}
\end{DescriptionEnregistrement}

The dimensioning parameters for this data structure, which are stored in the state vector $\mathcal{S}^{mg}$, represent:

\begin{itemize}

\item The first dimension of array {\tt NODE} $\mathcal{S}^{mg}_{1}$ 

\item The number of basic mixtures described with keywords {\tt RECT}, {\tt TRIA}, {\tt QUAD} or {\tt HEXA} $\mathcal{S}^{mg}_{2}$ 

\item Type of symmetry for the global macro-geometry $\mathcal{S}^{mg}_{3}$ 
\noindent where
\begin{displaymath}
\mathcal{S}^{mg}_{3} = \left\{
\begin{array}{rl}
 0 & \textrm{Undefined} \\
 1 & \textrm{{\tt NONE} symmetry} \\
 2 & \textrm{{\tt REFL} symmetry} \\
 3 & \textrm{{\tt ROTA} symmetry} \\
\end{array} \right.
\end{displaymath}

\item The number of mixtures remaining after merging of basic mixtures. Equal to zero if no merging is done. $\mathcal{S}^{mg}_{4}$ 
\item The number of dimensions $\mathcal{S}^{mg}_{5}$ 
\noindent where
\begin{displaymath}
\mathcal{S}^{mg}_{5} = \left\{
\begin{array}{rl}
 2 & \textrm{2D} \\
 3 & \textrm{3D} \\
\end{array} \right.
