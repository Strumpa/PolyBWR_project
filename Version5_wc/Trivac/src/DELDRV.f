...
*----
      ...
      CHARACTER TEXT4*4
      TYPE(C_PTR) JPFLU0,JPFLU1,JPFLU2,JPGPT,KPGPT,JPSYSP,KPSYSP
...
*----
   20 CALL LCMGET(IPFLU0,'K-EFFECTIVE',FKEFF)
      CALL LCMLEN(IPFLU0,'FLUX',LENGT,ITYLCM)
      IF(LENGT.EQ.0) THEN
         CALL LCMLEN(IPFLU0,'MODE',LENGT,ITYLCM)
         IF(LENGT.GT.0) THEN
            JPFLU0=LCMGID(IPFLU0,'MODE')
            IPFLU0=LCMGIL(JPFLU0,1)
         ELSE
            CALL LCMLIB(IPFLU0)
            CALL XABORT('DELDRV: UNABLE TO RECOVER A DIRECT FLUX.')
         ENDIF
      ENDIF
      JPFLU1=LCMGID(IPFLU0,'FLUX')
      CALL LCMLEN(IPFLU0,'AFLUX',LENGT,ITYLCM)
      IF(LENGT.EQ.0) THEN
         CALL LCMLEN(IPFLU0,'MODE',LENGT,ITYLCM)
         IF(LENGT.GT.0) THEN
            JPFLU0=LCMGID(IPFLU0,'MODE')
         ...
         ELSE
            CALL LCMLIB(IPFLU0)
            CALL XABORT('DELDRV: UNABLE TO RECOVER AN ADJOINT FLUX.')
         ENDIF
      ENDIF
