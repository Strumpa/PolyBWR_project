...
*  LOCAL VARIABLES
*----
      LOGICAL LFI,LCH,LOV,LD,LDX,LDY,LDZ,LHF,LSC,LSO,LDI,LBI
      DOUBLE PRECISION DFLOTT
      CHARACTER CM*2,TEXT4*4,TEXT8*8,TEXT*8
      TYPE(C_PTR) JPLIST,KPLIST
      REAL, DIMENSION(:), ALLOCATABLE :: WORK
      REAL, DIMENSION(:,:), ALLOCATABLE :: ZNUG,CHI,OVERV,DIFFX,DIFFY,
     1 DIFFZ,H,S
      REAL, DIMENSION(:,:,:), ALLOCATABLE :: TOTAL,NUSDL,CHDL
...
*----
      ALLOCATE(TOTAL(NMIXT,NGRP,10),ZNUG(NMIXT,NGRP),CHI(NMIXT,NGRP),
...
*        RECOVER THE EXISTING MACROLIB DATA.
         ...
         DO 20 IW=0,9
         WRITE(TEXT8,'(4HNTOT,I1)') IW
         CALL LCMLEN(KPLIST,TEXT8,ILENGT,ITYLCM)
         IF(ILENGT.EQ.NMIXT) THEN
            NW=MAX(NW,IW)
            CALL LCMGET(KPLIST,TEXT8,TOTAL(1,JGR,IW+1))
  ...
  25     CONTINUE        
         ...
         ELSE IF(ILENGT.GT.0) THEN
            ...
            DO 35 IBM=1,NMIXT
            IJJ0=IJJ(IBM,IL,JGR)
            DO 30 IGR=IJJ0,IJJ0-NJJ(IBM,IL,JGR)+1,-1
            IPOSDE=IPOSDE+1
...
*
   ...
   70    CALL REDGET(INDIC,NITMA,FLOTT,TEXT8,DFLOTT)
         ...
         IF((TEXT8.EQ.'TOTAL').OR.(TEXT8.EQ.'NTOT0')) THEN
            DO 80 IGR=1,NGRP
            CALL REDGET(INDIC,NITMA,TOTAL(IBM,IGR,1),TEXT4,DFLOTT)
            IF(INDIC.NE.2) CALL XABORT('MACXSI: REAL DATA EXPECTED.')
   80       CONTINUE
         ELSE IF(TEXT8(:4).EQ.'NTOT') THEN
            READ(TEXT8(5:5),'(I1)') IW
            IF(IW.GT.9) CALL XABORT('MACXSI: NW OVERFLOW.')
            NW=MAX(NW,IW)
            DO 85 IGR=1,NGRP
            CALL REDGET(INDIC,NITMA,TOTAL(IBM,IGR,IW+1),TEXT4,DFLOTT)
            IF(INDIC.NE.2) CALL XABORT('MACXSI: REAL DATA EXPECTED.')
   85       CONTINUE
...
*
  160 JPLIST=LCMLID(IPLIST,'GROUP',NGRP)
      ...
      DO 165 IW=0,NW
        WRITE(TEXT8,'(4HNTOT,I1)') IW
        CALL LCMPUT(KPLIST,TEXT8,NMIXT,2,TOTAL(1,JGR,IW+1))
  165 CONTINUE
...
*----
      DEALLOCATE(TOTAL,ZNUG,CHI,NUSDL,CHDL,OVERV,DIFFX,DIFFY,DIFFZ,
     1 H,S,SCAT,WORK)
