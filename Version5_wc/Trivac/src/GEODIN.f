...
*----
     ...
     2 DIR*1
      INTEGER ISTATE(NSTATE),JSTATE(NSTATE),NCODE(6),ICODE(6)
      REAL ZCODE(6),CENTER(2)
...
*  ALLOCATABLE ARRAYS
*----
      INTEGER, ALLOCATABLE, DIMENSION(:) :: MIX,MERGE,TURN,MESH,ITNODE,
     1 IREMIX
      REAL, ALLOCATABLE , DIMENSION(:) :: RMESH,XR0,RR0,ANG
...
*
      ...
      IF(ITYPL.NE.3) CALL XABORT('GEODIN: CHARACTER DATA EXPECTED(1).')
...
*
   50 CALL REDGET(ITYPL,INTLIR,REALIR,CARLIR,DBLLIR)
      IF(ITYPL.NE.3) CALL XABORT('GEODIN: CHARACTER DATA EXPECTED(2).')
   60 IF(CARLIR.EQ.'EDIT') THEN
        CALL REDGET(ITYPL,IMPX,REALIR,CARLIR,DBLLIR)
        IF(ITYPL.NE.1) CALL XABORT('GEODIN: INTEGER DATA EXPECTED.')
...
*       LOCATIONS.
   ...
   70   I=I+1
     ...
     8    (CARLIR.EQ.';').OR.(CARLIR.EQ.':::')) GO TO 90
           IF(I.GT.LREG) CALL XABORT('GEODIN: MIX/CELL INDEX OVERFLOW.')
           DO 80 J=1,I-1
           JKG=-MIX(J)
   ...
   90   CONTINUE
     ...
     1       //'ER OF TIMES.')
           ...
           I=JREP+1
           CALL REDGET(ITYPL,INTLIR,REALIR,CARLIR,DBLLIR)
           IF(ITYPL.NE.3) CALL XABORT('GEODIN: CHARACTER DATA EXPECTED'
     1     //'.')
        ENDIF
        IF(LTOT) LREG=I-1
...
*       INPUT CARTESIAN COORDINATES.
        ...
        DO 100 I=1,LMESH
        CALL REDGET(ITYPL,INTLIR,RMESH(I),TEXT12,DBLLIR)
        IF(ITYPL.NE.2) CALL XABORT('GEODIN: REAL DATA EXPECTED.')
        IF(I.GT.1) THEN
...
*       INPUT THE SIDE LENGTH IN TRIANGULAR OR HEXAGONAL GEOMETRY.
     ...
     1  //'HIBITED.')
        CALL REDGET(ITYPL,INTLIR,SIDE,CARLIR,DBLLIR)
        IF(ITYPL.NE.2) CALL XABORT('GEODIN: REAL DATA EXPECTED.')
...
*       OPTIONS FOR CYLINDRICAL CORRECTION IN CARTESIAN GEOMETRY.
     ...
     1  //'IN1: OPTION RADS IS LIMITED TO CARTESIAN GEOMETRIES.')
        ...
        IF(INDIC.NE.1) CALL XABORT('GEODIN: INTEGER DATA EXPECTED.')
...
*       INPUT MESH SPLITTING FACTORS.
        ...
        ALLOCATE(MESH(LMESH))
        DO 140 I=1,LMESH
        CALL REDGET(ITYPL,MESH(I),REALIR,TEXT12,DBLLIR)
        IF(ITYPL.NE.1) CALL XABORT('GEODIN: INTEGER DATA EXPECTED.')
...
*       DEFINE DO-IT-YOURSELF MACRO-REGIONS.
        IF(ISTATE(1).NE.31) CALL XABORT('GEODIN: REGIONS - KEYWORD LI'
        ...
        DO IM=1,LX
          ...
          ELSE IF(CARLIR.EQ.'POLY') THEN
            ...
            IF(ITNODE(IM).LT.3) THEN
              CALL XABORT('GEODIN: NCORN SHOULD BE >= 3.')
            ELSE IF(2*ITNODE(IM).GT.MAXNOD) THEN
              CALL XABORT('GEODIN: MAXNOD OVERFLOW.')
            ENDIF
            DO I=1,2*ITNODE(IM)
              CALL REDGET(ITYPL,INTLIR,NODE(I,IM),CARLIR,DBLLIR)
              IF(ITYPL.NE.2) CALL XABORT('GEODIN: REAL DATA EXPECTED.')
            ENDDO
          ELSE
            CALL XABORT('GEODIN: *RECT*, *TRIA*, *QUAD*, *HEXA* OR *PO'
     1      //'LY* KEYWORD EXPECTED.')
          ...
          IF(CARLIR.EQ.'AXIAL') THEN
            ...
            IF(ITYPL.NE.2) CALL XABORT('GEODIN: REAL DATA EXPECTED.')
            CALL REDGET(ITYPL,INTLIR,REALIR,CARLIR,DBLLIR)
            IF(ITYPL.NE.3) CALL XABORT('GEODIN: CHARACTER DATA EXPECT'
     1      //'ED.')
          ENDIF
        ENDDO
...
*
        IF(CARLIR.EQ.'CENTER') THEN
          ...
          ELSE IF(CARLIR.EQ.'ROTA') THEN
            JSTATE(3)=3
          ELSE
            CALL XABORT('GEODIN: NONE, REFL OR ROTA KEYWORD EXPECTED.')
          ENDIF
