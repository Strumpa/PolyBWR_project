...
*----
      ...
      CALL LCMLEN(KEN(5),'FLUX',LENGT,ITYLCM)
      IF(LENGT.GT.0) THEN
         MPFLX=LCMGID(KEN(5),'FLUX')
      ELSE
         CALL LCMLEN(KEN(5),'MODE',LENGT,ITYLCM)
         IF(LENGT.GT.0) THEN
            JPFLX=LCMGID(KEN(5),'MODE')
            KPFLX=LCMGIL(JPFLX,1)
            MPFLX=LCMGID(KPFLX,'FLUX')
         ELSE
            CALL LCMLIB(KEN(5))
            CALL XABORT('KINST1: UNABLE TO RECOVER A DIRECT FLUX.')
         ENDIF
      ENDIF
      DO 10 IGR=1,NGR
      CALL LCMGDL(MPFLX,IGR,EVECT(1,IGR))
   10 CONTINUE
*----
