...
& IYLG,SXY,VNORM,MATXYZ,VALUE)
  ...
  & MATXYZ(IXLG,IYLG)
  REAL X(IXLG),Y(IYLG),SIDE,EVECT(NUN),SXY(2),VNORM,VALUE(IXLG,IYLG)
  !----
  !  LOCAL VARIABLES
  !----
  INTEGER I,J,ISU,I1,I2,IE,IC,ICMIN,ICMAX,NBC,ISPX,ISPY,IND,IUN,IEL
  LOGICAL FVALUE
  DOUBLE PRECISION COEF(2,5),FLX(5),FLY(5),U,V,AXY(2),BXY(2),CXY(2),CENTER(2), &
  & SUMMIT(2,6),NRING,ANGLE,RADIUS,SIDEH,XYC(2),XYR(2),DVOL1,DVOL2,VSIZE
  CHARACTER HSMG*131
  ...
  DO IC=1,LXH
    DO ISPY=1,ISPLH
      DO ISPX=1,ISPLH
        ...
        DO ISU=1,3
          IF(ISS(ISU,IEL)>0) DVOL1=DVOL1+2.0D0*SQRT(3.0D0)*SIDE**2/REAL(ISPLH**2)
        ENDDO
      ENDDO
    ENDDO
  ENDDO
