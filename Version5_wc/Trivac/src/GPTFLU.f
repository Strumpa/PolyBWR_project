...
*  LOCAL VARIABLES
*----
      INTEGER   IPRINT,NITMA,I,J,IGR,ITYP,LENGT,ITYLCM,SIGNA(3),ITY,
     1          NSTART
...
*----
      ...
      TYPE(C_PTR) IPFLUP,IPFLU,JPFLU,IPGPT,IPTRK,IPSYS,JPFLU1,JPFLU2,
     1          JPGPT,KPGPT,JPFLUP,KPFLUP
*----
*  VALIDITY OF OBJECTS
*----
...
*----
      ...
      IF(LENGT.EQ.0) THEN
         ...
         ELSE
            CALL LCMLIB(IPFLU)
            CALL XABORT('GPTFLU: UNABLE TO RECOVER A DIRECT FLUX.')
         ENDIF
      ENDIF
      JPFLU1=LCMGID(IPFLU,'FLUX')
      CALL LCMLEN(IPFLU,'AFLUX',LENGT,ITYLCM)
      IF(LENGT.EQ.0) THEN
         CALL LCMLEN(IPFLU,'MODE',LENGT,ITYLCM)
         IF(LENGT.GT.0) THEN
            JPFLU=LCMGID(IPFLU,'MODE')
         ...
         ELSE
            CALL LCMLIB(IPFLU)
            CALL XABORT('GPTFLU: UNABLE TO RECOVER AN ADJOINT FLUX.')
         ENDIF
      ENDIF
